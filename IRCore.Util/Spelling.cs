...
namespace IRCore.Util
{
    ...
    {
        public static string SpellCorrectWord(string text)
        {
            var newText = string.Empty;

            var caminnhoArquivoTxt = ConfigurationManager.AppSettings["SpellingPTBR"] ?? string.Empty;

            if (!File.Exists(caminnhoArquivoTxt))
            {
                return text;
            }

            var sample = File.ReadAllText(caminnhoArquivoTxt);
            var corpus = new Corpus(sample);
            var corrector = new SpellCorrect(corpus);

            var words = text.Split(' ');

            newText = words.Aggregate(newText, (current, word) => current + string.Format("{0} ", corrector.Correct(word)));
