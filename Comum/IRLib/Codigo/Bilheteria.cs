...
namespace IRLib
{
    ...
    public partial class Bilheteria : MarshalByRefObject
    {
        ...
        public const string HORARIO = "Hor�rio";
        ...
        public const string PRECO = "Pre�o";
        ...
        public const string MSG_INGRESSO_PRECO = "A quota de ingressos nesse pre�o j� atingiu sua quantidade m�xima.";
        public const string MSG_INGRESSO_CANAL = "A quota de ingressos no seu canal j� atingiu sua quantidade m�xima.";
        public const string MSG_INGRESSO_CLIENTE = "A quota de ingressos por cliente j� atingiu sua quantidade m�xima.";
        public const string MSG_PACOTE_DISPONIVEL = "A quota de pacotes j� atingiu sua quantidade m�xima.";
        public const string MSG_PACOTE_CANAL = "A quota de pacotes no seu canal j� atingiu sua quantidade m�xima.";
        private const int ADM_OSESP_CANAL_ID = 444;
        private const int ADM_OSESP_LOJA_ID = 343;
        public const int INTERNET_EMPRESA_ID = 1;
        public const string INTERNET_MOTIVO_REIMPRESSAO = "Reimpress�o Internet";
        private const string GRUPO = "GRUPO";
        private const string CLASSIFICACAO = "Classificacao";
        public const string PENDENTE_CANCELAMENTO = "PendenteCancelamento";
        ...
        /// </summary>
        /// <param name="senha">Senha de pr�-reserva</param>
        /// <param name="canalID">Canal que est� realizando a troca</param>
        /// <returns></returns>
        public DataSet PesquisaPreReserva(string senha, int canalID)
        {
            ...
            try
            {
                    ...
                    "ORDER BY i.Codigo"))
                {
                    ...
                    while (oDataReader.Read())
                    {
                        if (bd.LerString("Status") != Ingresso.PRE_RESERVA)
                            throw new BilheteriaException("A senha '" + senha + "' n�o � referente a uma pr�-reserva.");

                        if (!bd.LerBoolean("DisponivelVenda"))
                            throw new BilheteriaException("A apresenta��o: " + bd.LerDateTime("Horario") + " do evento: " + bd.LerString("Evento") + " n�o est� dispon�vel para venda.");
                        ...
                        if (retorno.Tables[TABELA_EVENTO_TAXA_ENTREGA].Select("EventoID = " + bd.LerInt("EventoID")).Length == 0)
                        {
                            ...
                            // Busca regi�es de entrega do Evento
                ...
                {
                    // Lista de eventos da pr�-reserva
                    ...
                    // Valida��o se o evento est� dispon�vel para o canal
                    using (IDataReader oDataReader = bd.Consulta("" +
                        "SELECT " +
                        ...
                        "   CanalID = " + canalID))
                    {
                        if (oDataReader.Read())
                        {
                            if (bd.LerInt("TotalEventosCanal") != lstEventoID.Count)
                                throw new BilheteriaException("O evento n�o est� dispon�vel para o seu canal.");
                        }
                        else
                            throw new BilheteriaException("O evento n�o est� dispon�vel para o seu canal.");
                    }
                    ...
                    "WHERE tEvento.ID IN ( " + Utilitario.ArrayToString(lstEventoID.ToArray()) + ")"))
                    {
                        while (oDataReader.Read())
                        {
                            // Alimenta as informa��es do evento
        ...
        public IRLib.ClientObjects.EstruturaEventoInfo CarregarEventoInfo(int eventoID)
        {
            BD bd = new BD();
            ...
            try
            {
                ...
                IRLib.ClientObjects.EstruturaEventoInfo eventoInfo = new IRLib.ClientObjects.EstruturaEventoInfo();
        ...
        public IRLib.ClientObjects.EstruturaLocalInfo CarregarLocalInfo(int localID)
        {
            BD bd = new BD();
            ...
            try
            {
                ...
                IRLib.ClientObjects.EstruturaLocalInfo localInfo = new IRLib.ClientObjects.EstruturaLocalInfo();
        ...
        /// <summary>
        /// M�todo que retorna lista com os nomes das formas de pagamento do local. Usada para exibir essa informacao na tela de vendas.
        /// </summary>
        ...
        private DataSet estruturaReservasInternet(int seedReserva)
        {
            ...
            if (seedReserva <= 0)
                throw new BilheteriaException("Seed da reserva n�o pode ser zero ou menor que zero.");
            ...
            colGridReservaID.AutoIncrementSeed = seedReserva; //come�a pelo parametro
        ...
        public static DataSet EstruturaReservas()
        {
            ...
            try
            {
                ...
                colGridReservaID.AutoIncrement = true;
                colGridReservaID.AutoIncrementStep = 1;
                colGridReservaID.AutoIncrementSeed = 1; //come�a pelo parametro
        ...
        public DataTable FormasPagamento(string senha, ref int vendaBilheteriaID)
        {
            ...
            try
            {
                ...
                string sql = @"SELECT tVendaBilheteria.ID as VendaBilheteriaID, 
                                IsNull(tVendaBilheteria.PagamentoProcessado, 'T') AS PagamentoProcessado, 
                                tFormaPagamento.ID as FormaPagamentoID, tFormaPagamento.Nome, 
                ...
                if (bd.Consulta().Read())
                {
                    ...
                    if (vendaBilheteriaID == 0)
                        throw new BilheteriaException("Senha n�o existe.");
        ...
        /// <summary>
        /// Reserva ingresso, pacote e produto. Retorna um dataSet com as informa��es dos ingressos reservados.
        /// </summary>
        /// <returns></returns>
        public DataSet ReservarPista(int lojaID, int usuarioID, int canalID, int pacoteID, int qtdePacote, int eventoID, int apresentacaoID, int apresentacaoSetorID, int precoID, string precoNome, int cortesiaID, int bloqueioID, int qtdeIngresso, int seedReserva, bool buscaTaxasEntrega, bool buscaFormaPagamento, bool preReserva, int qtdeJaReservadaCota, EstruturaCotasInfo cotasInfo, int qtdeJaReservadaCotaItem, string eventoEspecial, int serieID, bool itemPromocional, int quantidadePorPromocional, int gerenciamentoIngressosID)
        {
            BD bd = new BD();
            ...
            //e uma tabela de reservas (informa�oes para a Grid)
            ...
            if (reservarIngresso)
            {
                ...
                try
                {
                    ...
                    int CotaItemID = oCotaItem.getCotaItemIDporPrecoID(precoID);
                    cotasInfo.CotaItemID = CotaItemID;

                    if (!apresentacao.Venda())
                    {
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Apresenta��o n�o est� mais a venda. Escolha outra apresenta��o.", CodMensagemReserva.ApresentacaoIndisponivel);
                    }



                    //qtdes de pre�o
                    int qtdePrecoDisponivel = 0; //qtde de pre�os disponiveis, q podem vender, independente do canal
                    int qtdePrecoPorCliente = 0; //qtde de pre�os q podem vender por cliente independente do canal
                    int qtdePrecoPorCanal = 0; //qtde de pre�os q podem vender por canal

                    int qtdePrecoVendido = 0; //qtde de ingressos vendidos com esse pre�o
                    int qtdeCanalVendido = 0; //qtde de ingressos vendidos com esse pre�o
                    int qtdePrecoVendidaPorCliente = 0; //qtde de ingressos vendidos com esse pre�o
                    ...
                    else
                    {
                        ...
                        {
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Pre�o n�o est� mais � venda nesse canal. Escolha outro pre�o.", CodMensagemReserva.PrecoIndisponivel);
                        }
                        else if (serieID > 0 && !infoPreco.DisponivelSerie)
                        {
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Pre�o n�o est� mais � venda nesse canal ou para a s�rie selecionada. Escolha outro pre�o.", CodMensagemReserva.PrecoIndisponivel);
                        }
                        ...
                        //2. verificar a qtde total de pre�o independente do canal
                        if (qtdePrecoDisponivel != 0)
                        {
                            //2.1 verificar qtde de ingressos vendidos a esse pre�o.
                            ...
                            if (qtdePrecoVendido >= qtdePrecoDisponivel)
                            {
                                ...
                                alertaMsg = AlertaMensagem.Exclamacao;
                                throw new BilheteriaException("Esse pre�o n�o pode mais ser vendido.\nJ� foi toda sua quota de " + qtdePrecoDisponivel + " ingressos!");
                            }
                        ...
                        //atribuir a qtde de pre�o ja vendida nao estourou qtde disponivel no canal
                        ...
                        if (qtdePrecoPorCanal != 0)
                        {
                            ...
                            if (qtdeCanalVendido >= qtdePrecoPorCanal)
                            {
                                ...
                                alertaMsg = AlertaMensagem.Exclamacao;
                                throw new BilheteriaException("Esse pre�o n�o est� mais dispon�vel no seu canal.\nJ� foi vendida toda sua quota de                                        " + qtdePrecoPorCanal + " ingressos!");
                            }
                    ...
                    if (cotasInfo.TemCotaItem && !cotasInfo.EncontrouCotaItem)
                    {
                        ...
                        {
                            if (precoNome.ToLower().StartsWith(dtt.Rows[i]["PrecoIniciaCom"].ToString().ToLower()))
                            {
                        ...
                        && (cotasInfo.ValidaBin != cotasInfo.ValidaBinAPS || cotasInfo.ParceiroID != cotasInfo.ParceiroIDAPS))
                    {
                        ...
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Este pre�o possui mais de uma promo��o cadastrada. Por favor, entre em contato com a equipe de atendimento.");
                    }

                    Enumerators.TipoCodigoBarra tipoCodigoBarra = new Evento().BuscarTipoCodigoBarra(eventoID);

                    ids.Clear(); // Limpa, para n�o ter acumulo e quebra da coluna UNIQUE.
                    oIngresso.Processar += new IRLib.Ingresso.ProcessHandler(oIngresso_ReservaPista);
                    ...
                    // Se n�o for uma pr�-reserva, alimentar as taxas e comiss�es
                    if (!preReserva)
                    {
                    ...
                    foreach (DataRow linha in reservados.Rows)
                    {
                        ...
                        //int CotaItemID = oCotaItem.getCotaItemIDporPrecoID(precoID, cotasInfo.CotaID_Apresentacao);
                    ...
                    int CotaItemIDs = oCotaItem.getCotaItemIDporPrecoID(precoID);
                    cotasInfo.CotaItemID = CotaItemIDs;
                    ...
                    if (qtdeIngresso != qtdeIngressosReservada)
                    {
                        ...
                        else if (qtdeIngressosReservada == qtdeMaxIngressosQPodeReservarPorPreco)
                        {
                            ...
                            msg = (qtdeIngressosReservada == 1) ?
                                "Somente um ingresso foi reservado!\nA quota de ingressos nesse pre�o j� estourou." :
                        ...
                        else if (qtdeIngressosReservada == qtdeMaxIngressosQPodeReservarPorCanal)
                        {
                            codMsg = CodMensagem.PorCanal;
                            msg = (qtdeIngressosReservada == 1) ?
                                "Somente um ingresso foi reservado!\nA quota de ingressos no seu canal j� estourou." :
                                "Somente " + qtdeIngressosReservada + " ingressos foram reservados!\nA quota de ingressos no seu canal j� estourou.";
                        }
                        else if (qtdeIngressosReservada == qtdeMaxIngressosQPodeReservarPorCliente)
                        {
                            ...
                            msg = (qtdeIngressosReservada == 1) ?
                                "Somente um ingresso foi reservado!\nA quota de ingressos por cliente j� estourou." :
                                "Somente " + qtdeIngressosReservada + " ingressos foram reservados!\nA quota de ingressos por cliente j� estourou.";
                        }
                    ...
                    else
                    {
                        ...
                        {
                            codMsg = CodMensagem.PorPreco;
                            //msg = "A quota de ingressos nesse pre�o j� atingiu sua quantidade m�xima.";
                        }
                        else if (qtdeIngressosReservada == qtdeMaxIngressosQPodeReservarPorCanal)
                        {
                            codMsg = CodMensagem.PorCanal;
                            //msg = "A quota de ingressos no seu canal j� atingiu sua quantidade m�xima.";
                        }
                        else if (qtdeIngressosReservada == qtdeMaxIngressosQPodeReservarPorCliente)
                        {
                            codMsg = CodMensagem.PorCliente;
                            //msg = "A quota de ingressos por cliente j� atingiu sua quantidade m�xima.";
                        }
                        else if (cotaExcedida)
                        {
                            codMsg = CodMensagem.PorCota;
            ...
            if (reservarPacote)
            {
                ...
                try
                {
                    ...
                    if (!PodeComprar)
                    {
                        codMsg = CodMensagem.PorPreco;
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Esse pre�o n�o pode ser vendido.\nVerifique a cota de pre�o do ingressos!");
                    }
                    ...
                    {
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Pacote n�o existe.");
                    }
                    ...
                    {
                        //2.1 verificar qtde de pacotes associados a esse pre�o.
                        //se qtde vendido for maior ou igual a qtde disponivel, nao pode vender
                        if (qtdePacoteVendido >= qtdePacoteDisponivel)
                        {
                            codMsg = CodMensagem.Disponivel;
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Esse pacote n�o pode mais ser vendido.\nSua quota de " + qtdePacoteDisponivel + " pacotes j� foi vendida!");
                        }
                    ...
                    if (qtdePacotePorCanal != 0)
                    {
                        ...
                        if (qtdePacoteVendido >= qtdePacotePorCanal)
                        {
                            codMsg = CodMensagem.PorCanal;
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Esse pacote n�o pode mais ser vendido no seu canal.\nSua quota de " + qtdePacotePorCanal + " pacotes pra esse canal j� foi vendida!");
                        }
                    ...
                    // Quando � pr�-reserva n�o deve considerar taxas e comiss�es
                    if (!preReserva)
                    {
                        //int taxaConvenienciaPacote = -1; //ver acima
                        CanalPacote canalPacote = new CanalPacote();

                        //busca as taxas de conveniencia e comiss�o e popula as variaveis
                    ...
                    while (qtdePacoteReservada < qtdePacoteQPodeReservar)
                    {
                        ...
                        //popula a comiss�o de acordo com a taxa minima e maxima
                    ...
                    if (qtdePacote != qtdePacoteReservada)
                    {
                        ...
                        else if (qtdePacoteReservada == qtdeMaxPacoteQPodeReservarPorCanal)
                        {
                            codMsg = CodMensagem.PorCanal;
                            msg = (qtdePacoteReservada == 1) ?
                                "Somente um pacote foi reservado!\nA quota de pacotes no seu canal j� estourou." :
                                "Somente " + qtdePacoteReservada + " pacotes foram reservados!\nA quota de pacotes nesse canal j� estourou.";
                        }
                        else
                        {
                            msg = (qtdePacoteReservada == 1) ?
                                "Somente um pacote foi reservado!\nN�o h� mais pacotes dispon�veis!" :
                                "Somente " + qtdePacoteReservada + " pacotes foram reservados!\nN�o h� mais pacotes dispon�veis!";
                        }
                    ...
                    { //emitir alerta caso...
                        ...
                        {
                            codMsg = CodMensagem.Disponivel;
                            //msg = "A quota de pacotes j� atingiu sua quantidade m�xima.";
                        }
                        else if (qtdePacoteReservada == qtdeMaxPacoteQPodeReservarPorCanal)
                        {
                            codMsg = CodMensagem.PorCanal;
                            //msg = "A quota de pacotes no seu canal j� atingiu sua quantidade m�xima.";
                        }
        ...
        public void ReservarInternetLugarMarcado(ref Ingresso ingresso, EstruturaPrecoReservaSite preco, int serieID, EstruturaReservaInternet estruturaReservaInternet)
        {
            try
            {
                ...
                BD bd = new BD();
                //Busca a ApresentacaoSetorID, j� aproveita e busca o tipo do setor, Cota ID da Apresentacao e da ApresentacaoSetor
                ...
                if (apresentacaoSetorID == 0)
                    throw new Exception("Falha ao buscar a apresenta��o");
                ...
                ingresso.TimeStampReserva.Valor = DateTime.Now.AddMinutes(new ConfigGerenciador().getValorTempoReserva());
        ...
        public void ReservarInternetMesa(ref List<Ingresso> ingressos, EstruturaPrecoReservaSite preco, EstruturaReservaInternet estruturaReserva)
        {
            ...
            try
            {
                ...
                bd.Consulta(sqlProc);
                if (!bd.Consulta().Read())
                    throw new Exception("N�o foi poss�vel encontrar as informa��es desta apresenta��o");
                ...
                foreach (Ingresso ingresso in ingressos)
                {
                    ...
                    if (!ingresso.Reservar(bd, false))
                        throw new Exception("N�o foi poss�vel reservar a mesa.");
                }
                int clienteID = ingressos[0].ClienteID.Valor;
                string sessionID = ingressos[0].SessionID.Valor;

                Ingresso.AumentarTempoReservasInternet(ingressos[0].ClienteID.Valor, ingressos[0].SessionID.Valor, bd);
        ...
        public List<Ingresso> ReservarInternet(string sessionID, int clienteID, int apresentacaoID, int setorID, List<EstruturaPrecoReservaSite> precos, int eventoID, int serieID, EstruturaReservaInternet estruturaReserva, bool isEntradaFranca)
        {
            var retorno = new List<Ingresso>();

            try
            {
                var oIngresso = new Ingresso();

                var apresentacaoSetorID = 0;
                var tipoSetor = new Setor.enumLugarMarcado();
                var cotaInfo = new EstruturaCotasInfo();
                var bd = new BD();

                //Busca a ApresentacaoSetorID, j� aproveita e busca o tipo do setor, Cota ID da Apresentacao e da ApresentacaoSetor
                var sqlProc = "EXEC sp_getApresentacaoInfo " + apresentacaoID + ", " + setorID;
                ...
                //Busca os melhores ingressos dispon�veis de: Pista, Mesa Aberta, Mesa Fechada e Cadeira.
                var melhoresIngressos = oIngresso.MelhoresIngressos(estruturaReserva.UsuarioID, estruturaReserva.LojaID, sessionID, clienteID,
                    eventoID, apresentacaoID, apresentacaoSetorID, setorID, precos, tipoSetor, cotaInfo, serieID, estruturaReserva, isEntradaFranca);

                //Tenta reservar os ingressos contanto que nao seja CORTESIA_SEM_CONVENIENCIA
                foreach (var ingresso in melhoresIngressos)
                {
                    if (ingresso.ReservarInternet(bd, false))
                        retorno.Add(ingresso);
                    else
                        throw new Exception("N�o foi poss�vel reservar o ingresso, por favor tente novamente");
                }
            }
            catch (Exception ex)
            {
        ...
        public Ingresso[] ReservarPacoteInternet(int pacoteID, int quantidade, string sessionID, int clienteID, EstruturaReservaInternet estruturaReservaInternet)
        {
            ...
            try
            {
                var oPacote = new Pacote(estruturaReservaInternet.UsuarioID) { Control = { ID = pacoteID } };

                var podeComprar = oPacote.VetificaQuantidadePreco(pacoteID, estruturaReservaInternet.CanalID, quantidade,
                    bd);

                if (!podeComprar)
                    throw new BilheteriaException("N�o ser� poss�vel efetuar a reserva do pre�o selecionado. O limite de vendas foi excedido.");

                var bilheteria = new Bilheteria();
                var pctTaxa = bilheteria.TaxaConvenienciaPacote(pacoteID, estruturaReservaInternet.CanalID);

                var itensPacote = oPacote.ItensParaReserva(bd);

                return oPacote.ReservarInternet(bd, itensPacote, estruturaReservaInternet.UsuarioID, sessionID,
                    clienteID, estruturaReservaInternet.LojaID, quantidade, pctTaxa);

            }
            catch (Exception ex)
            {
                LogUtil.Error(string.Format("##Post.ReservarPacoteInternet.EXCEPTION## SESSION {0}, QTD {1}, PACOTE {2}, MSG {3}", sessionID, quantidade, pacoteID, ex.Message));
                throw new Exception(ex.Message);
            }
            finally
            {
        ...
        public Ingresso[] MelhorCadeira(int usuarioID, int lojaID, string sessionID, int clienteID, int eventoID, int apresentacaoSetorID, int[,] precos, bool preReserva, int canalID)
        {
            ...
            {
                int qtd = 0;
                List<IRLib.ClientObjects.EstruturaPrecoIDValor> preco = new List<IRLib.ClientObjects.EstruturaPrecoIDValor>();
                ...
                for (int i = 0; i < (precos.Length / 2); i++)
                {
                    ...
                    // Se for pr�-reserva ignorar o pre�o
                    if (!preReserva)
                        if (precos[i, 1] != GetIngressosQPodeReservar(clienteID, precos[i, 0], precos[i, 1], ref valorPreco, canalID))
                            throw new BilheteriaException("Pre�o Indispon�vel", CodMensagemReserva.PrecoIndisponivel);

                    qtd += precos[i, 1];
                    IRLib.ClientObjects.EstruturaPrecoIDValor precoAux;
                    for (int x = 0; x < precos[i, 1]; x++)
                    {
                        precoAux = new IRLib.ClientObjects.EstruturaPrecoIDValor();
                ...
                */
                ...
                for (int i = 0; i < grupos.Count && ingressos.Count < qtd; i++)
                {
                    ...
                    while (bd.Consulta().Read() && ingressos.Count < qtd)
                    {
                        ...
                        /// Verifica se a classifica��o do item anterior + 1 � diferente da atual.
                        /// _____
                        /// 1
                        /// 2
                        /// 3
                        ...
                        ///		Classifica��o Atual = 4 = OK
                        ///		Classifica��o Atual = 5 = ERRADO.
                        ///
                    ...
                    if (ingressos.Count != qtd) // N�o encontrou a qtd suficiente?!
                        ingressos.Clear(); // Limpa os ingressos e passa para a pr�xima.
                }
                bd.FecharConsulta();
                ...
                try
                {
                    ...
                    foreach (Ingresso ing in ingressos)
                    {
                        ...
                        //se n�o tiver valor e n�o tiver conveniencia n�o deve reservar
                        if (valor == 0 && oIngresso.TxConv == 0)
                        {
                            // Atribui a Cortesia Padr�o do Evento/Local - INICIO
                            Cortesia oCortesia = new Cortesia();
                            int CortesiaPadraoID = oCortesia.CortesiaPadraoEvento(eventoID);
                            if (CortesiaPadraoID == 0)
                                throw new Exception("N�o foi poss�vel reservar o ingresso. Por favor, tente novamente mais tarde.");
                            oCortesia = null;
                            oIngresso.CortesiaID.Valor = CortesiaPadraoID;
                            // Atribui a Cortesia Padr�o do Evento/Local - FIM
                            ...
                            //como base de info para deletar o pre�o inv�lido do banco de dados do site.
                        }
                        else
                        {
                    ...
                    // se algum ingresso foi reservado aumenta o TimeStampReserva de todos ingressos da sess�o do cliente
        ...
        public decimal TaxaConvenienciaPacote(int pacoteId, int canalId)
        {
            ...
            try
            {
                var taxaConvenienciaPacote = 0;
                decimal taxaMin = 0;
                decimal taxaMax = 0;
                decimal valor = 0;

                var sql =
@"SELECT cp.TaxaConveniencia,
                                              cp.TaxaMinima,
                                              cp.TaxaMaxima,
                                              SUM(p.Valor * i.Quantidade) AS Valor
FROM tCanalPacote AS cp(NOLOCK)
     INNER JOIN tPacote AS pa(NOLOCK) ON pa.ID = cp.PacoteID
     INNER JOIN tPacoteItem AS i(NOLOCK) ON i.PacoteID = pa.ID
     INNER JOIN tPreco AS p(NOLOCK) ON p.ID = i.PrecoID
WHERE pa.ID = {0}
      AND CanalID = {1}
GROUP BY cp.TaxaConveniencia,
         cp.TaxaMinima,
         cp.TaxaMaxima;";

                bd.Consulta(string.Format(sql, pacoteId, canalId));
                ...
                var dAux = (taxaConvenienciaPacote / 100m) * valor;
        ...
        /// <summary>
        /// Traz do banco a quantidade permitida para o  pre�o verificando a 
        ...
        /// <summary>
        /// M�todo faz todas as verifica��es de quantidade e disponibilidade do pre�o.
        /// Caso tenha algum problema joga exception.
        /// Retorna a quantidade de ingressos que pode reservar.
        /// M�todo exclusivo da internet.
        /// </summary>
        /// <param name="clienteID"></param>
        ...
        public int GetIngressosQPodeReservar(int clienteID, int precoID, int qtde, ref decimal valorPreco, int canalID)
        {
            try
            {
                ...
                if (!precoOK)
                    throw new BilheteriaException("Esse pre�o n�o pode mais ser vendido. Sua cota de reservas para esse pre�o estourou!");
        ...
        #region M�todos antigos do WSIntegracao
        ...
        {
            CupomDesconto oCupom = new CupomDesconto();
        ...
        public DataSet ReservarLugarMarcadoAssinatura(int pacoteID, DataTable lugares, int canalID, int lojaID, int usuarioID, int seedReserva, bool buscaTaxasEntrega, bool buscaFormaPagamento, bool preReserva)
        {
            ...
            int lugarID, ingressoID; // vari�veis utilizadas dentro do for
            ...
            try
            {
                ...
                // Busca os itens do pacote e a taxa de conveni�ncia
                ...
                foreach (DataRow lugar in lugares.Rows) // lugares
                {
                    ...
                    foreach (DataRow itemPacote in itensPacote.Rows)
                    {
                        ...
                        //Verifica se � possivel reservar o ingresso apartir das cotas geradas p/ a apresentacao/setor
                            ...
                            for (int i = 0; i < listaCotaItem.Count; i++)
                            {
                                if (ePreco.PrecoNome.ToLower().StartsWith(listaCotaItem[i].PrecoIniciaCom.ToLower()))
                                {
                                    ...
                                    if ((ePreco.Quantidade + quantidadeDifDisponivel[0] > quantidadeMaxima ||
                                        ...
                                        throw new BilheteriaException("O Pre�o: " + ePreco.PrecoNome + " deste pacote atingiu o Limite de Venda/Reserva");
                        ...
                        // Zera a vari�vel de IngressoID Auxiliar
                        auxID = null;
                        ...
                        if (auxID == null)
                            throw new BilheteriaException("Um ou mais itens do pacote n�o possuem ingressos dispon�veis.");
                        else
        ...
        /// <summary>
        /// Reserva o pacote. M�todo utilizado somente para a importa�ao de assinaturas da OSESP.
        /// Esse M�todo est� preparado para reservar apenas pacotes de pista.
        /// </summary>
        public DataSet ReservarPacoteImportacaoAssinatura(BD bd, IRLib.ClientObjects.EstruturaAssinaturaInfo assinaturaInfo, List<int> ingressosID, int usuarioID, int lojaID, int canalID, int seedReserva)
        {
            try
            {
                ...
                // Toda a reserva do pacote deve estar em transa��o. Tudo ou nada.
                bd.IniciarTransacao();
                ...
                {
                    //Busca as informa�oes do ingresso.
                    ...
                    while (bd.Consulta().Read())
                    {
                        //popula o objeto ingresso com as informa��es nescessarias para a reserva
                        ...
                        novoItemGrid[VALOR] = valorPacote;
                        novoItemGrid[TIPO] = TIPO_PACOTE;
                        novoItemGrid[TIPO_LUGAR] = "P"; //esse m�todo s� � utilizado para a venda de pacote pista na importacao de assinatura
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet ReservarLugarMarcado(DataTable lugares, int lojaID, int usuarioID, int canalID, int eventoID, int apresentacaoID, int apresentacaoSetorID, int precoID, string precoNome, int cortesiaID, int seedReserva, bool venderBloqueios, bool buscaTaxasEntrega, bool buscaFormaPagamento, bool preReserva, int qtdeJaReservadaCota, EstruturaCotasInfo cotasInfo, int qtdeJaReservadaCotaItem, int serieID, bool itemPromocional, int quantidadePorPromocional)
        {
            BD bd = new BD();
            Ingresso oIngresso = new Ingresso();
            Cota oCota = new Cota();
            ...
            // Desconsiderar pre�o quando � reserva de pr�-reserva
            if (preReserva)
            {
                precoID = 0;
                cortesiaID = 0;
            ...
            try
            {
                ...
                int qtdeMaxIngressosQPodeReservarPorCanal = 0; //qtde maxima q pode reservar por canal
                int qtdeMaxIngressosQPodeReservarPorCliente = 0; //qtde maxima q pode reservar por cliente
                int qtdeCanalVendido = 0; //Quantidade j� vendida do canal
                int qtdeIngressosReservada = 0; //qtde de ingressos realmente reservada
                ...
                if (!preReserva)
                {
                    ...
                    {
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Pre�o n�o est� mais � venda nesse canal ou foi removido da s�rie selecionada. Escolha outro pre�o.", CodMensagemReserva.PrecoIndisponivel);
                    }
                    ...
                    {
                        alertaMsg = AlertaMensagem.Exclamacao;
                        throw new BilheteriaException("Limite de pre�os esgotado.");
                ...
                if (tipoLugar == Setor.Cadeira)
                    lugarEsgotado = "Cadeira n�o est� mais dispon�vel.";
                else
                    lugarEsgotado = "Mesa n�o est� mais dispon�vel.";
                ...
                foreach (DataRow lugar in lugares.Rows)
                {
                    ...
                    // Em caso de pr�-reserva n�o deve ser consideradas as taxas e comiss�es
                    if (!preReserva)
                    {
                        CanalEvento canalEvento = new CanalEvento();
                        DataTable taxas = canalEvento.BuscaTaxasConvenienciaComissao(canalID, eventoID);
                    ...
                    if (tipoLugar != Setor.MesaFechada)
                    {
                        ...
                        {
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Lugar n�o est� mais dispon�vel!");
                        }
                        ...
                        if (ids.Count > 0)
                        {
                            ...
                            foreach (DataRow linha in reservados.Rows)
                            {
                                CotaItem cotaite = new CotaItem();
                                cotasInfo.CotaItemID = cotaite.getCotaItemIDporPrecoID(precoID);
                                DataRow novoItemGrid = retorno.Tables[TABELA_GRID].NewRow();
                                novoItemGrid[PRECOID] = precoID;
                                novoItemGrid[APRESENTACAOSETORID] = apresentacaoSetorID;
                                novoItemGrid[LOCALID] = linha["LocalID"];
                    ...
                    else
                    {
                        ...
                        {
                            alertaMsg = AlertaMensagem.Exclamacao;
                            throw new BilheteriaException("Mesa n�o est� mais dispon�vel!");
                        }
                        ...
                        if (ok)
                        {
                            ...
                            if (reservados.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow linha in reservados.Rows)
                                {
                                    ...
                                    //popula o valor da comiss�o de acordo com a taxa minima e maxima
                                    if (taxaComissao > 0 || ComissaoMin > 0)
                                    {
                                        decimal dAux = (taxaComissao / 100m) * (decimal)linha["Valor"];
                                ...
                                //Na mesa fechada a taxa m�nima e m�xima � calculada em cima de cada lugar.
                                ...
                                foreach (DataRow linha in reservados.Rows)
                                {
                                    ...
                                    novoItemReserva[VALIDA_BIN_APS] = cotasInfo.ValidaBinAPS;
                ...
                if (qtdePedida != qtdeReservada)
                {
                    ...
                    {
                        msg = (qtdeReservada == 1) ?
                            "Somente um lugar foi reservado!\nOs demais lugares n�o est�o mais dispon�veis." :
                            "Somente " + qtdeReservada + " lugares foram reservados!\nOs demais lugares n�o est�o mais dispon�veis.";
                    }
                }
                else
        ...
        /// Utiliza��o: Transfere todos os ingressos reservados via Internet
        /// </summary>
        /// <param name="clienteID"></param>
        /// <param name="sessionID"></param>
        /// <returns></returns>
            ...
            try
            {
                ...
                if (clienteID > 0)
                {
                    ...
                    if (ok) // Conseguiu Transferir todos os ingressos
                        return 1;
                    else // Cancela opera��o de transferencia do carrinho
                        return 0;
                }
                else
                    return 0;
        ...
        public void CancelarImpressao(List<IRLib.ClientObjects.IngressoImpressaoCancelar> ingressoImpressaoCancelar, int usuarioID, int empresaID, string motivo, int SupervisorID)
        {
            BD bd = new BD();
                ...
                if (ingressoImpressaoCancelar.Count > 0 && ingressoImpressaoCancelar != null)
                {
                    ...
                    for (int i = 0; i < ingressoImpressaoCancelar.Count; i++)
                    {
                        ...
                        if (ok)
                        {
                            ...
                            bool okV = (x == 1);
                            if (!okV)
                                throw new BilheteriaException("Log de venda do ingresso n�o foi inserido.");
                            ...
                            if (ingressoImpressaoCancelar[i].TipoCodigoBarra != Enumerators.TipoCodigoBarra.ListaBranca)
                            {
                                ...
                                bool okV2 = (x == 1);
                                if (!okV2)
                                    throw new Exception("Status do ingresso n�o pode ser cancelado.");
                            }
                            regAtualizados += 1;
                        }
                        else
                    ...
                    if (regAtualizados == 0)
                        throw new BilheteriaException("Status do ingresso n�o pode ser atualizado.");
        ...
        public void CancelarImpressao(BD bd, List<IRLib.ClientObjects.IngressoImpressaoCancelar> ingressoImpressaoCancelar, int usuarioID, int empresaID, string motivo, int SupervisorID)
        {
            if (ingressoImpressaoCancelar.Count > 0 && ingressoImpressaoCancelar != null)
            {
        ...
        public string CancelarValeIngresso(EstruturaRetornoVendaValeIngresso vendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int entregaControleID, decimal valorEntrega, decimal valorTotal, int usuarioID, string notaFiscalCliente, string notaFiscalEstabelecimento, bool cancelaTaxaEntrega, string motivo, int motivoId, int SupervisorID, DateTime data, int vendaBilheteriaID, bool cancelaAdyen, bool cancelaPayPal, string TransactionID)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public string CancelarValeIngresso(EstruturaRetornoVendaValeIngresso vendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int entregaControleID, decimal valorEntrega, decimal valorTotal, int usuarioID, string notaFiscalCliente, string notaFiscalEstabelecimento, bool cancelaTaxaEntrega, string motivo, int motivoId, int SupervisorID, int EntregaAgendaID, int vendaBilheteriaID, bool cancelaAdyen, bool cancelaPayPal, string TransactionID)
        {
            if (caixaID == 0)
                throw new BilheteriaException("Caixa n�o pode ser nulo.");
            ...
            if (cancelaAdyen && !ConfiguracaoAdyen.Instancia.Chaves.Ativo.Valor)
                throw new Exception("N�o ser� poss�vel efetuar o cancelamento e estorno desta venda atrav�s da Adyen, ela est� desativada!");
            ...
            try
            {
                ...
                if (vendaBilheteria.Control.ID == 0)
                    throw new BilheteriaException("Venda n�o foi gerada.");
                ...
                foreach (EstruturaImpressaoVir valeIngresso in vendidos.EstruturaImpressaoVir)
                {
                    ...
                    if (vendaBilheteriaItem.Control.ID == 0)
                        throw new BilheteriaException("Item de venda n�o foi gerada.");
                    ...
                    if (valeIngresso.ValidadeEmDiasImpressao > 0)
                        ...
                        sqlValidade = string.Empty;//� atualiza a validade fixa
                ...
                for (int i = 0; i < pagamentos.Rows.Count; i++)
                {
                    ...
                    bool ok = (x >= 1);
                    if (!ok)
                        throw new BilheteriaException("Forma de pagamento n�o foi cadastrada.");
                ...
                if (cancelaAdyen)
                {
                    ...
                    new Adyen() { CodigoReferencia = codigoReferencia }.CancelarPagamento();
                }
        ...
        /// <summary>
        /// Sobrecarga do m�todo de cancelar, este m�todo � especifico para a VendaBilheteria (Tela de Vendas)
        /// Como n�o existe EntregaAgendaID, deve buscar no banco e passar para o m�todo sobrecarregado.
        /// </summary>
        ...
        /// <returns></returns>
        public string Cancelar(DataTable itensVendidos, DataTable ingressosVendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int entregaControleID, decimal valorEntrega, decimal valorTotal, int usuarioID, string NotaFiscalCliente, string NotaFiscalEstabelecimento, bool cancelaTaxaConv, bool cancelaTaxaEntrega, string motivo, bool cancelamentoFraude, int vendaBilheteriaID, int motivoId, int SupervisorID, DateTime data, bool cancelaTaxaProcessamento, decimal valorTaxaProcessamento, bool cancelarAdyen, string ApoliceMondial, bool cancelaPayPal, bool cancelasmiles, string TransactionID, bool cancelaIngresso)
        {
            int entregaAgendaID = 0;
            if (data != DateTime.MinValue)
                entregaAgendaID = new EntregaAgenda().BuscarID(entregaControleID, data);
            ...
            return this.Cancelar(itensVendidos, ingressosVendidos, pagamentos, caixaID, lojaID, canalID, empresaID, clienteID, entregaControleID, valorEntrega, valorTotal,
                 ...
                 cancelaTaxaEntrega, motivo, cancelamentoFraude, vendaBilheteriaID, motivoId, SupervisorID, entregaAgendaID, cancelaTaxaProcessamento, valorTaxaProcessamento,
                 cancelarAdyen, ApoliceMondial, cancelaPayPal, cancelasmiles, TransactionID, cancelaIngresso, false);
        }

        private void CancelamentoItensNormais(DataRow[] itensNormais, DataTable ingressosVendidos, VendaBilheteria vendaBilheteria, string codigoBarraAntigo, bool cancelaTaxaConv, Enumerators.TipoCodigoBarra tipoCodigoBarra, string codigoBarra, BD bd, BD bdConsulta, int usuarioID, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int motivoId, int SupervisorID, string motivo, Assinatura infoAssinatura, int assinaturaClienteID)
        {
            ...
            foreach (DataRow item in itensNormais)
            {
                ...
                if (tipoCodigoBarra == Enumerators.TipoCodigoBarra.ListaBranca)
                {
                    ...
                    //Se gerou o c�digo de barras deve inserir na tIngressoCodigoBarra
                    IngressoCodigoBarra ingressoCodigoBarra = new IngressoCodigoBarra();
                ...
                if (ok)
                {
                    ...
                    if (bdConsulta.Consulta().Read())
                    {
                        ...
                        string strSql = "update tIngresso SET CotaItemID = NULL WHERE ID = " + ingressos[0][INGRESSOID].ToString();

                        bd.Executar(strSql);

                        sql = "UPDATE tCotaItemControleInMemory SET tCotaItemControleInMemory.Quantidade = tCotaItemControleInMemory.Quantidade - 1 WHERE tCotaItemControleInMemory.CotaItemID = " + cotaItemID + " AND tCotaItemControleInMemory.ApresentacaoID = " + apresentacaoID + " AND tCotaItemControleInMemory.ApresentacaoSetorID = " + apresentacaoSetorID;
                        ...
                        //if (x < 1)
                        //{
                        //    sql = "UPDATE tCotaItemControleInMemory SET tCotaItemControleInMemory.Quantidade = tCotaItemControleInMemory.Quantidade - 1 WHERE tCotaItemControleInMemory.CotaItemID = " + cotaItemID + " AND tCotaItemControleInMemory.ApresentacaoID = " + apresentacaoID + " AND tCotaItemControleInMemory.ApresentacaoSetorID = 0";
                        //    bd2.Executar(sql);
                        //}
                    ...
                    //Ingresso oIngresso = new Ingresso();
                    //oIngresso.RetirarCotaItemIngresso((int)ingressos[0][INGRESSOID]);
                }
                else
                    throw new BilheteriaException("Status do ingresso n�o pode ser atualizado.");
            }
        ...
        /// </summary>
        /// <returns></returns>
        public string Cancelar(DataTable itensVendidos, DataTable ingressosVendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int entregaControleID, decimal valorEntrega, decimal valorTotal, int usuarioID, string NotaFiscalCliente, string NotaFiscalEstabelecimento, bool cancelaTaxaConv, bool cancelaTaxaEntrega, string motivo, bool cancelamentoFraude, int vendaBilheteriaID, int motivoId, int SupervisorID, int EntregaAgendaID, bool cancelaTaxaProcessamento, decimal valorTaxaProcessamento, bool cancelarAdyen, string ApoliceMondial, bool cancelaPayPal, bool cancelasmiles, string TransactionID, bool cancelaIngressos, bool PayPalParcial = false)
        {

            LogUtil.Info(string.Format("##Bilheteria.Cancelando## VENDA: {0}", vendaBilheteriaID));
            ...
            if (valorTotal > 0 && pagamentos.Rows.Count == 0)
                throw new BilheteriaException("Forma de pagamento n�o foi adicionada.");

            if (cancelarAdyen && !ConfiguracaoAdyen.Instancia.Chaves.Ativo.Valor)
                throw new Exception("N�o ser� poss�vel efetuar o cancelamento da Adyen, ela est� inativa.");

            var bd = new BD();
            var bdConsulta = new BD();

            var vendabilheteriacancelamento = 0;

            try
            {
                if (cancelaIngressos || cancelaTaxaConv || cancelaTaxaEntrega || cancelaTaxaProcessamento)
                {
                    //VendaBilheteria
                    bd.IniciarTransacao();

                    var vendaBilheteria = new VendaBilheteria
                    {
                        ClienteID = { Valor = clienteID },
                        CaixaID = { Valor = caixaID },
                        Status = { Valor = VendaBilheteria.CANCELADO },
                        DataVenda = { Valor = System.DateTime.Now },
                        NotaFiscalCliente = { Valor = NotaFiscalCliente },
                        NotaFiscalEstabelecimento = { Valor = NotaFiscalEstabelecimento }
                    };
                    ...
                    if (cancelaTaxaConv)
                    {
                        ...
                        {
                            var comisaoAux = item[COMISSAO_VALOR] != DBNull.Value ? (decimal)item[COMISSAO_VALOR] : 0;
                    ...
                    vendaBilheteria.VendaBilheteriaIDOrigem.Valor = vendaBilheteriaID;

                    var sqlVendaBilheteria = vendaBilheteria.StringInserir();
                    var vendaID = bd.ConsultaValor(sqlVendaBilheteria);
                    vendaBilheteria.Control.ID = (vendaID != null) ? Convert.ToInt32(vendaID) : 0;

                    vendabilheteriacancelamento = vendaBilheteria.Control.ID;

                    var lstBoletos = new List<int>();

                    if (vendaBilheteria.Control.ID == 0)
                        throw new BilheteriaException("Venda n�o foi gerada.");

                    var vendacancelada = cancelaIngressos ? 'T' : 'F';
                    var entregacancelada = cancelaTaxaEntrega ? 'T' : 'F';
                    var convenienciacancelada = cancelaTaxaConv ? 'T' : 'F';
                    var processamentocancelada = cancelaTaxaProcessamento ? 'T' : 'F';
                    var segurocancelada = !string.IsNullOrEmpty(ApoliceMondial) ? 'T' : 'F';

                    if (vendaBilheteriaID > 0 && !cancelamentoFraude)
                        bd.Executar(string.Format(@"UPDATE tVendaBilheteria SET PagamentoProcessado = 'T', VendaCancelada = '{0}', TaxaProcessamentoCancelada = '{1}', EntregaCancelada = '{2}', ConvenienciaCancelada = '{3}', SeguroCancelado = '{4}' WHERE ID = {5}", vendacancelada, processamentocancelada, entregacancelada, convenienciacancelada, segurocancelada, vendaBilheteriaID));
                    else if (vendaBilheteriaID > 0)
                        bd.Executar(string.Format(@"UPDATE tVendaBilheteria SET PagamentoProcessado = 'T', VendaCancelada = '{0}', TaxaProcessamentoCancelada = '{1}', EntregaCancelada = '{2}', ConvenienciaCancelada = '{3}', SeguroCancelado = '{4}', NivelRisco = 3 WHERE ID = {5}", vendacancelada, processamentocancelada, entregacancelada, convenienciacancelada, segurocancelada, vendaBilheteriaID));
                    ...
                    if (cancelaIngressos)
                    {
                        ...
                        var itensNormais = itensVendidos.Select(TIPO + "='" + TIPO_INGRESSO + "' AND " + TIPO_LUGAR + "<>'" + Setor.MesaFechada + "'");
                        var itensMesaFechada = itensVendidos.Select(TIPO + "='" + TIPO_INGRESSO + "' AND " + TIPO_LUGAR + "='" + Setor.MesaFechada + "'");
                        var itensPacote = itensVendidos.Select(TIPO + "='" + TIPO_PACOTE + "'");
                        var codigoBarra = string.Empty;
                        var tipoCodigoBarra = Enumerators.TipoCodigoBarra.Estruturado;
                        var codigoBarraAntigo = string.Empty;

                        var assinaturas = TabelaMemoria.DistinctComFiltro(itensVendidos, ASSINATURA_CLIENTE_ID, ASSINATURA_CLIENTE_ID + " > 0");
                        var oAssinatura = new Assinatura(usuarioID);
                        var oAssinaturaCliente = new AssinaturaCliente(usuarioID);
                        ...
                        int cotaItemID;
                        int apresentacaoID;
                        int apresentacaoSetorID;
                        foreach (var item in itensMesaFechada)
                        {
                            var ingressos = ingressosVendidos.Select(RESERVAID + "='" + (int)item[RESERVAID] + "'");

                            var vendaBilheteriaItem = new VendaBilheteriaItem
                            {
                                VendaBilheteriaID = { Valor = vendaBilheteria.Control.ID },
                                PacoteID = { Valor = 0 },
                                Acao = { Valor = VendaBilheteriaItem.CANCELAMENTO },
                                TaxaComissao =
                                {
                                    Valor = item[TAXA_COMISSAO] != DBNull.Value ? (int)item[TAXA_COMISSAO] : 0
                                },
                                ComissaoValor =
                                {
                                    Valor = item[COMISSAO_VALOR] != DBNull.Value ? (decimal)item[COMISSAO_VALOR] : 0
                                }
                            };
                            ...
                            var sqlVendaBilheteriaItem = vendaBilheteriaItem.StringInserir();
                            var id = bd.ConsultaValor(sqlVendaBilheteriaItem);
                            vendaBilheteriaItem.Control.ID = (id != null) ? Convert.ToInt32(id) : 0;
                            ...
                            foreach (var i in ingressos)
                            {
                                ...
                                var eventoID = Convert.ToInt32(i[EVENTOID]);
                                ...
                                if (tipoCodigoBarra == Enumerators.TipoCodigoBarra.ListaBranca)
                                {
                                    ...
                                    //Se gerou o c�digo de barras deve inserir na tIngressoCodigoBarra
                                    var ingressoCodigoBarra = new IngressoCodigoBarra
                                    {
                                        EventoID = { Valor = eventoID },
                                        CodigoBarra = { Valor = codigoBarra }
                                    };
                                ...
                                if (ok)
                                {
                                    ...
                                    var ingressoLog = new IngressoLog
                                    {
                                        TimeStamp = { Valor = DateTime.Now },
                                        IngressoID = { Valor = (int)i[INGRESSOID] },
                                        UsuarioID = { Valor = usuarioID },
                                        BloqueioID = { Valor = (int)i[BLOQUEIOID] },
                                        CortesiaID = { Valor = (int)i[CORTESIAID] },
                                        PrecoID = { Valor = (int)i[PRECOID] },
                                        VendaBilheteriaItemID = { Valor = vendaBilheteriaItem.Control.ID },
                                        VendaBilheteriaID = { Valor = vendaBilheteria.Control.ID },
                                        CaixaID = { Valor = caixaID },
                                        LojaID = { Valor = lojaID },
                                        CanalID = { Valor = canalID },
                                        EmpresaID = { Valor = empresaID },
                                        ClienteID = { Valor = clienteID },
                                        Acao = { Valor = IngressoLog.CANCELAR },
                                        Obs = { Valor = motivo },
                                        MotivoId = { Valor = motivoId },
                                        SupervisorID = { Valor = SupervisorID },
                                        CodigoBarra = { Valor = codigoBarraAntigo }
                                    };
                        ...
                        {
                            var ingressosPacote = ingressosVendidos.Select(RESERVAID + "='" + (int)item[RESERVAID] + "'");
                    ...
                    //Cancelamento Adyen
                    LogUtil.Debug(string.Format("##Bilheteria.Cancelando.CancelarAdyen## VENDA: {0}, CANCEL_ADYEN: {1}", vendaBilheteriaID, cancelarAdyen));
                    if (cancelarAdyen)
                    {
                        string codigoReferencia = bd.ConsultaValor(@"
                                SELECT
                        ...
                        if (!new Adyen { CodigoReferencia = codigoReferencia }.CancelarPagamento())
                        {
                            LogUtil.Debug(string.Format("##Bilheteria.Cancelando.CancelarAdyen.ERROR## VENDA: {0}, MSG: {1}", vendaBilheteriaID, "N�o foi poss�vel efetuar o cancelamento atrav�s da Adyen"));
                            throw new Exception("N�o foi poss�vel efetuar o cancelamento atrav�s da Adyen, por favor tente novamente!");
                        }
                    }
                    ...
                    if (cancelasmiles)
                    {
                        ...
                        if (string.IsNullOrEmpty(cancelamento))
                            throw new Exception("N�o foi poss�vel efetuar o estorno atrav�s da Smiles, por favor tente novamente!");
                    }

                    if (cancelaPayPal)
                    {
                        if (PayPalParcial)
                            this.RefundPartial(TransactionID, valorTotal);
                        else
                            this.Refund(TransactionID);
                    }
                    ...
                    if (clienteID <= 0)
                    {
                        vendaBilheteria.LerFormaPagamento(vendaBilheteriaID);
                        clienteID = vendaBilheteria.ClienteID.Valor;
                    }
                ...
                {
                    Mondial.TicketProtectorWebService objService = new Mondial.TicketProtectorWebService();
                ...
                var sqlSenha = "SELECT Senha FROM tVendaBilheteria (NOLOCK) WHERE ID=" + vendabilheteriacancelamento;
                var ret = bd.ConsultaValor(sqlSenha);
                var senha = (ret != null) ? Convert.ToString(ret) : null;

                LogUtil.Info(string.Format("##Bilheteria.Cancelando.SUCCESS## VENDA: {0}, SENHA: {1}", vendaBilheteriaID, senha));
                return senha;
            }
            catch (Exception ex)
            {
                LogUtil.Error(string.Format("##Bilheteria.Cancelando.EXCEPTION## VENDA: {0}, MSG: {1}", vendaBilheteriaID, ex.Message), ex);
        ...
        public EstruturaRetornoVendaValeIngresso VenderValeIngresso(BindingList<EstruturaRetornoReservaValeIngresso> reservados, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string NotaFiscalCliente, string NotaFiscalEstabelecimento, int IndiceInstituicaoTransacao, int IndiceTipoCartao, int NSUSitef, int NSUHost, int CodigoAutorizacaoCredito, int BIN, int ModalidadePagamentoCodigo, string ModalidadePagamentoTexto, int clienteEnderecoIDSelecionado, EstruturaEntregaAgenda entregaSelecionada, int pdvIDSelecionado, string NomeCartao, bool CartaoOutraPessoa)
        {
            //verificar se os dados est�o preenchidos
            if (NotaFiscalCliente.Trim() == string.Empty)
                throw new Exception("Falha no envio de dados da transa��o TEF");
        ...
        public EstruturaRetornoVendaValeIngresso VenderValeIngresso(BindingList<EstruturaRetornoReservaValeIngresso> reservados, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string cartao, int clienteEnderecoIDSelecionado, EstruturaEntregaAgenda entregaSelecionada, int pdvIDSelecionado, string NomeCartao, bool CartaoOutraPessoa)
        {
            int numCartao = 0;
        ...
        private EstruturaRetornoVendaValeIngresso venderValeIngresso(BindingList<EstruturaRetornoReservaValeIngresso> reservados, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string NotaFiscalCliente, string NotaFiscalEstabelecimento, int IndiceInstituicaoTransacao, int IndiceTipoCartao, int NSUSitef, int NSUHost, int CodigoAutorizacaoCredito, int BIN, int ModalidadePagamentoCodigo, string ModalidadePagamentoTexto, int clienteEnderecoIDSelecionado, EstruturaEntregaAgenda entregaSelecionada, int pdvIDSelecionado, string NomeCartao, bool CartaoOutraPessoa)
        {
            ...
            if (lojaID == 0)
                throw new BilheteriaException("Loja n�o pode ser nula.");

            if (canalID == 0)
                throw new BilheteriaException("Canal n�o pode ser nulo.");

            if (usuarioID == 0)
                throw new BilheteriaException("Usu�rio n�o pode ser nulo.");

            if (empresaID == 0)
                throw new BilheteriaException("Empresa n�o pode ser nula.");
            ...
            try
            {
                ...
                if (oEA.PodeSerAgendado(entregaSelecionada))
                {
                    ...
                    if (verifAgendado == null)
                        throw new BilheteriaException("Venda n�o foi gerada. Verificar a Data de Entrega");
                    else
                ...
                //passa pelos itens reservados registrando nas tabelas necess�rias.
                foreach (EstruturaRetornoReservaValeIngresso valeIngresso in reservados)
                {
                    ...
                    retornoItem.Status = (ValeIngresso.enumStatus)statusFinal; //necess�rio preencher para o registro de impress�o
                    ...
                    //INFORMA��ES GERADAS DE ACORDO COM A IMPRESS�O
                    //Gera o c�digo de troca, codigo de barra e validade, caso necess�rio
                    if (imprimir)
                    {
                        //CODIGO TROCA
                        if (valeIngresso.CodigoTroca.Trim().Length == 0)//Codigo din�mico
                    ...
                    {
                        //Se n�o vai imprimir n�o deve gerar agora.
                        if (valeIngresso.CodigoTroca.Trim().Length == 0)
                            codigoTroca = string.Empty;
                        else//Codigo fixo
                            codigoTroca = valeIngresso.CodigoTroca;

                        //Codigo barra gerado na impress�o
                    ...
                    if (codigoImpressao == -1)
                        throw new BilheteriaException("Status do ingresso n�o p�de ser atualizado. Por favor, refa�a todas as reservas.(" + valeIngresso.ID + "-" + usuarioID + ")");
                    else
                    {
                        //VALE INGRESSO LOG
                        ...
                        //Se for imprimir j� registra a impress�o. 
                        if (imprimir)
                        {
                            oValeIngressoLog.CodigoBarra.Valor = codigoBarra;
                            oValeIngressoLog.Acao.Valor = IngressoLog.IMPRIMIR;
                            //Insere o registro de Impress�o
                            cmd.CommandText = oValeIngressoLog.StringInserir();
                            if (cmd.ExecuteNonQuery() != 1)
                                throw new BilheteriaException("Log de impress�o do ingresso n�o foi inserido.");
                        }
                    }
                ...
                #region Manipula��o de formas de pagamento
                //VendaBilheteriaFormaPagamento
                ...
                //Preenche as informa��es restantes
            ...
            catch (Exception ex)
            {
                ...
                catch (ArgumentException) { } // transa��o n�o espec�ficada
                catch (InvalidOperationException) { }// J� foi dado o commit ou o rollback
                catch (Exception ex1)
                {
        ...
        public EstruturaRetornoVenda Vender(DataTable itensReservados, DataTable ingressosVendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string NotaFiscalCliente, string NotaFiscalEstabelecimento, int IndiceInstituicaoTransacao, int IndiceTipoCartao, int NSUSitef, int NSUHost, int CodigoAutorizacaoCredito, int BIN, int ModalidadePagamentoCodigo, string ModalidadePagamentoTexto, int CupomID, Dictionary<int, int> IDsComDesconto, Dictionary<int, int> ingressoXcliente, Canal.TipoDeVenda tipoVenda, bool preReserva, bool preReservaEfetivar, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, List<int> VirsID, string codigoTrocaFixo, string CodigoRespostaVenda, string mensagemretorno, string horatransacao, string datatransacao, string codigoir, string numeroautorizacao, string cupom, string dadosConfirmacaoVenda, string Rede, string CodigoRespostaTransacao, bool AntiFraude, int clienteEnderecoID, EstruturaEntregaAgenda entregaSelecionada, int pdvID, string NomeCartao, bool CartaoOutraPessoa, decimal SeguroMondial)
        {
            //verificar se os dados est�o preenchidos
            if (NotaFiscalCliente.Trim() == string.Empty)
                throw new Exception("Falha no envio de dados da transa��o TEF");
        ...
        public EstruturaRetornoVenda Vender(DataTable itensReservados, DataTable ingressosVendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string NumeroCartao, int CupomID, Dictionary<int, int> IDsComDesconto, Dictionary<int, int> ingressoXcliente, Canal.TipoDeVenda tipoVenda, bool preReserva, bool preReservaEfetivar, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, List<int> VirsID, string codigoTrocaFixo, int clienteEnderecoID, EstruturaEntregaAgenda entregaSelecionada, int pdvID, string NomeCartao, bool CartaoOutraPessoa, decimal SeguroMondial)
        {
            int NroCartao = 0;
        ...
        /// <summary>
        /// Faz a venda para esses ingressos. Retorna a senha �nica da venda. Retorna null, caso nao conseguiu vender.
        /// </summary>
        /// <returns></returns>
        private EstruturaRetornoVenda vender(DataTable itensReservados, DataTable ingressosVendidos, DataTable pagamentos, int caixaID, int lojaID, int canalID, int empresaID, int clienteID, int taxaEntregaID, decimal valorEntrega, decimal valorTotal, int usuarioID, decimal troco, bool imprimir, string NotaFiscalCliente, string NotaFiscalEstabelecimento, int IndiceInstituicaoTransacao, int IndiceTipoCartao, int NSUSitef, int NSUHost, int CodigoAutorizacaoCredito, int BIN, int ModalidadePagamentoCodigo, string ModalidadePagamentoTexto, int CupomID, Dictionary<int, int> IDsComDesconto, Dictionary<int, int> ingressoXcliente, Canal.TipoDeVenda tipoVenda, bool preReserva, bool preReservaEfetivar, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, List<int> VirIds, string codigoTrocaFixo, string CodigoRespostaVenda, string MensagemRetorno, string HoraTransacao, string DataTransacao, string CodigoIR, string NumeroAutorizacao, string Cupom, string DadosConfirmacaoVenda, string Rede, string CodigoRespostaTransacao, bool AntiFraude, int clienteEnderecoIDSelecionado, EstruturaEntregaAgenda entregaSelecionada, int pdvIDSelecionado, string NomeCartao, bool CartaoOutraPessoa, decimal SeguroMondial, BD bd = null)
        {

            bool transacaoExterna = (bd != null) ? true : false;
            ...
            try
            {
                ...
                //Divis�o de ingressos por tipo. Manipula��o vari�vel.
                ...
                foreach (DataRow item in itensNormais)
                {
                    ...
                    if (imprimir && tipoVenda != Canal.TipoDeVenda.ImpressaoVoucher && (!preReserva))
                    {
                        ...
                        //C�digo de barras � gerado apenas quando h� necessidade de registrar a impress�o do ingresso.
                        ...
                        if (codigoBarra.Trim().Length > 0)
                        {
                            ...
                            if (bd.Executar(ingressoCodigoBarra.StringInserir()) != 1)
                                throw new BilheteriaException("N�o foi possivel inserir na tIngressoCodigoBarra.");
                        }
                    }
                    else
                        codigoBarra = ingressos[0][CODIGO_BARRA].ToString(); // Se n�o for imprimir, codigo de barras igual ao que est� no ingresso -- pode ser lista branca!		   

                    string SQL = string.Empty;

                    if (preReservaEfetivar)
                        // Registra a efetiva��o da pr�-reserva. Se existir o cliente selecionado, atualiza com o ID do Cliente
                    ...
                    if (codigoImpressao == -1)
                        throw new BilheteriaException("Status do ingresso n�o p�de ser atualizado. Por favor, refa�a todas as reservas.(" + (int)ingressos[0][INGRESSOID] + "-" + usuarioID + ")");
                    else
                    {
                    ...
                    if ((int)ingressos[0][COTA_ITEM_ID] > 0 || (int)ingressos[0][COTA_ITEM_ID_APS] > 0)
                    {
                        ...
                        if (oIngressoCliente.IngressoID.Valor == 0)
                        {
                            oIngressoCliente.IngressoID.Valor = (int)ingressos[0][INGRESSOID];
                        }
                        ...
                        oIngressoCliente.CotaItemID.Valor = (int)ingressos[0][COTA_ITEM_ID];

                        if (!transacaoExterna)
                            bd.FinalizarTransacao();

                        Ingresso oIngresso = new Ingresso();
                        oIngresso.AdicionarCotaItem2((int)ingressos[0][INGRESSOID], (int)ingressos[0][COTA_ITEM_ID]);
                ...
                foreach (DataRow item in itensMesaFechada)
                {
                    ...
                    foreach (DataRow ingresso in ingressos)
                    {
                        ...
                        if (imprimir && tipoVenda != Canal.TipoDeVenda.ImpressaoVoucher && (!preReserva))
                        {
                            ...
                            codigoBarra = ingresso[CODIGO_BARRA].ToString();// Se n�o for imprimir, codigo de barras igual ao que est� no ingresso -- pode ser lista branca!

                        string SQL = string.Empty;

                        if (!preReserva)
                            //Registra o ingresso em quest�o para o cliente e devolve o CodigoImpressao Atualizado
                            SQL = @"EXEC dbo.IncrementarUltimoCodigoImpressoVenda
                                               @CodigoBarra = '" + codigoBarra + "' , @LojaID = " + lojaID + ", @VendaBilheteriaID = " +
                                                vendaBilheteria.Control.ID +
                                                " , @Status = '" + statusFinal + "', @ClienteID = " + clienteID + ", @UsuarioID = " + usuarioID +
                        ...
                        if ((int)ingresso[COTA_ITEM_ID] > 0 || (int)ingresso[COTA_ITEM_ID_APS] > 0)
                        {
                            ...
                            oIngressoCliente.CotaItemID.Valor = (int)ingressos[0][COTA_ITEM_ID] > 0 ? (int)ingressos[0][COTA_ITEM_ID_APS] : dono.CotaItemID;
                ...
                foreach (DataRow item in itensPacote)
                {
                    ...
                    foreach (DataRow ingresso in ingressosPacote)
                    {
                        ...
                        if (codigoImpressao == -1)
                            throw new BilheteriaException("Status do ingresso n�o p�de ser atualizado. Por favor, refa�a todas as reservas.(" + (int)ingresso[INGRESSOID] + "-" + usuarioID + ")");
                        else
                        {
                        ...
                        if ((int)ingresso[COTA_ITEM_ID] > 0 || (int)ingresso[COTA_ITEM_ID_APS] > 0)
                        {
                            ...
                            oIngressoCliente.ApresentacaoID.Valor = (int)ingresso[APRESENTACAOID];
                            oIngressoCliente.ApresentacaoSetorID.Valor = (int)ingresso[APRESENTACAOSETORID];
                            oIngressoCliente.CotaItemID.Valor = (int)ingresso[COTA_ITEM_ID];
                            oIngressoCliente.IngressoID.Valor = (int)ingresso[INGRESSOID];

                            if (!transacaoExterna)
                                bd.FinalizarTransacao();

                            Ingresso oIngresso = new Ingresso();
                            oIngresso.AdicionarCotaItem2((int)ingresso[INGRESSOID], (int)ingresso[COTA_ITEM_ID]);
                            bd.IniciarTransacao();
                            bd.Executar(oIngressoCliente.StringInserir());
                        }
                    }
                ...
                for (int i = 0; i < pagamentos.Rows.Count; i++)
                {
                    ...
                    else if (VIR == Convert.ToInt32(pagto["ID"]))
                    {
                        ...
                        if (Convert.ToInt32(pagto["VirID"]) == 0 && codigoTrocaFixo.Length == 0)
                            throw new Exception("A forma de pagamento Vale ingresso foi informada no entanto o c�digo de troca n�o foi encontrado.");
                    }
                    else
                        vendaBilheteriaFormaPagamento.Valor.Valor = (decimal)pagto[VALOR];
                    ...
                    #region BlackList - Insere Cart�o na tBlackList
                    ...
                    if (codigoTrocaFixo.Length > 0)
                    {
                        ...
                        if (!oTrocaFixa.Encontrado)
                            throw new BilheteriaException("Vale Ingresso n�o encontrado");
                        ...
                        else
                        {
                            ...
                            if (bd.Executar(valeIngressoLog.StringInserir()) == 0)
                                throw new Exception("N�o foi possivel gerar o Log do Vale Ingresso, Troca imposs�vel.");
                    ...
                    else if (VirIds.Count > 0)
                    {
                        foreach (int virID in VirIds)
                        {
                            if (virID == (int)pagto["VirID"])
                            {
                                ...
                                if (bd.Executar(SQL) != 1)
                                    throw new BilheteriaException("Nao foi poss�vel Trocar o Vale Ingresso - A mudan�a do Status gerou um erro.");
                                else
                                {
                                    valeIngressoLog = new ValeIngressoLog();
                                    valeIngressoLog.Acao.Valor = ((char)ValeIngressoLog.enumAcao.Trocar).ToString();
                ...
                try
                {
                    ...
                    if (SeguroMondial > 0)
                    {
                        ...
                        Mondial.MDLPolicyRequest mdlPolicyRequest = new Mondial.MDLPolicyRequest();
                        ...
                        mdlPolicyRequest.PaymentTypeID = 2; // 1: Cart�o de Cr�dito; 2: Pagamento autom�tico
                        ...
                        // Ap�lice prim�ria, no caso da Ingresso R�pido, sempre passar vazio
                        ...
                        // Identificador da compra na Ingresso R�pido
                        mdlPolicyRequest.ReferenceOrderCode = vendaBilheteria.Control.ID.ToString();

                        mdlPolicyRequest.RequestDate = DateTime.Now;
                        mdlPolicyRequest.RequestOriginID = 1; // No caso da Ingresso R�pido, sempre 1
                        mdlPolicyRequest.RequestStatus = 1; // Booking Path (7): Pronto para cobran�a do cart�o via gateway
                        // Pagamento autom�tico (1): Compras sem pagamento (faturamento inverso)

                        // Valor
                        mdlPolicyRequest.TotalCost = SeguroMondial; // Valor dos produtos * qtde. de segurados

                        // Instanciar cada segurado.
                        Mondial.MDLCustomer[] mdlCustomer = new Mondial.MDLCustomer[ingressosVendidos.Rows.Count];
                        ...
                        foreach (DataRow item in ingressosVendidos.Rows)
                        {
                            ...
                            mdlCustomer[cont] = new Mondial.MDLCustomer();
                            ...
                            mdlCustomer[cont].InsuredID = 0; // No caso da ingresso R�pido, sempre 0
                            mdlCustomer[cont].InsuredName = oCliente.Nome.Valor; // Nome do Cliente
                            mdlCustomer[cont].ProductID = ProductID; // Produto Mondial

                            // Dados do Evento
                            mdlCustomer[cont].EventClave = estruturaInfo.ApresentacaoID.ToString(); // C�digo do Evento. Este � o identificador do evento na Ingresso R�pido. IMPORTANTE sempre informar o mesmo c�digo para o mesmo evento!!
                            mdlCustomer[cont].EventName = estruturaInfo.Evento;
                            mdlCustomer[cont].EventDate = estruturaInfo.Horario;
                            mdlCustomer[cont].EventID = 0; // No caso da Ingresso R�pido, sempre 0, pois o identificador do evento � o campo "EventClave"

                            // Dados do setor
                            mdlCustomer[cont].SectorCode = estruturaInfo.SetorID + "_" + item["PrecoID"]; // C�digo do Setor. Este � o identificador do setor do evento na Ingresso R�pido. IMPORTANTE sempre informar o mesmo c�digo para o mesmo setor!!
                            ...
                            mdlCustomer[cont].SectorID = 0; // No caso da Ingresso R�pido, sempre 0, pois o identificador do setor � o campo "SectorCode"
                        ...
                        Mondial.MDLRequestResult[] mdlRequestResult;
            ...
            catch (Exception ex)
            {
                try
                {
                    //TODO: FABRICIO --> Venda N�o encerra caso seja externa
                    if (!transacaoExterna)
                        bd.DesfazerTransacao();
                }
                catch (ArgumentException) { } // transa��o n�o espec�ficada
        ...
        /// <summary>
        ///		Verifica se existe um caixa aberto para o Usu�rio, se existe verifica se � de hoje, e se n�o for, fecha o caixa atual e 
        ///		abre um novo caixa.
        /// </summary>
        /// <returns></returns>
        ...
        /// </summary>
        /// <returns></returns>
        public int VerificaCaixaInternet(int usuarioID, int lojaID)
        {
            Servidor servidor = new Servidor();
        ...
        public DataSet Carregar(int usuarioID, int canalID, int empresaID, Canal.TipoDeVenda tipoDeVenda)
        {
            ...
            try
            {
                ...
                bd.Fechar();
                if (usuarioID == 0)
                    throw new BilheteriaException("Usu�rio incorreto!");
                ...
                // Carregar informa��es de formas de pagamento do canal
                sql = string.Empty +
                ...
                // Carrega as cortesias dispon�veis.
                sql = string.Empty +
                ...
                //Carrega informa��es de pacotes com lugar marcado
                sql = string.Empty +
                ...
                //Carrega informa��es dos setores dos pacotes
                sql = string.Empty +
                ...
                //Carrega informa��es dos Vale Ingressos
                sql = string.Empty +
                ...
                if (locais.Rows.Count == 0 && series.Rows.Count == 0 && pacotes.Rows.Count == 0 && pacotesLugarMarcado.Rows.Count == 0)
                    throw new BilheteriaException("N�o existem informa��es suficientes para venda. Verifique se existem eventos/pre�os distribuidos para o canal selecionado.");
        ...
        public static DataSet CarregarDoServico(int CanalID, bool DadosPeloCache)
        {
            try
            {
                ...
                File.WriteAllText("schema.txt", Arquivos.SchemaBuffer);
        ...
        public RetornoCarregarDados NovoCarregar(int canalID)
        {
            ...
            try
            {
                ...
                var carregartodos = Configuracao.GetBoolean(Configuracao.Keys.CarregarTodos, versao);
                ...
                {
                    retorno.CarregadoComSucesso = false;
                    retorno.Mensagem = "N�o existem informa��es suficientes para venda. Verifique se existem eventos/pre�os distribuidos para o canal selecionado.";
                    retorno.Dados = null;
                    return retorno;
                }
        ...
        public DataSet CarregarSerieItem(int SerieID, int CanalID)
        {
            ...
            try
            {
                ...
                if (!temItem)
                    throw new Exception("N�o existem eventos dispon�veis para a s�rie selecionada, por favor, tente outra s�rie.");
        ...
        /// <summary>
        /// M�todo respons�vel por alterar o status de um ou mais vale ingressos, al�m de alterar o seu respectivo c�digo de barras e inserir o registro de auditoria.
        /// A rotina est� preparada para registrar impress�es ou reimpress�es de tickets conforme par�metro reimpressao.
        /// </summary>
        /// <param name="ingressosIDs">IDs de ingressos a serem "impressos"</param>
        /// <param name="usuarioID">ID do usu�rio respons�vel pela a��o</param>
        /// <param name="empresaID">ID da empresa do usu�rio respons�vel pela a��o</param>
        /// <param name="caixaID">ID do caixa do Usu�rio - caixa deve estar aberto</param>
        /// <param name="canalID">ID do canal em que o usu�rio tem o caixa aberto</param>
        /// <param name="lojaID">ID da loja em que o usu�rio tem o caixa aberto</param>
        /// <param name="reimpressao">Defini��o do tipo de a��o (Para reimpress�o = true)</param>
        /// <param name="motivoReimpressao">Motivo da reimpress�o (vazio em caso de impress�o de ticket)</param>
        /// <returns>Objeto Dictionary composto por ID do ingresso (int) e c�digo de barras gerado (string)</returns>
        public List<EstruturaRetornoRegistroImpressao> RegistrarImpressaoValeIngresso(List<EstruturaRegistroImpressaoVir> valeIngressos, int vendaBilheteriaID, int usuarioID, int empresaID, int caixaID, int canalID, int lojaID, bool reimpressao, string motivoReimpressao, int SupervisorID, EstruturaLogistica estruturaLogistica)
        {
            #region Consist�ncias
            if (usuarioID == 0)
                throw new BilheteriaException("Usuario nao pode ser nulo.");
            ...
            try
            {
                ...
                foreach (EstruturaRegistroImpressaoVir valeIngresso in valeIngressos)
                {
                    ...
                    if (!Convert.ToBoolean(bd.Executar(oValeIngressoLog.StringInserir())))
                        throw new BilheteriaException("Log de impress�o do ingresso n�o foi inserido."); // Falha ao inserir o registro.
        ...
        /// <summary>
        /// M�todo respons�vel por alterar o status de um ou mais ingressos para impresso al�m de alterar o seu respectivo c�digo de barras e inserir o registro de auditoria.
        /// A rotina est� preparada para registrar impress�es ou reimpress�es de tickets conforme par�metro reimpressao.
        /// </summary>
        /// <param name="ingressosIDs">IDs de ingressos a serem "impressos"</param>
        /// <param name="usuarioID">ID do usu�rio respons�vel pela a��o</param>
        ...
        /// <param name="motivoReimpressao">Motivo da reimpress�o (vazio em caso de impress�o de ticket)</param>
        /// <returns>Objeto Dictionary composto por ID do ingresso (int) e c�digo de barras gerado (string)</returns>
        private List<EstruturaRetornoRegistroImpressao> RegistrarImpressao(int[] ingressosIDs, int usuarioID, int empresaID, int caixaID, int canalID, int lojaID, bool reimpressao, string motivoReimpressao, bool impressaoEmLote, bool registrarVendaBilheteriaAntiFraude, int quantidadeImpressoesInternet, EstruturaLogistica estruturaLogistica, int SupervisorID, bool ImpressaoPos)
        {
            //TODO: Tentar remover a select de pesquisa no ingresso.

            // Detalhes:
            // Rotina respons�vel por alterar o status de um ingresso de Vendido para Impresso ou atualizar o c�digo de barras no caso de reimpress�o e inserir o log de auditoria.
            // Passos:
            //		1) Consist�ncias de par�metros. Todos s�o obrigat�rios.
            //		2) Tratamento do tipo de a��o a ser executada - Impress�o / Reimpress�o
            //		3) Abertura de transa��es.
            //		4) Cria��o do c�digo de barras com base no precoID.
            //		5) Mudan�a do status (tIngresso).
            //		6) Auditoria na tIngressoLog.
            //      7) Mudan�a do status (tVendaBilheteria) case seja necess�rio --> Apenas telas de Impressao e ImpressaoEmLote
            //      8) Envia o Email para tracking de pedidos
            //		8) Retorno do dicion�rio combinado de IngressoID x C�digo de barras gerado.

            LogUtil.Debug(string.Format("##Bilheteria.RegistrarImpressao## INGRESSO_IDS: {0}", string.Join(",", ingressosIDs.Select(s => s.ToString()).ToArray())));

            #region Consist�ncias

            if (reimpressao == true && SupervisorID == 0)
                throw new BilheteriaException("Impress�o precisa ser autorizada pelo Supervisor");
            ...
            try
            {
                bd.IniciarTransacao(); // Start da transa��o no banco de dados.
                List<EstruturaRetornoRegistroImpressao> retorno = new List<EstruturaRetornoRegistroImpressao>(); // reposit�rio de combina��es ingressoID X c�digo de barras e CodigoImpressao

                CodigoBarra oCodigoBarra = new CodigoBarra(); // Objeto com m�todos para cria��o do c�digo de barras.

                int ingressosAtualizados = 0;
                int vendaBilheteriaID = 0;

                // Vai do primeiro ao �ltimo ingressoID da lista atualizando status, gerando c�digo de barras e inserindo registros de auditoria.
                foreach (int ingressoID in ingressosIDs)
                {
                    // Busca as informa��es do ingresso.
                    using (IDataReader oDataReader = bd.Consulta("" +
                        "SELECT " +
                    ...
                    {
                        if (!oDataReader.Read())
                            oDataReader.Close();// Caso nada tenha sido encontrado, fecha a consulta e vai ao pr�ximo item.
                        else
                        {
                            ingressosAtualizados++;

                            // Popula vari�veis com informa��es da consulta.
                            ...
                            {
                                //Incrementa o Codigo de impress�o e atualiza na tIngresso:
                                ...
                                if (reimpressao)
                                {
                                    ...
                                    // Isso � nescess�rio para invalidar codigos de barra ao reimprimir ou cancelar um ingresso.
                                ...
                                // C�digo de barras - um novo a cada impress�o ou reimpress�o.
                                if (tipoCodigoBarra == Enumerators.TipoCodigoBarra.Estruturado && !ImpressaoPos)
                                    codigoBarra = oCodigoBarra.NovoCodigoBarraIngresso(precoID, eventoID, codigoImpressao); // Cria um novo c�digo de barras.
                                ...
                                }); // adiciona a combina��o � lista.
                            }

                            //Atualiza o ingresso no banco de dados e armazena o n�mero de linhas afetadas na vari�vel linhasAlteradas.
                            // Status do ingresso sempre ser� impresso independente do tipo de a��o.
                            ...
                            if (linhasAlteradas != 1) // Erro caso o retorno seja diferente de 1.
                                throw new BilheteriaException("N�o foi poss�vel alterar o status do ingresso..");
                            else
                            {
                                ...
                                ingressoLog.CanalID.Valor = canalID;
                                ingressoLog.LojaID.Valor = lojaID;
                                ingressoLog.CodigoBarra.Valor = codigoBarra; // C�digo de barras gerado acima.
                ...
                LogUtil.Debug(string.Format("##Bilheteria.RegistrarImpressao.SUCCESS## INGRESSO_IDS: {0}", string.Join(",", ingressosIDs.Select(s => s.ToString()).ToArray())));

                return retorno;
            }
            catch (Exception ex)
            {
                LogUtil.Error(string.Format("##Bilheteria.RegistrarImpressao.EXCEPTION## INGRESSO_IDS: {0}, MSG: {1}", string.Join(",", ingressosIDs.Select(s => s.ToString()).ToArray()), ex.Message), ex);
                ...
                foreach (int ingressoID in ingressosIDs)
                {
                    ...
                    else
                    {
                        ...
                        if (codigoBarra.Trim().Length > 0)
                        {
                            ...
                            throw new Exception("N�o ser� poss�vel gerar o c�digo de barras.\nN�o existem mais registros na lista branca.\nPor favor, entre em contato com a equipe de atendimento.");
        ...
        /// <param name="lojaID">ID da loja em que o usu�rio tem o caixa aberto</param>
        /// <returns>Objeto Dictionary composto por ID do ingresso (int) e c�digo de barras gerado (string)</returns>
        public List<EstruturaRetornoRegistroImpressao> RegistrarImpressaoEmLote(int[] ingressosIDs, int usuarioID, int empresaID, int caixaID, int canalID, int lojaID, bool registrarVendaBilheteriaAntiFraude, EstruturaLogistica estruturaLogistica, int SupervisorID, bool ImpressaoPos)
        {
        ...
        /// <summary>
        /// M�todo respons�vel por registrar a reimpress�o de um ou mais ingressos al�m de alterar o seu respectivo c�digo de barras.
        /// </summary>
        ...
        /// <param name="canalID">ID do canal em que o usu�rio tem o caixa aberto</param>
        /// <param name="lojaID">ID da loja em que o usu�rio tem o caixa aberto</param>
        /// <param name="motivo">Motivo da Reimpress�o</param>
        /// <returns>Objeto Dictionary composto por ID do ingresso (int) e c�digo de barras gerado (string)</returns>
                ...
                foreach (int ingressoID in ingressosIDs)
                {
                    ...
                    if (bd.Consulta().Read())
                    {
                        ...
                        else
                        {
                            ...
                            throw new BilheteriaException("Status do ingresso n�o foi atualizado.");
                        }
                    }
                    else
                    {
            ...
            try
            {
                ...
                foreach (int ingressoID in ingressosIDs)
                {
                    ...
                    if (bd.Consulta().Read())
                    {
                        ...
                        if (codigoImpressao == -1)
                            throw new Exception("N�o foi poss�vel emitir o pr�-impresso, erro ao incrementar o �timo c�digo de impress�o");
                        ...
                        if (!okV)
                            throw new BilheteriaException("Log de pr�-impress�o do ingresso " + ingressoID + " n�o foi inserido.");
                    ...
                    {
                        bd.Consulta().Close();
                        throw new BilheteriaException("Pr�-impress�o do ingresso " + ingressoID + " n�o foi conclu�do.");
                    }
            ...
            try
            {
                ...
                foreach (int ingressoID in ingressosIDs)
                {
                    ...
                    if (ok)
                    {
                        ...
                        if (!okB)
                            throw new BilheteriaException("Log de bloqueio do ingresso n�o foi inserido.");
                    }
                    else
                    {
        ...
        public int Bloquear(int[] bloqueioIDs, int apresentacaoSetorID, int[] lugarIDs, int[] qtdes, int usuarioID, int empresaID)
        {
            ...
            for (int i = 0; i < lugarIDs.Length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        throw new BilheteriaException("Ingressos n�o-dispon�veis..");
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public int Desbloquear(int apresentacaoSetorID, int[] lugarIDs, int usuarioID, int empresaID, bool parceiroMidia = false)
        {
            ...
            for (int i = 0; i < lugarIDs.Length; i++)
            {
                ...
                try
                {
                    ...
                    foreach (int ingressoID in ingressosIDs)
                    {
                        ...
                        {
                            bd.Executar(
                            @"insert into ParceiroMidiaControleIngresso(ParceiroMidiaID, IngressoID, DataLiberacao, Status, ClasseID)
                            SELECT Top 1 i.ParceiroMidiaID, i.ID, GETDATE() as DataLiberacao,'L' as Status, pmc.ID as ClasseID
                            FROM [tIngresso] i
                            left join [ParceiroMidiaClasseSetor] pmcs on i.SetorID = pmcs.SetorID
                            left join [ParceiroMidiaClasse] pmc on pmc.ID = pmcs.ParceiroMidiaClasseID
                            where i.ID = " + ingressoID + ";");
                        }

                        string sqlUpdate = "UPDATE tIngresso SET Status='" + Ingresso.DISPONIVEL + "', " +
                            (parceiroMidia ? "ParceiroMidiaID=null, " : "") +
                            "BloqueioID=0 " +
                            "WHERE " +
                            (parceiroMidia ? "" : " (AssinaturaClienteID IS NULL OR AssinaturaClienteID = 0) AND ") +
                            "Status='" + Ingresso.BLOQUEADO + "' AND ID=" + ingressoID;
                        int x = bd.Executar(sqlUpdate);
                        bool ok = (x == 1);
                        if (ok)
                        {
                        ...
                        {
                            bd.Executar("Delete from ParceiroMidiaControleIngresso WHERE IngressoID = " + ingressoID + ";");
                        }
        ...
        /// </summary>
        /// <returns></returns>
        public int DesbloquearIngresso(int apresentacaoSetorID, int[] ingressosID, int usuarioID, int empresaID, bool parceiroMidia = false)
        {
            ...
            for (int i = 0; i < ingressosID.Length; i++)
            {

                try
                {

                    int ingressoID = ingressosID[i];
                    ...
                    if (parceiroMidia)
                    {
                        ...
                        @"insert into ParceiroMidiaControleIngresso(ParceiroMidiaID, IngressoID, DataLiberacao, Status, ClasseID)
                            ...
                            left join [ParceiroMidiaClasseSetor] (NOLOCK) pmcs on i.SetorID = pmcs.SetorID
                            left join [ParceiroMidiaClasse] (NOLOCK) pmc on pmc.ID = pmcs.ParceiroMidiaClasseID
                            where i.ID = " + ingressoID + ";");
                    }
        ...
        public DataSet MontaDadosVendaImportacaoAssinatura(int[] ingressosID, int clienteID, int usuarioID, int canalID)
        {
            ...
            // Buscar as informa��es de pacotes.
            ...
            //                linha["TaxaConvenienciaValor"] = 0; // Ser� atualizada conforme ingressos. 
            ...
            //                linha[VALOR] = 0; // Ser� atualizada conforme ingressos.
            ...
            //somente a manipula��o de pacotes deve ser usada para assinatura
            ...
            foreach (DataRow pacote in pacotesReservados.Rows)
            {
                ...
                linha[COMISSAO_VALOR] = 0; // Ser� atualizada conforme ingressos.
                linha[TIPO] = TIPO_PACOTE;
        ...
        /// <summary>
        /// Retorna os setores que ainda possuem qtde disponivel + qtde disponivel + primeiro pre�o cadastrado com ingresso disponivel
        /// </summary>
        /// <param name="tabelaSetores">Informe os setoresIDs para retonar as informa��es de qtde e pre�o</param>
        /// <param name="lstApresentacoes">Lista de apresenta��es a serem atualizadas</param>
        /// <returns></returns>
        public DataTable AtualizarSetoresQtdePreco(DataTable tabelaSetores, List<string> lstApresentacoes)
        {
            ...
            try
            {
                //Cria uma tabela tempor�ria para guardar os setores que dever�o ter suas quantidades atualizadas
                ...
                //Faz um foreach que cria as strings de inser��o para o banco
                ...
                //Esse update traz as quantidades para a tabela tempor�ria
        ...
        public void ReservarAssinatura(BD bdReserva, int clienteID, int assinaturaID, int setorID, int lugarID, int assinaturaAnoID, EstruturaReservaInternet estruturaIdentificacaoUsuario, int assinaturaClienteID)
        {
            BD bd = new BD();
            Ingresso ingresso = new Ingresso();
            ...
            try
            {
                ...
                if (!bd.Consulta().Read())
                    throw new Exception("Este lugar n�o est� dispon�vel para reserva.");

                do
                {
                    string status = bd.LerString("Status");
                    if (status != Ingresso.DISPONIVEL)
                        throw new IngressoException("Este lugar n�o est� dispon�vel para reservas, por favor tente outro lugar.");
                    ...
                    if (bdReserva.Executar(ingresso.StringReservar()) != 1)
                        throw new IngressoException("N�o foi poss�vel efetuar a reserva desta assinatura, por favor tente novamente.");
        ...
        public List<EstruturaRetornoVendaPos> ReservarPos(string poscounter, string posnumber, string posticket)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    List<IRLib.Lugar> lugares = new IRLib.Lugar().MelhorLugarMarcado(qtdReserva, estruturaEvento.ApresentacaoSetorID, tipoSetor);
                    ...
                    if (mesaAberta)
                    {
                        foreach (IRLib.Lugar itemLugar in lugares)
                            quantidadeMesaAberta += itemLugar.Quantidade.Valor;

                        if (quantidadeMesaAberta != qtdReserva)
                        {
                            lugares.Clear();
                            throw new Exception("N�o existem mesas com a capacidade de acomodar todas as pessoas juntas");
                        }
                    }
                    else
                    ...
                    foreach (IRLib.Lugar lugar in lugares)
                    {
                        if (!mesaAberta)
                ...
                ingressosIDs = new int[retorno.Tables[Bilheteria.TABELA_RESERVA].Rows.Count];

                for (int i = 0; i < retorno.Tables[Bilheteria.TABELA_RESERVA].Rows.Count; i++)
                    if (string.IsNullOrEmpty(retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i][Bilheteria.CODIGO_BARRA].ToString()))
                        ingressosIDs[i] = (int)retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i]["IngressoID"];
                ...
                if (retornoRegistroImpressao != null || retornoRegistroImpressao.Count != 0)
                {
                    ...
                    for (int i = 0; i < retorno.Tables[Bilheteria.TABELA_RESERVA].Rows.Count; i++)
                    {
                        ...
                        {
                            if (item.ID == Convert.ToInt32(retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i]["IngressoID"]))
                            {
                                retornoVenda.Add(new EstruturaRetornoVendaPos
                                {
                                    Contador = contadorIngresso,
                                    EventoID = Convert.ToInt32(retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i]["EventoID"]),
                                    IngressoID = Convert.ToInt32(retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i]["IngressoID"]),
                                    Senha = Convert.ToString(retorno.Tables[Bilheteria.TABELA_GRID].Rows[i]["Cod"]),
                                    CodigoBarra = item.CodigoBarra,
                                    PrecoID = Convert.ToInt32(retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i]["PrecoID"])
                                });

                                retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i][Bilheteria.CODIGO_BARRA] = item.CodigoBarra;
                                retorno.Tables[Bilheteria.TABELA_RESERVA].Rows[i][Bilheteria.CODIGO_SEQUENCIAL] = item.CodigoImpressao;
        ...
        public bool VenderPos(DataSet reserva, EstruturaEstadoCidadeSenhaPos estrutura, string ticketbarcode, string poscpf)
        {
            try
            {
                ...
                DataRow[] item = reserva.Tables[Bilheteria.TABELA_RESERVA].Select("CodigoBarra='" + ticketbarcode + "'");

                foreach (DataRow linha in reserva.Tables[Bilheteria.TABELA_GRID].Select("ReservaID=" + item[0][Bilheteria.RESERVAID].ToString()))
                {
                    EstruturaVendaPos estruturaEvento = new Evento().BuscaInfosVendaPos(Convert.ToInt32(linha[Bilheteria.PRECOID]));

                    taxaMaximaPorEmpresa = Convert.ToDecimal(new Empresa().CarregaTaxa(estruturaEvento.EmpresaID));
                    decimal valor = (decimal)linha[Bilheteria.VALOR];
                    int conv = (int)linha[Bilheteria.CONV];
                    decimal taxaMin = (decimal)linha[Bilheteria.CONV_MIN];
                    decimal taxaMax = (decimal)linha[Bilheteria.CONV_MAX];
                    ...
                    if (valorTotalConveniencia < taxaMaximaPorEmpresa || taxaMaximaPorEmpresa == 0)
                        linha[Bilheteria.VALOR_CONV] = dAux;
                    else
                    {
                        if ((dAux - (valorTotalConveniencia - taxaMaximaPorEmpresa)) < 0)
                            linha[Bilheteria.VALOR_CONV] = 0;
                        else
                            linha[Bilheteria.VALOR_CONV] = (dAux - (valorTotalConveniencia - taxaMaximaPorEmpresa));
                    }
                    ...
                    DataTable tGridLeve = reserva.Tables[Bilheteria.TABELA_GRID].Copy();

                    foreach (DataRow excluir in tGridLeve.Select("ReservaID<>" + item[0][Bilheteria.RESERVAID].ToString()))
                        excluir.Delete();

                    tGridLeve.Columns.Remove(Bilheteria.EVENTO_PACOTE);
                    tGridLeve.Columns.Remove(Bilheteria.HORARIO);
                    tGridLeve.Columns.Remove(Bilheteria.SETOR_PRODUTO);
                    tGridLeve.Columns.Remove(Bilheteria.CODIGO);
                    tGridLeve.Columns.Remove(Bilheteria.PRECO);
                    tGridLeve.Columns.Remove(Bilheteria.CORTESIA);
                    tGridLeve.Columns.Remove(Bilheteria.VALOR);
                    tGridLeve.Columns.Remove(Bilheteria.LUGAR_MARCADO);
                    tGridLeve.Columns.Remove(Bilheteria.CLIENTE);
                    tGridLeve.Columns.Remove(Bilheteria.COTA_ITEM_ID);
                    tGridLeve.Columns.Remove(Bilheteria.COTA);

                    tGridLeve.AcceptChanges();

                    DataTable tReserva = reserva.Tables[Bilheteria.TABELA_RESERVA].Copy(); ;
                    ...
                    retorno = this.AtualizaCodigoBarraPos((int)item[0][Bilheteria.INGRESSOID], estrutura.UsuarioID, estrutura.EmpresaID, caixaID, estrutura.CanalID, estrutura.LojaID, string.Empty, estrutura.UsuarioID, (string)item[0][Bilheteria.CODIGO_BARRA], (int)item[0][Bilheteria.CODIGO_SEQUENCIAL]);
                }
        ...
        public bool CancelarPos(string ticketbarcode, string posnumber)
        {
            try
            {
                ...
                cancelamento.PerfilID = Perfil.SAC_SUPERVISOR;
                cancelamento.EmpresaID = estrutura.EmpresaID;

                DataSet retorno = cancelamento.PesquisarCodigoBarras(ticketbarcode, false);
                ...
                if (tGrid.Rows.Count > 0)
                {
                    ...
                    if (disponivelAjuste.Length == 0)
                    {
                        ...
                        string senhaTmp = this.Cancelar(itensReservados, tReserva, tPagamento, caixaID, estrutura.LojaID, estrutura.CanalID, estrutura.EmpresaID, 0, 0, 0, valorTotalVenda, estrutura.UsuarioID, string.Empty, string.Empty, true,
                            false, "Solicita��o do Cliente", tInfoVenda.Rows[0][CancelamentoGerenciador.STATUS_VENDA].ToString() == VendaBilheteria.FRAUDE, VendaBilheteriaID, 12,
                                    estrutura.UsuarioID, 0, false, 0, false, string.Empty, false, false, string.Empty, true);
                    }
                    else
                        throw new Exception("Evento n�o disponivel para Ajuste");
                }
                else
                    throw new Exception("Sem ingressos para cancelar!");
        ...
        public DataTable ParceirosMidia(string registroZero = null)
        {
            try
            {
                DataTable tabela = new DataTable("ParceiroMidia");

                tabela.Columns.Add("ID", typeof(int));
                tabela.Columns.Add("Nome", typeof(string));

                string sql = "SELECT ID, Nome FROM ParceiroMidia (NOLOCK) ORDER BY Nome";
    ...
    public class VendaCanceladaException : ApplicationException
    {
        ...
        public override string Message
        {
            get
            {
                return string.Format("Venda {0} j� cancelada", this.VendaBilheteriaID.ToString());
            }
        }
    ...
    {
        private Bilheteria.CodMensagemVenda codigoErro = Bilheteria.CodMensagemVenda.ErroIndefinido; // -1 n�o cont�m erro.

        private Bilheteria.CodMensagemReserva codigoErroReserva = Bilheteria.CodMensagemReserva.Nulo; // -1 n�o cont�m erro.

        private Bilheteria.CodMensagemCarregarInternet codigoErroCarregar = Bilheteria.CodMensagemCarregarInternet.Nulo; // -1 n�o cont�m erro.

        public Bilheteria.CodMensagemVenda CodigoErro
        {
            get { return this.codigoErro; }
            set { this.codigoErro = value; }
        }

        public BilheteriaException(string msg, Bilheteria.CodMensagemVenda codigoErro)
            : base(msg)
        {
            this.codigoErro = codigoErro;
        }
        public BilheteriaException(string msg, Bilheteria.CodMensagemReserva codigoErro)
            : base(msg)
        {
            this.codigoErroReserva = codigoErro;
        }
        public BilheteriaException(string msg, Bilheteria.CodMensagemCarregarInternet codigoErro)
            : base(msg)
        {
