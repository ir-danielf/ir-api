...
namespace IRLib
{
    ...
    public class BannerIMais : BannerIMais_B
    {
        ...
        public BannerIMais(int usuarioIDLogado) : base(usuarioIDLogado) { }

        public int GetMaxID()
        {
            try
            {
                string sql = "SELECT ISNULL(MAX(ID), 0) FROM cBanner_IngressoMais";
        ...
        public List<ClientObjects.Banner> CarregarBannersDG()
        {
            ...
            try
            {
                ...
                string sql = "SELECT ID, Nome, Localizacao, Posicao, Descricao FROM tBanner_IngressoMais(nolock) ORDER BY Localizacao, Posicao";
    ...
    public class BannerListaImais : BannerListaImais_B
    {
        public BannerListaImais() { }

        public BannerListaImais(int usuarioIDLogado) : base(usuarioIDLogado) { }


    }

    [Serializable]
    public class BannerListaImais2 : List<IRLib.ClientObjects.Banner>
    {
        public BannerListaImais2() { }
        public BannerListaImais2 CarregarBannersDG()
        {
            BD bd = new BD();
            BannerListaImais2 lista = new BannerListaImais2();
            ...
            try
            {
                ...
                string sql = "SELECT ID, Nome, Localizacao, Posicao FROM tBanner_IngressoMais(nolock) ORDER BY Posicao";

                bd.Consulta(sql);

                while (bd.Consulta().Read())
                {
                    oBanner = new IRLib.ClientObjects.Banner();
                    //popula objeto
                    oBanner.ID = bd.LerInt("ID");
                    oBanner.Nome = bd.LerString("Nome");
                    oBanner.Localizacao = bd.LerInt("Localizacao");
    ...
    public class EstruturaBannersImais
    {
        ...
        public static List<EstruturaBannersImais> retornarListaBanners()
        {
            BD oBD = new BD();
            try
            {
                List<EstruturaBannersImais> listaBanners = new List<EstruturaBannersImais>();

                StringBuilder stbSql = new StringBuilder();
                stbSql.Append("Select Nome, Img, Url, Descricao ");
                stbSql.Append("FROM tBanner_IngressoMais WHERE Localizacao = 2");

                oBD.Consulta(stbSql.ToString());

                while (oBD.Consulta().Read())
                {
                    EstruturaBannersImais banner = new EstruturaBannersImais();
