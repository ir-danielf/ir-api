...
namespace IRLib
{
    ...
    public partial class Cliente : Cliente_B
    {
        ...
        public EstruturaCliente AtribuirDadosEstrutura(IRLib.ClientObjects.EstruturaCliente cliente)
        {
        ...
        public int ClienteManutencao(IRLib.ClientObjects.EstruturaCliente cliente)
        {
            try
            {
        ...
        public List<string> Atualizar(IRLib.ClientObjects.EstruturaCliente cliente, bool tudoObrigatorio)
        {
            if (tudoObrigatorio)
        ...
        public List<string> Atualizar(IRLib.ClientObjects.EstruturaCliente cliente)
        {
            int usuarioSite = UsuarioSiteID;
            bool blnAtualizar = true;
            List<string> oMensagens = new List<string>();
            IRLib.ClientObjects.EstruturaRetornoProcSalvarCliente retornoProc = new EstruturaRetornoProcSalvarCliente();
            ...
            else
            {
                ...
                if (DateTime.ParseExact(cliente.DataNascimentoTS, "yyyy/MM/dd", CultureInfo.InvariantCulture) < DateTime.Now.AddYears(-99))
                {
            ...
            if (blnAtualizar)
            {
                ...
                if (cliente.DataNascimentoTS.Trim() != "")
                    cliente.DataNascimento = DateTime.ParseExact(cliente.DataNascimentoTS, "yyyy/MM/dd", CultureInfo.InvariantCulture);
        ...
        /// </summary>
        /// <returns>Estrutura de Cliente</returns>	
        public List<string> AtualizarSemObrigatorios(IRLib.ClientObjects.EstruturaCliente cliente)
        {
            int usuarioSite = UsuarioSiteID;
            bool blnAtualizar = true;
            List<string> oMensagens = new List<string>();
        ...
        /// <returns></returns>
        public List<string> AtualizarEntrega(IRLib.ClientObjects.EstruturaCliente cliente)
        {
            int usuarioSite = UsuarioSiteID;
            bool blnAtualizar = true;
            List<string> oMensagens = new List<string>();
        ...
        public bool ValidacaoACSP(EstruturaClienteVendas clienteVendas, bool IsEntrega)
        {
            ...
            if (!IsEntrega)
            {
                ...
                //    retorno = true;
                if (this.CadastroValido().TipoRetorno == IRLib.Codigo.Brainiac.Enumeradores.EnumTipoRetorno.ImplicarErro)
                    return false;
            ...
            else
            {
                ...
                //    retorno = true;
                if (this.CadastroEntregaValido().TipoRetorno == IRLib.Codigo.Brainiac.Enumeradores.EnumTipoRetorno.ImplicarErro)
                    return false;
        ...
        {
            bool retorno = false;
            IRLib.Cliente Cliente = new IRLib.Cliente();
        ...
        public bool AtualizarClienteComNovaSenha(int ID, string senha)
        {
        ...
        public string RetornarEmail(int ID)
        {
            object email = bd.ConsultaValor("SELECT Email FROM tCliente WHERE ID = " + ID);
            ...
            try
            {
                if (ID > 0)
                {
                    using (IDataReader oDataReader = bdEventos.Consulta("" +
                        ...
                        "WHERE " +
                        "	tIngresso.ClienteID = " + ID + " AND tIngresso.Status IN ('V', 'I', 'E')" +
                        "GROUP BY " +
                        "	tIngresso.EventoID, " +
                        "	tEvento.Nome, " +
                        "	tIngresso.VendaBilheteriaID " +
        ...
        public string[] BuscaEmail(int id)
        {
            try
            {
                ...
                if (bd.Consulta(strSql.ToString()).Read())
                {
					...
					this.logCliente(id, this.Control.UsuarioID, 'V');
                }
        ...
        public int[] BuscarClienteAssinatura(string Login, string Senha, IRLib.Utils.Enums.EnumTipoAssinatura tipoAssinatura)
        {
            try
            {
                Login = Login.Replace("'", "''");
        ...
        public IRLib.Assinaturas.Models.Cliente BuscaClienteAssinatura(int ClienteID)
        {
            try
            {
                IRLib.Assinaturas.Models.Cliente ClienteAssinatura = new Assinaturas.Models.Cliente();
        ...
        public EstruturaRetornoProcSalvarCliente SalvarClienteAssinatura(IRLib.Assinaturas.Models.Cliente estruturaCliente)
        {
        ...
        public void EsqueciSenha(string caminho, string identificacao)
        {
            try
            {
                ...
                if (this.Senha.Valor.Length == 0)
                {
                    ...
                    this.Salvar(IRLib.Usuario.INTERNET_USUARIO_ID);
                }

                ServicoEmail.EnviarNovaSenha(this.Nome.Valor, this.Senha.Valor, this.Email.Valor);
            }
            finally
            {
                bd.Fechar();
        ...
        public bool VerificaAssinanteFilarmonica(string CPF)
        {
            try
            {
                bool retorno = false;
                string sql = string.Format("SELECT TOP 1 ID FROM UsuarioFilarmonica usu WHERE usu.CPF = '{0}'", CPF);
        ...
        public DataTable CarregarClientesBusca(string filtro, int totItemPorPagina, bool pesquisaAvancada)
        {
            string query;

            if (pesquisaAvancada)
                query = @"SELECT TOP (@RowspPage) ID, Nome, Email, CPF FROM tCliente (NOLOCK)
                       WHERE CPF = @filtro OR Nome like '%' + @filtro + '%' OR Email like '%' + @filtro + '%'";
            else
                query = @"SELECT TOP (@RowspPage) ID, Nome, Email, CPF FROM tCliente (NOLOCK)
                        WHERE CPF = @filtro OR Nome like @filtro + '%' OR Email like @filtro + '%'";
        ...
        public bool AtualizaCliente()
        {
            try
            {
                ...
                if (this.Control.ID > 0)
                {
                    parametros.Add(new SqlParameter() { DbType = DbType.Int32, ParameterName = "@ID", Value = this.Control.ID });

                    sql = @"UPDATE tcliente SET DataNascimento = @DataNascimento,
                                                Sexo = @Sexo,
                                                Pais = @Pais,
                                                RG = @RG,
                ...
                {
                    sql = @"INSERT INTO tcliente (DataNascimento, Sexo, Pais, RG, CPFResponsavel, Email, CarteiraEstudante, DDDTelefone, 
                            Telefone, DDDCelular, Celular, DDDTelefoneComercial, Obs, RecebeEmail, ContatoTipoID, 
                            CEPCliente, Endereco, NumeroCliente, Complemento, BairroCliente, Estado, Cidade)
                            VALUES (@DataNascimento, @Sexo, @Pais, @RG, @CPFResponsavel, @Email, @CarteiraEstudante, @DDDTelefone,
                            @Telefone, @DDDCelular, @Celular, @DDDTelefoneComercial, @TelefoneComercial, @Obs, @RecebeEmail,
                            ContatoTipoID, @CEPCliente, @Endereco, @NumeroCliente, @Complemento, @BairroCliente, @Estado, @Cidade)";

                }

                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@DataNascimento", Value = _Validacoes.removeCaracteres(this.DataNascimento.Valor.ToShortDateString()) });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Sexo", Value = this.Sexo.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Pais", Value = this.Pais.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@RG", Value = this.RG.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@CPFResponsavel", Value = this.CPFResponsavel.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Email", Value = this.Email.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@CarteiraEstudante", Value = this.CarteiraEstudante.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@DDDTelefone", Value = this.DDDTelefone.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Telefone", Value = this.Telefone.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@DDDCelular", Value = this.DDDCelular.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Celular", Value = this.Celular.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@DDDTelefoneComercial", Value = this.DDDTelefoneComercial.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@TelefoneComercial", Value = this.TelefoneComercial.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Obs", Value = this.Obs.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@RecebeEmail", Value = this.RecebeEmail.Valor ? "T" : "F" });
                parametros.Add(new SqlParameter() { DbType = DbType.Int32, ParameterName = "@ContatoTipoID", Value = this.ContatoTipoID.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@CEPCliente", Value = this.CEPCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Endereco", Value = this.EnderecoCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@NumeroCliente", Value = this.NumeroCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Complemento", Value = this.ComplementoCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@BairroCliente", Value = this.BairroCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Estado", Value = this.EstadoCliente.Valor });
                parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Cidade", Value = this.CidadeCliente.Valor });

                return bd.Executar(sql, parametros) > 0;
        ...
        public DataTable CarregarEmailsCliente(int pageNumber, int pageSize, out int totalRegistros)
        {
            if (this.Control.ID > 0)
            {
                try
                {
                    ...
                    string strQuery = @"SELECT
                                    ...
                                    FROM 
                                        (SELECT TOP @endRow 
                                            ROW_NUMBER() OVER (ORDER BY MSS.DataEnvio DESC) AS RowNumber
                                            ,MSS.ID
                                            ,MSS.Email
	                                        ,MSS.DataEnvio
                                            ,MSS.Assunto
                                            ,CASE WHEN ISNULL(MSS.SendIDOrigem,0) > 0 THEN 'X' ELSE '' END AS Reenvio
                                        FROM 
                                            MailServiceSend MSS (NOLOCK)
                                        WHERE 
                                            MSS.ClienteID = @clienteID
                                        ) AS TMP 
                                    WHERE 
                                        TMP.RowNumber > @startRow".Replace("@endRow", endRow.ToString()).Replace("@startRow", startRow.ToString());
                    #endregion
                    ...
                    parametrosCount.Add(new SqlParameter()
                    {
                        DbType = DbType.Int32,
                    ...
                    bd.Consulta(strCount, parametrosCount);
                    if (bd.Consulta().Read())
                    {
                        totalRegistros = bd.LerInt("totalRegistros");
                    }
                    else
                    {
                        totalRegistros = 0;
                    ...
                    List<SqlParameter> parametrosStr = new List<SqlParameter>();

                    parametrosStr.Add(new SqlParameter()
                    {
                        DbType = DbType.Int32,
                        ParameterName = "@clienteID",
                        Value = this.Control.ID
                    ...
                    return bd.QueryToTable(strQuery, parametrosStr);
                }
                catch (Exception ex)
                {
                    if (ex.ToString().Trim().Contains("O timeout esgotou antes da conclusão da operação ou o servidor não está respondendo."))
    ...
    {
        private Validacoes _Validacoes;
        ...
        public DataTable CarregaCliente(string nome, int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            if (!String.IsNullOrEmpty(nome))
            {
                try
                {
                    ...
                    strBuilder.Append("SELECT ROW_NUMBER() OVER(ORDER BY tCliente.Nome) AS RowNumber, ");
                    strBuilder.Append("tCliente.ID, ");
                    strBuilder.Append("tCliente.Nome, ");
                    strBuilder.Append("tCliente.Email, ");
                    strBuilder.Append("tCliente.CPF ");
                    strBuilder.Append("FROM tCliente (NOLOCK) ");
                    strBuilder.Append("WHERE ");
                    strBuilder.Append(_Validacoes.ValidaCPF(nome) ? "tCliente.CPF = @nome " : "tCliente.Nome LIKE @nome + '%' ");
                    strBuilder.Append("OR tCliente.Email LIKE @nome + '%' ");
                    strBuilder.Append(") AS TBL ");
                    strBuilder.Append("WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage) ");
                    strBuilder.Append("SELECT COUNT(tCliente.ID) as totalRegistros ");
                    strBuilder.Append("FROM tCliente (NOLOCK) ");
                    strBuilder.Append("WHERE ");
                    strBuilder.Append(_Validacoes.ValidaCPF(nome) ? "tCliente.CPF = @nome " : "tCliente.Nome LIKE @nome + '%' ");
                    strBuilder.Append("OR tCliente.Email LIKE @nome + '%' ; ");
                    ...
                    parametros.Add(new SqlParameter()
                    {
                        DbType = DbType.String,
                        ParameterName = "@nome",
                        Value = _Validacoes.ValidaCPF(nome) ? _Validacoes.removeCaracteres(nome) : nome
                    });
        ...
        public DataTable CarregaCliente(int idCliente)
        {
            if (idCliente > 0)
            {
                try
                {
                    ...
                    StringBuilder strBuilder = new StringBuilder();
                    strBuilder.Append("SELECT TOP 1 tCliente.ID, ");
                    strBuilder.Append("Nome, ");
                    strBuilder.Append("RG, ");
                    strBuilder.Append("CPF, ");
                    strBuilder.Append("CPFResponsavel, ");
                    strBuilder.Append("CarteiraEstudante, ");
                    strBuilder.Append("Sexo, ");
                    strBuilder.Append("DDDTelefone, ");
                    strBuilder.Append("Telefone, ");
                    strBuilder.Append("DDDCelular, ");
                    strBuilder.Append("Celular, ");
                    strBuilder.Append("DDDTelefoneComercial, ");
                    strBuilder.Append("TelefoneComercial, ");
                    strBuilder.Append("StatusAtual, ");
                    strBuilder.Append("REPLACE(CONVERT(VARCHAR(11), DataNascimento, 106), ' ','/' ) AS DataNascimento, ");
                    strBuilder.Append("Email, ");
                    strBuilder.Append("RecebeEmail, ");
                    strBuilder.Append("REPLACE(CONVERT(VARCHAR(11), DataCadastro, 106), ' ','/' ) AS DataCadastro, ");
                    strBuilder.Append("CEPCliente AS CEP, ");
                    strBuilder.Append("Endereco, ");
                    strBuilder.Append("NumeroCliente AS Numero, ");
                    strBuilder.Append("ComplementoCliente AS Complemento, ");
                    strBuilder.Append("BairroCliente AS Bairro, ");
                    strBuilder.Append("CidadeCliente AS Cidade, ");
                    strBuilder.Append("EstadoCliente AS Estado, ");
                    strBuilder.Append("CEPEntrega AS CEP, ");
                    strBuilder.Append("ContatoTipoID, ");
                    strBuilder.Append("Pais, ");
                    strBuilder.Append("CNPJ, ");
                    strBuilder.Append("InscricaoEstadual, ");
                    strBuilder.Append("NomeFantasia, ");
                    strBuilder.Append("RazaoSocial, ");
                    strBuilder.Append("Profissao, ");
                    strBuilder.Append("Situacao, ");
                    strBuilder.Append("Obs ");
                    strBuilder.Append("FROM tCliente (NOLOCK)");
                    strBuilder.Append("LEFT JOIN tSituacaoProfissional (NOLOCK) on SituacaoProfissionalID = tSituacaoProfissional.ID ");
                    strBuilder.Append("WHERE tCliente.ID = @idCliente");
        ...
        public DataTable CarregaComprasCliente(int idCliente, int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            if (idCliente > 0)
            {
                try
                {
                    ...
                    string Query = @"SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY tCanal.ID) AS RowNumber, 
                                        ...
                                        tVendaBilheteria.Status AS StatusVenda 
                                        FROM tVendaBilheteria (NOLOCK) 
                                        LEFT JOIN tCaixa (NOLOCK) ON tCaixa.ID = tVendaBilheteria.CaixaID
                                        LEFT JOIN tLoja (NOLOCK) ON tLoja.ID = tCaixa.LojaID
                                        LEFT JOIN tCanal (NOLOCK) ON tCanal.ID = tLoja.CanalID
                                        WHERE tVendaBilheteria.ClienteID = @idCliente AND tVendaBilheteria.Status <> 'C'
                                        ) AS TBL 
                                        WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage) 
                                        SELECT COUNT(tCanal.ID) as totalRegistros 
                                        FROM tVendaBilheteria (NOLOCK) 
                                        LEFT JOIN tCaixa (NOLOCK) ON tCaixa.ID = tVendaBilheteria.CaixaID
                                        LEFT JOIN tLoja (NOLOCK) ON tLoja.ID = tCaixa.LojaID
                                        LEFT JOIN tCanal (NOLOCK) ON tCanal.ID = tLoja.CanalID
        ...
        public DataTable CarregaVersoesCliente(int idCliente, string acao, string data, string usuario, int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            if (idCliente > 0)
            {
                try
                {
                    ...
                    string clausulasExtras = string.Empty;
                    if (!String.IsNullOrEmpty(acao))
                    {
                        clausulasExtras += "AND cCliente.Acao = @Acao ";
                        parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Acao", Value = acao }); 
                    }

                    if (!String.IsNullOrEmpty(data))
                    {
                        clausulasExtras += "AND SUBSTRING(TimeStamp, 0, 9) = @TimeStamp "; 
                        parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@TimeStamp", Value = data }); 
                    }

                    if (!String.IsNullOrEmpty(usuario))
                    {
                        clausulasExtras += "AND tUsuario.Nome LIKE @Nome + '%' ";
                        parametros.Add(new SqlParameter() { DbType = DbType.String, ParameterName = "@Nome", Value = usuario }); 
                    }


                    strBuilder.Append("SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY cCliente.Versao DESC) AS RowNumber, ");
                    strBuilder.Append("cCliente.ID AS IdCliente, ");
                    strBuilder.Append("cCliente.Versao, ");
                    strBuilder.Append("cCliente.Acao, ");
                    strBuilder.Append("[dbo].[StringToDateTime](cCliente.TimeStamp) AS TimeStamp, ");
                    strBuilder.Append("cCliente.UsuarioID AS IdUsuario, ");
                    strBuilder.Append("tUsuario.Nome AS NomeUsuario ");
                    strBuilder.Append("FROM cCliente (NOLOCK) ");
                    strBuilder.Append("LEFT JOIN tUsuario (NOLOCK) on tUsuario.ID = cCliente.UsuarioID ");
                    strBuilder.Append("WHERE cCliente.ID = @IdCliente " + clausulasExtras);
                    strBuilder.Append(") AS TBL ");
                    strBuilder.Append("WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage) ");
                    strBuilder.Append("ORDER BY TBL.RowNumber ");
                    strBuilder.Append("SELECT COUNT(cCliente.ID) as totalRegistros ");
                    strBuilder.Append("FROM cCliente (NOLOCK) ");
                    strBuilder.Append("LEFT JOIN tUsuario (NOLOCK) on tUsuario.ID = cCliente.UsuarioID ");
                    strBuilder.Append("WHERE cCliente.ID = @IdCliente " + clausulasExtras);
        ...
        public DataTable CarregaVersao(int IdCliente, int idVersao)
        {
            try
            {
                StringBuilder strBuilder = new StringBuilder();
                strBuilder.Append("SELECT TOP 1	xCliente.Nome AS NomeCliente, ");
                strBuilder.Append("RG, ");
                strBuilder.Append("CPF, ");
                strBuilder.Append("CPFResponsavel, ");
                strBuilder.Append("CarteiraEstudante, ");
                ...
                strBuilder.Append("TelefoneComercial, ");
                strBuilder.Append("StatusAtual, ");
                strBuilder.Append("DataNascimento, ");
                strBuilder.Append("Email, ");
                strBuilder.Append("RecebeEmail, ");
                strBuilder.Append("DataCadastro, ");
                strBuilder.Append("CEPCliente, ");
                strBuilder.Append("Endereco, ");
                strBuilder.Append("NumeroCliente, ");
                strBuilder.Append("ComplementoCliente, ");
                strBuilder.Append("BairroCliente, ");
                strBuilder.Append("CidadeCliente, ");
                strBuilder.Append("EstadoCliente, ");
                strBuilder.Append("CEPEntrega, ");
                strBuilder.Append("ContatoTipoID, ");
                strBuilder.Append("Pais, ");
                strBuilder.Append("CNPJ, ");
                strBuilder.Append("InscricaoEstadual, ");
                ...
                strBuilder.Append("Situacao, ");
                strBuilder.Append("Obs, ");
                strBuilder.Append("tContatoTipo.Nome AS Contato, ");
                strBuilder.Append("Versao ");
                strBuilder.Append("FROM xCliente (NOLOCK) ");
                strBuilder.Append("LEFT JOIN tSituacaoProfissional (NOLOCK) ON SituacaoProfissionalID = tSituacaoProfissional.ID ");
                strBuilder.Append("LEFT JOIN tContatoTipo (NOLOCK) ON xCliente.ContatoTipoID = tContatoTipo.ID ");
                strBuilder.Append("WHERE xCliente.ID = @IdCliente ");
                strBuilder.Append("AND xCliente.Versao  = (SELECT MAX(Versao) FROM cCliente (NOLOCK) WHERE ID = @IdCliente AND Versao <= @IdVersao AND Acao <> 'v')");
