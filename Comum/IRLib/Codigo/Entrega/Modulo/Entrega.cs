...
namespace IRLib
{
    ...
    public class Entrega : Entrega_B
    {
            ...
            try
            {
                ...
                string sql =
                    string.Format(@"SELECT DISTINCT e.ID AS EntregaID, e.Nome, e.ProcedimentoEntrega, ec.Valor
                                ...
                                left JOIN tEntregaArea ea (NOLOCK) ON ea.ID = ec.EntregaAreaID
                                left JOIN tEntregaAreaCep eac (NOLOCK) ON eac.EntregaAreaID = ea.ID
                                WHERE ec.EntregaID in ({0}) 
                                  AND (eac.CepInicial is null or eac.CepFinal is null or ( eac.CepInicial <= '{1} ' AND eac.CepFinal >= '{1}'))
                                  AND ec.EntregaID NOT IN ({2})
                                  AND ec.Ativa	= 'T' 
        ...
        public static List<int> EticketPendente()
        {
            var retorno = new List<int>();

            using (BD bd = new BD())
            {
                while (bd.Consulta("EXEC spu_EticketPendente").Read())
