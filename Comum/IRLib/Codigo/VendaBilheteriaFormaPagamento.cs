    ...
    public class VendaBilheteriaFormaPagamento : VendaBilheteriaFormaPagamento_B
    {
        ...
        public DataTable ConciliacaoBancaria(string empresaID, DateTime dataInicial, DateTime dataFinal)
        {
            try
            {
                ...
                bd.Executar(sql);
                DataRow linha;
                IRLib.ClientObjects.EstruturaConciliacaoBancaria atual = new IRLib.ClientObjects.EstruturaConciliacaoBancaria();
                IRLib.ClientObjects.EstruturaConciliacaoBancaria anterior = new IRLib.ClientObjects.EstruturaConciliacaoBancaria();
        ...
        public string StringInserir()
        {
            try
            {
                ...
                sql.Replace("@031", this.VendaBilheteriaFormaPagamentoIdOrigem.ValorBD);
        ...
        public void LerPorVendaBilheteriaID(int id)
        {
            LerPorVendaBilheteriaID(bd, id);
            bd.Fechar();
        }
        public void LerPorVendaBilheteriaID(BD bd, int id)
        {

            try
            {

                string sql = "SELECT * FROM tVendaBilheteriaFormaPagamento WHERE VendaBilheteriaID = " + id;
                bd.Consulta(sql);
                ...
                if (bd.Consulta().Read())
                {
                    ...
                    this.VendaBilheteriaFormaPagamentoIdOrigem.ValorBD = bd.LerInt("VendaBilheteriaFormaPagamentoIDOrigem").ToString();
                }
                else
                {
                    this.Limpar();
