...
namespace IRLib
{
    ...
    public class Usuario : Usuario_B
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable TodosComPaginacao(string filtro, int regionalID, int empresaID, int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            string filtroIDs = "";

            if (regionalID != 0)
                filtroIDs = "AND (tEmpresa.RegionalID = @regionalID)";

            if (empresaID != 0)
                filtroIDs = "AND (EmpresaID = @empresaID)";

            if (filtro != String.Empty)
                filtroIDs = "AND (tUsuario.Nome LIKE '%' + @filtro + '%' OR tUsuario.Email  LIKE '%' + @filtro + '%' OR tUsuario.Login LIKE '%' + @filtro + '%')";

            string query = @"SELECT * FROM (
                                     SELECT ROW_NUMBER() OVER(ORDER BY tUsuario.ID) AS RowNumber,
                                                                tUsuario.ID,
                                                        ...
                                                        INNER JOIN tEmpresa (NOLOCK) ON tUsuario.EmpresaID = tEmpresa.ID
                                                        WHERE 1=1 " + filtroIDs + @"
                                       ) AS TBL
                                        WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage)
                                        ORDER BY ID;
                                        SELECT COUNT(tUsuario.ID) as totalRegistros FROM tUsuario (NOLOCK)
                                                        INNER JOIN tEmpresa (NOLOCK) ON tUsuario.EmpresaID = tEmpresa.ID
                                                        WHERE 1=1 " + filtroIDs;
            ...
            foreach (DataRow linha in dtResult.Rows)
            {
                string status = "";
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable TodosComEmpresaAtivaPaginacao(string filtro, int regionalID, int empresaID, int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            string filtroIDs = "";
            ...
            string query = @"SELECT * FROM (
                                     SELECT ROW_NUMBER() OVER(ORDER BY tUsuario.ID) AS RowNumber,
														...
														FROM tUsuario (NOLOCK)
                                                        INNER JOIN tEmpresa (NOLOCK) ON tUsuario.EmpresaID = tEmpresa.ID
                                                        WHERE tEmpresa.Ativo = 'T' " + filtroIDs + @"
                                       ) AS TBL
                                        WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage)
                                        ORDER BY ID;
                                        SELECT COUNT(tUsuario.ID) as totalRegistros FROM tUsuario (NOLOCK)
        ...
        public override bool Atualizar()
        {
            try
            {
                ...
                if (!ok)
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append("UPDATE tUsuario SET EmpresaID = @001, Nome = '@002', Sexo = '@003', Email = '@004', Login = '@005', Status = '@007', Validade = '@008', ValidoDe = '@009', ValidoAte = '@010', CodigoTerminal = '@011' @012 ");
                    ...
                    if (this.Senha.ValorBD.Length > 0)
                    {
                        SenhaCriptografada = new senhacriptografada(this.Senha.Valor);
                        sql.Replace("@012", ", senha = '" + this.SenhaCriptografada.Valor + "'");
                    }
                    else
                        sql.Replace("@012", "");
        ...
        public DataTable PerfilSacEspecial()
        {
            try
            {
                ...
                string sql = @"SELECT p.Nome
                            ...
                            WHERE p.ID = " + Perfil.SAC_SUPERVISOR + " AND pc.UsuarioID = " + this.Control.ID;
                ...
                {
                    DataRow linha = tabela.NewRow();
                    linha["ID"] = Perfil.SAC_SUPERVISOR;
        ...
        public Usuario LogarAssinatura(string usuario, string senha, int canalID, int assinaturaTipoID)
        {
            try
            {
                ...
                string sql = string.Format(@"SELECT u.ID, u.Login, u.Nome, p.ID AS PerfilID
                        ...
                        usuario, senhaC, canalID, Liberado, Temporario, DateTime.Now.ToString("yyyyMMdd"), Perfil.CANAL_BILHETEIRO);
        ...
        public bool ValidarSupervisorSac(int usuarioID)
        {
            try
            {
                string query = "select p.UsuarioID from tPerfilEspecial p where p.UsuarioID = {0} and p.PerfilID in ({1}, {2})";
                string sql = string.Format(query, usuarioID, Perfil.SAC_SUPERVISOR, Perfil.SAC_SUPERVISOR_NOVO);
        ...
        public static bool UsuarioValido(int id)
        {
            if (id < 1)
                return false;
            ...
            using (BD bd = new BD())
            {
                var reader = bd.Consulta("SELECT ID, Status FROM tUsuario(NOLOCK) WHERE ID = @UsuarioID", new List<SqlParameter> { new SqlParameter("@UsuarioID", id) });
                if (!reader.Read())
                    return false;
                else
                {
                    return Utils.Enums.ParseCharEnum<Usuario.TipoStatus>(bd.LerString("Status")) == Usuario.TipoStatus.Liberado;
                }
            }
        }
    } // fim da classe
