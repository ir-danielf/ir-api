...
namespace IRLib.Assinaturas.Relatorios
{
    public class BorderoAvulso
    {
        ...
        public IRLib.Assinaturas.Relatorios.ModelBorderoAvulso BuscarRelatorio(int Eventos, int Apresentacoes, bool SemCortesia)
        {
            try
            {
                ...
                string sql = "";
                string sqlCondicao = "";
                string sqlContinuacao = "";

                retorno.Assinatura = infos[1];
                retorno.Local = infos[0];

                sql = string.Format(@"SELECT s.Nome AS Setor, p.Nome AS Preco, 
                ...
                WHERE i.ApresentacaoID ={0}  And i.EventoID ={1} AND 
                (i.Status = 'V' OR i.Status= 'I' OR i.Status = 'E') 
                AND i.AssinaturaClienteID = 0
                AND i.CortesiaID = 0
                GROUP BY s.ID, s.Nome, p.Nome
                ORDER BY s.Nome, p.Nome ", Apresentacoes, Eventos);

                if (SemCortesia == false)
                {
                    sqlCondicao = string.Format(@" SELECT
                ...
                sqlContinuacao = string.Format(@" SELECT
                s.Nome AS Setor,
                ...
                FROM tIngresso i (NOLOCK)
                INNER JOIN tSetor s (NOLOCK) ON s.ID = i.SetorID
                WHERE i.ApresentacaoID ={0}  And i.EventoID ={1} GROUP BY s.ID,s.Nome ORDER BY s.ID,s.Nome ", Apresentacoes, Eventos);


                string sqlConsolidado = sql + sqlCondicao + sqlContinuacao;

                bd.Consulta(sqlConsolidado);
