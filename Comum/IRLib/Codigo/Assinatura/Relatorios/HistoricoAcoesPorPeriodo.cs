...
namespace IRLib.Assinaturas.Relatorios
{
    ...
    {
        public List<HistoricoPorPeriodo> FotoAtual(int assinaturaTipoID, string ano = null)
        {
            BD bd = new BD();
            try
            {
                ...
                {
                    ano = (DateTime.Now.Year + 1).ToString();
                }

                bd.Executar("EXEC historicoAcoesPorPeriodo " + assinaturaTipoID + ", '" + oAssinaturaTipo.RenovacaoInicio.Valor.ToString("yyyyMMddHHmmss") + "', '" + oAssinaturaTipo.RenovacaoFim.Valor.AddDays(1).ToString("yyyyMMddHHmmss") + "', '" + ano + "'");
                if (bd.Consulta().Read())
