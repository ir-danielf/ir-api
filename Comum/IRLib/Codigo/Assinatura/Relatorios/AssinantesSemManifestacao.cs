...
namespace IRLib.Assinaturas.Relatorios
{
    public class AssinantesSemManifestacao
    {
        ...
        public List<IRLib.Assinaturas.Models.AssinantesSemManifestacao> BuscarRelatorio(int AssinaturaTipoID, int Temporadas, int Assinaturas, AssinaturaCliente.EnumStatus filtroStatus)
        {
            try
            {
                ...
                switch (filtroStatus)
                {
                    case AssinaturaCliente.EnumStatus.Aguardando:
                        filtro += " AND ac.Status = '" + IRLib.Utils.Enums.GetChar(filtroStatus) + "' AND ac.Acao = '" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumAcao.AguardandoAcao) + "' ";
                        break;
                    case AssinaturaCliente.EnumStatus.TrocaSinalizada:
                        filtro += " AND ac.Status = '" + IRLib.Utils.Enums.GetChar(filtroStatus) + "' AND ac.Acao = '" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumAcao.Trocar) + "' ";
                        break;
                    default:
                        filtro += " AND ac.Status IN ('" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumStatus.Aguardando) + "', '" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumStatus.TrocaSinalizada) + "') AND ac.Acao IN ('" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumAcao.Trocar) + "', '" + IRLib.Utils.Enums.GetChar(AssinaturaCliente.EnumAcao.AguardandoAcao) + "') ";
                        break;
                }
