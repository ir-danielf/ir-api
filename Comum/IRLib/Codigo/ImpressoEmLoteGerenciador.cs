...
namespace IRLib
{
    ...
    public class ImpressoEmLoteGerenciador : MarshalByRefObject
    {
        ...
        public const string AGREGADONOME = "AgregadoNome";
        public const string AGREGADOCPF = "AgregadoCPF";

        public const string MENOR_APRESENTACAO = "MenorApresentacao";

        public const string APRESENTACAO_CANCELADA = "ApresentacaoCancelada";

        private bool SoImprimirDataApresentacao = new ConfigGerenciador().ImprimirSomenteDataApresentacao();
        //BD bd;
        ...
        public ImpressoEmLoteGerenciador()
        {
            //	bd = new BD();
        }
        ...
        private DataTable estruturaGrid()
        {
            ...
            grid.Columns.Add(LOCAL, typeof(string));
            ...
            grid.Columns.Add(AGREGADONOME, typeof(string));
            grid.Columns.Add(AGREGADOCPF, typeof(string));
            grid.Columns.Add(APRESENTACAO_CANCELADA, typeof(string));
            return grid;
        ...
        public DataTable ImpressaoVenda(int vendaID, int lojaID)
        {
            ...
            try
            {
                ...
                {
                    if (bd.LerInt("VendaBilheteriaID") == vendaID && bd.LerString("Status") == "V")
                    {
        ...
        public DataTable Buscar(EstruturaBuscaImpressaoEmLote busca)
        {
            ...
            try
            {
                ...
                string ParceiroMidiaID = busca.ParceiroMidiaID > 0 ?
                    string.Format(" AND i.ParceiroMidiaID = '{0}'", busca.ParceiroMidiaID) : string.Empty;
                ...
                string selectQuery =
                            @"SELECT * FROM (
                                SELECT 
                                    ...
                                    te.Tipo AS TipoTaxa, 
                                    l.Nome AS Loja, 
                                    vb.ID AS VendaBilheteriaID, 
                                    vb.Senha,
                                    vb.DataVenda, 
                                    vb.Status, 
                                    vb.AgregadoNome, 
                                    vb.AgregadoCPF, 
                                    cn.ComprovanteQuantidade, 
                                    cn.TipoVenda, 
                                    Count(i.ID) AS Qtd,
                                    CASE 
                                        WHEN LEN(c.CNPJ) > 0 
                                    ...
                                    END AS Cliente,
                                    c.StatusAtual AS ClienteStatus, 
                                    MIN(ap.Horario) AS MenorApresentacao,
                                    te.DiasTriagem AS EntregaTriagem,
                                    tc.DiasTriagem AS ControleTriagem, 
                                    c.RG AS ClienteRG,
                                    CASE
                                        WHEN LEN(c.CNPJ) > 0 
                                        THEN c.CNPJ 
                                        ELSE c.CPF 
                                            COLLATE Latin1_General_CI_AI 
                                    END AS ClienteCPF, 
                                    c.CEPCliente AS ClienteCEP, 
                                    c.EnderecoCliente AS ClienteEndereco, 
                                    c.NumeroCliente AS ClienteNumero, 
                                    c.ComplementoCliente AS ClienteComplemento, 
                                    c.BairroCliente AS ClienteBairro, 
                                    c.CidadeCliente AS ClienteCidade, 
                                    c.EstadoCliente AS ClienteEstado, 
                                    IsNull(ec.ID, 0) AS ClienteEnderecoID, 
                                    ec.Nome AS NomeEntrega, 
                                    ec.RG AS RGEntrega, 
                                    ec.Endereco AS EnderecoEntrega, 
                                    ec.Numero AS NumeroEntrega, 
                                    ec.Complemento AS ComplementoEntrega, 
                                    ec.Bairro AS BairroEntrega, 
                                    ec.Cidade AS CidadeEntrega, 
                                    ec.Estado AS EstadoEntrega, 
                                    ec.CEP AS CEPEntrega, 
                                    te.Nome AS EntregaNome, 
                                    ep.Nome AS PeriodoEntrega, 
                                    ea.Data AS DataEntrega, 
                                    ear.Nome AS AreaEntrega, 
                                    vb.CodigoRastreio AS CodigoRastreioCorreio,
                                    ap.Cancelada as ApresentacaoCancelada,
                                ...
                                FROM tVendaBilheteria vb (NOLOCK) 
                                    ...
                                    INNER JOIN tApresentacao ap (NOLOCK) ON i.ApresentacaoID = ap.ID and ap.Cancelada = 'F' 
                                    ...
                                    INNER JOIN tLocal lc (NOLOCK) ON i.LocalID = lc.ID ";
                ...
                ConfigGerenciador Config = new ConfigGerenciador();
                ...
                if (!string.IsNullOrEmpty(ParceiroMidiaID))
                    selectQuery += ParceiroMidiaID;
                ...
                e.Nome, lc.Nome, te.Nome, te.Tipo, l.Nome, 
                vb.ID, vb.Senha, vb.DataVenda, vb.Status, vb.AgregadoNome, vb.AgregadoCPF, c.Nome, 
                ...
                c.ComplementoCliente, c.BairroCliente, c.CidadeCliente, c.EstadoCliente, vb.CodigoRastreio, ap.Cancelada";
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha[AGREGADONOME] = bd.LerString(AGREGADONOME);
                    linha[AGREGADOCPF] = bd.LerString(AGREGADOCPF);
                    linha[APRESENTACAO_CANCELADA] = bd.LerString(APRESENTACAO_CANCELADA);
                    ...
                    string cepInicial, string cepFinal)
        {
            ...
            try
            {
                ...
                stbSQL.Append("vb.AgregadoNome, vb.AgregadoCPF ");
        ...
        public BindingList<EstruturaRetornoVendaValeIngresso> BuscarValeIngresso(EstruturaBuscaImpressaoEmLote busca)
        {
            ...
            try
            {
                ...
                stb.Append("    vb.AgregadoNome, vb.AgregadoCPF, ");
                ...
                stb.Append(" GROUP BY ");
                stb.Append("    te.Nome, l.Nome, vb.AgregadoNome, vb.AgregadoCPF, ");
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    item = new EstruturaRetornoVendaValeIngresso()
                    {
                        ...
                        AgregadoNome = bd.LerString(AGREGADONOME),
                        AgregadoCPF = bd.LerString(AGREGADOCPF),
