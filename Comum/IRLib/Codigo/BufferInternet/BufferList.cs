...
namespace IRLib
{
    ...
    public class BufferList<T> : List<T> where T : aSync<T>
    {
        ...
        private void SaveLog()
        {
            ...
            {
                string prefixo = "";
                if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["EventLogPrefixo"]))
                    prefixo = ConfigurationManager.AppSettings["EventLogPrefixo"].ToString() + " - ";

                if (!System.Diagnostics.EventLog.SourceExists(prefixo + "Robot Johnny Five"))
                    System.Diagnostics.EventLog.CreateEventSource(prefixo + "Robot Johnny Five", prefixo + "Atualização SiteIR");

                EventLog log = new EventLog(prefixo + "Robot Johnny Five");
                log.Source = prefixo + "Robot Johnny Five";
                log.Log = prefixo + "Atualização SiteIR";
                if (this.Error)
