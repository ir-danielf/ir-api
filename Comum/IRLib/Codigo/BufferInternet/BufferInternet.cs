...
namespace IRLib
{
    ...
    public partial class BufferInternet
    {
        ...
        public BufferList<LoteStatus> lstLoteStatus { get; set; }
        ...
        public void Carregar()
        {
            ...
            try
            {
                ...
                bd.Consulta("SELECT DISTINCT " +
                    ...
                    "WHERE (Publicar = '" + Convert.ToChar(IRLib.Evento.PublicarTipo.PublicadoParaVenda).ToString() + "' OR Publicar = '" + Convert.ToChar(IRLib.Evento.PublicarTipo.PublicadoSemVenda).ToString() + "') " +
                    "ORDER BY EventoID");

                var now = Convert.ToInt64(DateTime.Now.ToString("yyyyMMddHHmmss", CultureInfo.InvariantCulture));

                while (bd.Consulta().Read())
                {
                    var eventoToADD = new Evento
                    {
                        ID = bd.LerInt("ID"),
                        Nome = Utilitario.LimparTitulo(bd.LerString("Nome")),
                    ...
                    if ((!string.IsNullOrEmpty(eventoToADD.DataAberturaVenda)) && Convert.ToInt64(eventoToADD.DataAberturaVenda) < now && eventoToADD.PublicarSemVendaMotivo == 6)
                    {
                        eventoToADD.Publicar = "T";
                        eventoToADD.PublicarSemVendaMotivo = 0;

                        IRLib.Evento e = new IRLib.Evento();
                        e.Ler(eventoToADD.ID);
                        e.PublicarSemVendaMotivo.Valor = eventoToADD.PublicarSemVendaMotivo;
                        e.Publicar.Valor = eventoToADD.Publicar;
                        e.Atualizar();
                    }

                    lstEvento.Add(eventoToADD);
                }
                bd.FecharConsulta();
                ...
                //PontoVenda
                bd.Consulta(@"SELECT ID, Local, Nome, Endereco, Numero, Compl, Cidade, Estado, Bairro, Obs, Referencia, CEP, PermiteRetirada FROM tPontoVenda (NOLOCK)");
                while (bd.Consulta().Read())
                {
                    lstPontoVenda.Add(new PontoVenda
                    {
                        ...
                        CEP = bd.LerString("CEP"),
                        PermiteRetirada = bd.LerString("PermiteRetirada")
                    });
                }
                bd.FecharConsulta();
        ...
        {
            if (this.lstLoteStatus != null)
                this.lstLoteStatus.Clear();
            else
                this.lstLoteStatus = new BufferList<LoteStatus>();
            BD bd = new BD();
            try
            {
                #region Query
                if (ConfigurationManager.AppSettings["LimiarExibicaoLotes"] == null)
                    throw new Exception("A propriedade LimiarExibicaoLotes não existe no App.Config ou não foi configurada corretamente.");
                List<SqlParameter> parameters = new List<SqlParameter>();
                parameters.Add(new SqlParameter("@Limiar", Convert.ToInt32(ConfigurationManager.AppSettings["LimiarExibicaoLotes"])));
                bd.Consulta(@"EXEC Proc_LotesAtualizar @Limiar", parameters);
                #endregion
                while (bd.Consulta().Read())
                {
                    lstLoteStatus.Add(new LoteStatus
                    {
                        ID = bd.LerInt("ID"),
                        DataLimite = (String.IsNullOrEmpty(bd.LerString("DataLimite"))) ? DateTime.MinValue : DateTime.Parse(bd.LerString("DataLimite")),
                        Quantidade = bd.LerInt("Quantidade"),
                        Vendidos = bd.LerInt("Vendidos"),
                        LoteSeguinte = bd.LerInt("LoteSeguinte"),
                        LoteSeguinteStatus = bd.LerString("LoteSeguinteStatus")
                    });
                }
                bd.FecharConsulta();
            }
        ...
        public void CarregarSite()
        {
            ...
            try
            {
                ...
                dr = oDAL.SelectToIDataReader("SELECT IR_PontoVendaID AS ID, Local, Nome, Endereco, Numero, Compl, Cidade, Estado, Bairro, Obs, Referencia, CEP, PermiteRetirada FROM PontoVenda (NOLOCK) ");
                while (dr.Read())
                {
                    lstPontoVenda.lstSite.Add(new PontoVenda
                    {
                        ...
                        CEP = Convert.ToString(dr["CEP"]),
                        PermiteRetirada = Convert.ToString(dr["PermiteRetirada"])
                    });
                }
                dr.Close();
