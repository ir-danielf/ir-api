...
namespace IRLib.CancelamentoIngresso
{
    ...
    public class CancelamentoGerenciador : MarshalByRefObject, ISponsoredObject
    {
        ...
        public const string ENTREGACANCELADA = "EntregaCancelada";
        public const string CONVENIENCIACANCELADA = "ConvenienciaCancelada";
        public const string SEGUROCANCELADO = "SeguroCancelado";
        ...
        public const string CANAL_ID = "CanalID";
        ...
        public const string CARTAO_ID = "CartaoID";
        public const string FORMA_PAGAMENTO = "FormaPagamentoTipoID";
        public const string PERMITIR_IMPRESSAO_INTERNET = "PermitirImpressaoInternet";
        public const string PENDENTE_CANCELAMENTO = "PendenteCancelamento";
        public const string VALOR_ESTORNADO = "ValorEstornado";
        public const string CONVENIENCIA_ESTORNADA = "ConvenienciaEstornada";
        public const string PAGAMENTO_TROCA = "PagamentoTroca";
        ...
        { //auxiliar
            ...
            tabela.Columns.Add(PENDENTE_CANCELAMENTO, typeof(int));
            tabela.Columns.Add(VALOR_ESTORNADO, typeof(int));
            tabela.Columns.Add(CONVENIENCIA_ESTORNADA, typeof(int));
            return tabela;
        }
        ...
        public static DataSet EstruturaGrid()
        {
            ...
            tGrid.Columns.Add(PENDENTE_CANCELAMENTO, typeof(int));
            tGrid.Columns.Add(VALOR_ESTORNADO, typeof(int));
            tGrid.Columns.Add(CONVENIENCIA_ESTORNADA, typeof(int));
            ...
            tReserva.Columns.Add(VALOR_CONV, typeof(decimal)).DefaultValue = 0;
        ...
        public static DataTable EstruturaInfoVenda()
        {
            ...
            tGrid.Columns.Add(CANAL_ID, typeof(string));
            ...
            tGrid.Columns.Add(VENDACANCELADA, typeof(bool)).DefaultValue = false;
            tGrid.Columns.Add(TAXAPROCESSAMENTOCANCELADA, typeof(bool)).DefaultValue = false;

            tGrid.Columns.Add(ENTREGACANCELADA, typeof(bool)).DefaultValue = false;
            tGrid.Columns.Add(CONVENIENCIACANCELADA, typeof(bool)).DefaultValue = false;
            tGrid.Columns.Add(SEGUROCANCELADO, typeof(bool)).DefaultValue = false;
            ...
            tGrid.Columns.Add(CARTAO_ID, typeof(int)).DefaultValue = 0;
            tGrid.Columns.Add(FORMA_PAGAMENTO, typeof(int)).DefaultValue = 0;
            tGrid.Columns.Add(PERMITIR_IMPRESSAO_INTERNET, typeof(bool)).DefaultValue = false;

            tGrid.Columns.Add(PAGAMENTO_TROCA, typeof(int)).DefaultValue = 0;
            return tGrid;
        }
        ...
        private string MontaSQLPesquisa(string strCriterio)
        {
            ...
            sb.Append("c.Nome AS Cliente, vb.ClienteID , c.Email, tCanal.ID AS CanalID, tCanal.Nome AS Canal, u.Nome AS Vendedor, tCaixa.DataAbertura AS DataCaixa, vb.NivelRisco, ");
            sb.Append("IsNull(e.Tipo, '') AS TaxaEntregaTipo , c.CNPJ , c.NomeFantasia, IsNull(vb.PagamentoProcessado, 'T') AS PagamentoProcessado, TaxaProcessamentoValor, ");
            sb.Append("TaxaProcessamentoCancelada, EntregaCancelada, ConvenienciaCancelada, SeguroCancelado, ISNULL(vb.ValorSeguro, 0) AS ValorSeguro, ISNULL(TransactionID,'') as TransactionID,  ");
            sb.Append("vb.VendaCancelada, vbfp.CartaoID, fp.FormaPagamentoTipoID, e.PermitirImpressaoInternet,  ISNULL(vbfp.VendaBilheteriaFormaPagamentoIDOrigem, 0) as PagamentoTroca ");
            ...
            if (perfilID == Perfil.LOCAL_SUPERVISOR || perfilID == Perfil.SEGURANCA_ESPECIAL)
            ...
            sb.Append("LEFT JOIN tFormaPagamento fp ( NOLOCK ) ON fp.ID = vbfp.FormaPagamentoID ");
        ...
        private string MontaSQLIngressos(string strCriterio)
        {
            ...
            sb.Append("ISNULL(( SELECT max(cdpi.ID) ");
            sb.Append("FROM   tCancelDevolucaoPendenteIngresso AS cdpi ( NOLOCK ) ");
            sb.Append("INNER JOIN tCancelDevolucaoPendente AS cdp ( NOLOCK ) ON cdp.ID = cdpi.CancelDevolucaoPendenteID ");
            sb.Append(" WHERE  cdpi.IngressoID = LOG.IngressoID ");
            sb.Append("AND cdp.StatusCancel = 'P' ");
            sb.Append("), 0) AS PendenteCancelamento, ");

            sb.Append("case when (SELECT max(lm.id) ");
            sb.Append("FROM tIngressoLog lm ");
            sb.Append("where lm.acao = 'S' and lm.ingressoid = LOG.IngressoID) > ");
            sb.Append("(SELECT max(lm.id) ");
            sb.Append("FROM tIngressoLog lm ");
            sb.Append("where lm.acao = 'V' and lm.ingressoid = LOG.IngressoID) then  ");
            sb.Append("1 ");
            sb.Append("else 0  ");
            sb.Append("end as ValorEstornado, ");

            sb.Append("CASE WHEN ( ");
            sb.Append("select top 1 vbc.ConvenienciaCancelada from tVendaBilheteria vbc ");
            sb.Append("inner join  tCancelDevolucaoPendente cp on cp.VendaBilheteriaIDCancel = vbc.id and cp.StatusCancel in ('A', 'D', 'P') ");
            sb.Append("inner join  tCancelDevolucaoPendenteIngresso  ci on cp.id = ci.CancelDevolucaoPendenteid ");
            sb.Append("where cp.VendaBilheteriaIDVenda = tVendaBilheteria.ID ");
            sb.Append("and ci.IngressoID = LOG.IngressoID and vbc.ConvenienciaCancelada = 'T') = 'T' then 1 ");
            sb.Append("else 0 ");
            sb.Append("end as ConvenienciaEstornada, ");

            sb.Append("CASE WHEN (SELECT top 1 vbc.ConvenienciaCancelada from tVendaBilheteria vbc ");
            sb.Append("inner join tCancelDevolucaoPendente cp on cp.VendaBilheteriaIDCancel = vbc.id and cp.StatusCancel in ('A', 'D', 'P') ");
            sb.Append("inner join tCancelDevolucaoPendenteIngresso  ci on cp.id = ci.CancelDevolucaoPendenteid ");
            sb.Append("WHERE cp.VendaBilheteriaIDVenda = tVendaBilheteria.ID ");
            sb.Append("and ci.IngressoID = LOG.IngressoID and vbc.ConvenienciaCancelada = 'T' ");
            sb.Append(") = 'T' THEN 0 ");
            sb.Append("ELSE tVendaBilheteriaItem.TaxaConvenienciaValor ");
            sb.Append("END AS TaxaConvenienciaValor ");
        ...
        public DataSet PesquisarCodigoBarras(string codigoBarra, bool vendaremota)
        {
            try
            {
                ...
                switch (perfilID)
                {
                    ...
                    case Perfil.SAC_SUPERVISOR_NOVO:
                        sql = MontaSQLPesquisa("" +
                            "AND (vb.ID = " + vendaID + ") " + (vendaremota ? " AND tCanal.TipoVenda = 'F'" : string.Empty) +
                            "");
                        break;
                    default:
                ...
                if (bd.Consulta().Read())
                {
                    ...
                    linha[VENDACANCELADA] = bd.LerBoolean(VENDACANCELADA);
                    linha[TAXAPROCESSAMENTOCANCELADA] = bd.LerBoolean(TAXAPROCESSAMENTOCANCELADA);

                    linha[ENTREGACANCELADA] = bd.LerBoolean(ENTREGACANCELADA);
                    linha[CONVENIENCIACANCELADA] = bd.LerBoolean(CONVENIENCIACANCELADA);
                    linha[SEGUROCANCELADO] = bd.LerBoolean(SEGUROCANCELADO);
                    ...
                    linha[CARTAO_ID] = bd.LerInt(CARTAO_ID);
                    linha[FORMA_PAGAMENTO] = bd.LerInt(FORMA_PAGAMENTO);
                    linha[PERMITIR_IMPRESSAO_INTERNET] = bd.LerBoolean(PERMITIR_IMPRESSAO_INTERNET);
                ...
                foreach (DataRow linha in tabelaTmp.Rows)
                {
                    ...
                    for (int i = 0; i < linhas.Length; i++)
                    {
                        ...
                        novoItemReserva[INGRESSOID] = (int)linhas[i][INGRESSOID];
                        ...
                        novoItemReserva[VALOR_CONV] = linhas[i][VALOR_CONV];
        ...
        public DataSet PesquisarCodigoIngresso(int apresentacaoSetorID, string codigo, bool vendaremota)
        {
            try
            {
                DataSet retorno = EstruturaGrid();
                ...
                while (bd.Consulta().Read())
                {
                    if (bd.LerInt("VendaBilheteriaID") == vendaID)
                    {
                        ...
                        linha[EVENTO_ID] = bd.LerInt(EVENTO_ID);
                        linha[APRESENTACAO_SETOR_ID] = bd.LerInt(APRESENTACAO_SETOR_ID);
                        linha[PENDENTE_CANCELAMENTO] = bd.LerInt(PENDENTE_CANCELAMENTO);
                        linha[VALOR_ESTORNADO] = bd.LerInt(VALOR_ESTORNADO);
                        linha[CONVENIENCIA_ESTORNADA] = bd.LerInt(CONVENIENCIA_ESTORNADA);
                        tabela.Rows.Add(linha);
                    }
                }
        ...
        public DataSet PesquisarSenha(string senha, bool vendaremota)
        {
            try
            {
                DataSet retorno = EstruturaGrid();
                ...
                switch (perfilID)
                {
                    ...
                    case Perfil.SAC_SUPERVISOR_NOVO:
                        sql = MontaSQLPesquisa("" +
                            "AND (vb.Senha = '" + senha + "') " + (vendaremota ? " AND tCanal.TipoVenda = 'F'" : string.Empty) +
                            "");
                        break;
                    default:
                        throw new CancelamentoGerenciadorException("Perfil nulo ou não permitido.");
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha[EMAIL] = bd.LerString("Email");
                    linha[CANAL] = bd.LerString("Canal");
                    linha[CANAL_ID] = bd.LerString("CanalID");
                    linha[VENDEDOR] = bd.LerString("Vendedor");
                    linha[SENHA] = bd.LerString("Senha");
                    linha[DATA_VENDA] = bd.LerDateTime("DataVenda");
                    linha[DATA_CAIXA] = bd.LerDateTime("DataCaixa");
                    ...
                    linha[PAGAMENTO_TROCA] = bd.LerInt(PAGAMENTO_TROCA);
                ...
                if (vendaID != 0)
                {
                    ...
                    foreach (DataRow linha in tabelaTmp.Rows)
                    {
                        ...
                        novoItemGrid[PENDENTE_CANCELAMENTO] = (int)linhas[0][PENDENTE_CANCELAMENTO];
                        novoItemGrid[VALOR_ESTORNADO] = (int)linhas[0][VALOR_ESTORNADO];
                        novoItemGrid[CONVENIENCIA_ESTORNADA] = (int)linhas[0][CONVENIENCIA_ESTORNADA];
        ...
        public DataSet PesquisarVenda(int vendaID, bool vendaremota)
        {
            try
            {
                DataSet retorno = EstruturaGrid();
        ...
        public string PesquisarVendaeCancelar(int vendaID, int motivoId, int SupervisorID)
        {
            BD bd = new BD();

            try
            {
                DataSet retorno = PesquisarVenda(vendaID, true);
                ...
                DataTable tGridInfoAssinatura = retorno.Tables[CancelamentoGerenciador.TABELA_INFO_ASSINATURA];
                ...
                if (disponivelAjuste.Length == 0)
                {
                    if (tGrid.Rows.Count > 0)
                    {
                        ...
                        List<int> ingressosID = new List<int>();
                        foreach (DataRow linha in tReserva.Rows)
                            ingressosID.Add(Convert.ToInt32(linha["IngressoID"]));

                        int vendaBilheteriaItemID = (int)tGrid.Rows[0][CancelamentoGerenciador.VENDA_BILHETERIA_ITEM_ID];

                        string NotaFiscalCliente = string.Empty;
                        string NotaFiscalEstabelecimento = string.Empty;

                        DataTable itensReservados = tGridInfoPacote.Clone();

                        foreach (DataRow item in tGrid.Rows)
                        {
                            if (item["AssinaturaClienteID"] != DBNull.Value && Convert.ToInt32(item["AssinaturaClienteID"]) > 0)
                            {
                                DataRow[] itensAssinatura = tGridInfoAssinatura.Select(CancelamentoGerenciador.ASSINATURA_CLIENTE_ID + " = " + (int)item["AssinaturaClienteID"] + " ");
                        ...
                        DataTable tPagamento = new VendaBilheteria().FormasPagamentoCancelamento(vendaID);

                        int entregaAgendaID = Convert.ToInt32(tInfoVenda.Rows[0][CancelamentoGerenciador.ENTREGA_AGENDA_ID]);
                        decimal valorEntrega = (decimal)tInfoVenda.Rows[0][CancelamentoGerenciador.TAXA_ENTREGA_VALOR];
                        int clienteID = (int)tInfoVenda.Rows[0][CancelamentoGerenciador.CLIENTEID];
                        var impressos = tGrid.Select("Status='" + Ingresso.StatusIngresso.IMPRESSO + "'");
                        int entregaControleID = (int)tInfoVenda.Rows[0][CancelamentoGerenciador.ENTREGA_CONTROLE_ID];
                        bool imprimiu = impressos.Length > 0;
                        decimal valortotal = valorIngressos + (imprimiu ? 0 : valorEntrega);

                        string senhaTmp = new Bilheteria().Cancelar(itensReservados, tReserva, tPagamento, caixaID, lojaID, canalID, empresaID,
                            clienteID, entregaControleID, valorEntrega, valorIngressos, usuarioID,
                            NotaFiscalCliente, NotaFiscalEstabelecimento, false,
                            !imprimiu, string.Empty, false, vendaID, motivoId,
                                    SupervisorID, entregaAgendaID, false, 0, false,
                                    string.Empty, false, false, string.Empty, false);
                    }
                }
                else
                    throw new Exception("Um ou mais ingressos não estão disponíveis para ajuste!");
