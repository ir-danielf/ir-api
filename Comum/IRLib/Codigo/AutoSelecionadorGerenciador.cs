...
namespace IRLib
{
    ...
    /// </summary>
    public class AutoSelecionadorGerenciador : MarshalByRefObject, ISponsoredObject
    {
        ...
        public DataSet CarregarCanais(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarCanais(empresaID, ativos);
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tCanal.ID AS CanalID, tCanal.Nome AS Canal, tCanal.TaxaConveniencia, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK) " +
                    "WHERE tCanal.EmpresaID=tEmpresa.ID {0} " +
                    clausulaRegional +
                    "ORDER BY tEmpresa.Nome,tCanal.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tCanal.Ativo = 'T'" : "");
                ...
                if (IncluirRegional)
                    buffer.Tables.Add(this.CarregarRegionais(ativos));
                return buffer;
        ...
        private DataSet carregarCanais(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT ID,Nome,TaxaConveniencia FROM tCanal (NOLOCK) " +
                    "WHERE EmpresaID=" + empresaID + " {0} " +
                    "ORDER BY Nome", ativos ? "AND Ativo = 'T'" : "");
        ...
        public DataSet CarregarLojas(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarLojas(empresaID, ativos);
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tCanal.ID AS CanalID, tCanal.Nome AS Canal, tCanal.TaxaConveniencia, tLoja.ID AS LojaID, tLoja.Nome AS Loja, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK),tLoja (NOLOCK) " +
                    "WHERE tCanal.EmpresaID=tEmpresa.ID AND tCanal.ID=tLoja.CanalID " +
                    "{0} ORDER BY tEmpresa.Nome,tCanal.Nome,tLoja.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tCanal.Ativo = 'T'" : "");

                bd.Consulta(sql);

                Hashtable tempEmp = new Hashtable();
                Hashtable tempCanal = new Hashtable();

                while (bd.Consulta().Read())
                {

                    //Filtra Empresas
                    if (!tempEmp.ContainsKey(bd.LerInt("EmpresaID")))
                    {
                        DataRow empresa = empresas.NewRow();
                        empresa["ID"] = bd.LerInt("EmpresaID");
                        empresa["Nome"] = bd.LerString("Empresa");
                        ...
                        tempEmp.Add(empresa["ID"], true);
                    }

                    //Filtra Canais
                    string chaveCanal = string.Format("{0}-{1}", bd.LerInt("CanalID"), bd.LerInt("EmpresaID"));
                    if (!tempCanal.ContainsKey(chaveCanal))
                    {
                        DataRow canal = canais.NewRow();
                        canal["ID"] = bd.LerInt("CanalID");
                        canal["Nome"] = bd.LerString("Canal");
        ...
        private DataSet carregarLojas(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tCanal.ID AS CanalID, tCanal.Nome AS Canal, tCanal.TaxaConveniencia, tLoja.ID AS LojaID, tLoja.Nome AS Loja " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK),tLoja (NOLOCK) " +
                    "WHERE tCanal.EmpresaID=tEmpresa.ID AND tCanal.ID=tLoja.CanalID AND tEmpresa.ID=" + empresaID + " " +
                    "{0} ORDER BY tCanal.Nome,tLoja.Nome", ativos ? "AND tCanal.Ativo = 'T'" : "");
        ...
        public DataSet CarregarUsuarios(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarUsuarios(empresaID, ativos);
                ...
                string sql = String.Format("SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tCanal.ID AS CanalID, tCanal.Nome AS Canal, tCanal.TaxaConveniencia, tLoja.ID AS LojaID, tLoja.Nome AS Loja, tUsuario.ID AS UsuarioID, tUsuario.Nome AS Usuario, tEmpresa.RegionalID " +
                    ...
                    "{0} ORDER BY tEmpresa.Nome,tCanal.Nome,tLoja.Nome,tUsuario.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tCanal.Ativo = 'T'" : "");
        ...
        private DataSet carregarUsuarios(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT DISTINCT tCanal.ID AS CanalID, tCanal.Nome AS Canal, tCanal.TaxaConveniencia, tLoja.ID AS LojaID, tLoja.Nome AS Loja, tUsuario.ID AS UsuarioID, tUsuario.Nome AS Usuario " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK),tLoja (NOLOCK),tCaixa (NOLOCK),tUsuario (NOLOCK) " +
                    ...
                    "{0} ORDER BY tCanal.Nome,tLoja.Nome,tUsuario.Nome", ativos ? "AND tCanal.Ativo = 'T'" : "");
        ...
        public DataSet CarregarCaixas(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarCaixas(empresaID, ativos);
                else if (canalID != 0)
                    if (usuarioID != 0)
                    ...
                    return carregarLocalCaixas(localID, ativos);
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tCanal.ID AS CanalID, tCanal.Nome AS Canal, tLoja.ID AS LojaID, tLoja.Nome AS Loja, tUsuario.ID AS UsuarioID, tUsuario.Nome AS Usuario, tCaixa.ID AS CaixaID, tCaixa.DataAbertura, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK),tLoja (NOLOCK),tCaixa (NOLOCK),tUsuario (NOLOCK) " +
                    "WHERE tCanal.EmpresaID=tEmpresa.ID AND tCanal.ID=tLoja.CanalID AND tCaixa.LojaID=tLoja.ID AND tCaixa.UsuarioID=tUsuario.ID " +
                    "{0} ORDER BY tEmpresa.Nome,tCanal.Nome,tLoja.Nome,tUsuario.Nome,tCaixa.ID DESC", ativos ? "AND tEmpresa = 'T' AND tCanal = 'T'" : "");
        ...
        private DataSet carregarCaixas(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tCanal.ID AS CanalID, tCanal.Nome AS Canal, tLoja.ID AS LojaID, tLoja.Nome AS Loja, tUsuario.ID AS UsuarioID, tUsuario.Nome AS Usuario, tCaixa.ID AS CaixaID, tCaixa.DataAbertura " +
                    "FROM tEmpresa (NOLOCK),tCanal (NOLOCK),tLoja (NOLOCK),tCaixa (NOLOCK),tUsuario (NOLOCK) " +
                    "WHERE tCanal.EmpresaID=tEmpresa.ID AND tCanal.ID=tLoja.CanalID AND tCaixa.LojaID=tLoja.ID AND tCaixa.UsuarioID=tUsuario.ID AND tEmpresa.ID=" + empresaID + " " +
                    "{0} ORDER BY tCanal.Nome,tLoja.Nome,tUsuario.Nome,tCaixa.ID DESC", ativos ? "AND tCanal.Ativo = 'T'" : "");
        ...
        private DataSet carregarLocalCaixas(int localID, bool ativos = false)
        {
        ...
        public DataSet CarregarLocais(bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID {0} " +
                    "ORDER BY tEmpresa.Nome,tLocal.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T'": "");
        ...
        public DataSet CarregarEventos(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarEventos(empresaID, ativos);
                else if (localID != 0)
                    return carregarLocalEventos(localID, ativos);
                ...
                string sql = String.Format("SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOL OCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID " +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tLocal.Nome,tEvento.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tLocal.Ativo = 'T'" : "");
        ...
        private DataSet carregarEventos(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT DISTINCT tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tLocal.Nome,tEvento.Nome", ativos ? "AND tEvento.Ativo = 'T' AND tLocal.Ativo = 'T'" : "");
        ...
        private DataSet carregarLocalEventos(int localID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT DISTINCT tEvento.ID AS EventoID, tEvento.Nome AS Evento " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " ORDER BY tEvento.Nome", ativos ? "AND tEvento.Ativo = 'T'" : "");
        ...
        public DataSet CarregarEmpresaEventos(bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID {0} " +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    clausulaRegional +
                    " ORDER BY tEmpresa.Nome,tEvento.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T'"  : "");
        ...
        public DataSet CarregarApresentacoes(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarApresentacoes(empresaID, ativos);
                else if (localID != 0)
                    return carregarLocalApresentacoes(localID, ativos);
                ...
                //Empresas
                string sql = String.Format("SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tEmpresa.RegionalID  " +
                          "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                          ...
                          disponivelRelatorio +
                          clausulaRegional +
                          " {0} ORDER BY tEmpresa.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
                ...
                // Local
                sql = String.Format("SELECT DISTINCT  tEmpresa.ID AS EmpresaID, tLocal.ID AS LocalID, tLocal.Nome AS Local " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID " + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    clausulaRegional +
                    " {0} ORDER BY tLocal.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
                ...
                // Evento
                sql = String.Format("SELECT DISTINCT tEvento.LocalID, tEvento.ID AS EventoID, tEvento.Nome AS Evento " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID " + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    clausulaRegional +
                    " {0} ORDER BY tEvento.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
                ...
                // Apresentações
                sql = String.Format("SELECT DISTINCT tApresentacao.EventoID, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID " + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    clausulaRegional +
                    " {0} ORDER BY tApresentacao.Horario", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
        ...
        private DataSet carregarApresentacoes(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                // Local
                string sql = String.Format("SELECT DISTINCT tLocal.ID AS LocalID, tLocal.Nome AS Local " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tLocal.Nome", ativos ? "AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
                ...
                sql = String.Format("SELECT DISTINCT tEvento.LocalID, tEvento.ID AS EventoID, tEvento.Nome AS Evento " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEvento.Nome", ativos ? "AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
                ...
                sql = String.Format("SELECT DISTINCT tApresentacao.EventoID, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tApresentacao.Horario", ativos ? "AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
        ...
        private DataSet carregarLocalApresentacoes(int localID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao " +
                    "FROM tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=" + localID + " " + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEvento.Nome,tApresentacao.Horario", ativos ? "AND tEvento.Ativo = 'T'" : "");
        ...
        public DataSet CarregarEmpresaApresentacoes(bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID " + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tEvento.Nome,tApresentacao.Horario", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T'" : "");
        ...
        public DataSet CarregarSetores(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarSetores(empresaID, ativos);
                else if (localID != 0)
                    return carregarLocalSetores(localID, ativos);
                else if (canalID != 0)
                    return carregarCanalSetores(canalID, ativos);
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tEmpresa.RegionalID " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tLocal.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        private DataSet carregarSetores(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK) " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tLocal.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome", ativos ? "AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        private DataSet carregarLocalSetores(int localID, bool ativos = false)
        {
            ...
            try
            {
                ...
                // Evento
                sql = String.Format("SELECT DISTINCT tEvento.ID AS EventoID, tEvento.Nome AS Evento " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEvento.Nome", ativos ? "AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
                ...
                sql = String.Format("SELECT DISTINCT tApresentacao.EventoID, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tApresentacao.Horario", ativos ? "AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
                ...
                // Setores
                sql = String.Format("SELECT DISTINCT tApresentacaoSetor.ApresentacaoID, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tSetor.VersaoBackground " +
                    "FROM tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK) " +
                    "WHERE tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID AND tApresentacaoSetor.SetorID=tSetor.ID AND tApresentacaoSetor.ApresentacaoID=tApresentacao.ID AND tEvento.LocalID=" + localID + " " + filtroSetor + tipoSetoresDesc + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tSetor.Nome", ativos ? "AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        private DataSet carregarCanalSetores(int canalID, bool ativos = false)
        {
            ...
            try
            {
                ...
                // Setores
                sql = String.Format("SELECT DISTINCT tApresentacao.ID AS ApresentacaoID, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado " +
                    "FROM tCanalEvento (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK) " +
                    "WHERE tEvento.ID=tApresentacao.EventoID AND tEvento.ID=tCanalEvento.EventoID AND tApresentacaoSetor.SetorID=tSetor.ID AND tApresentacaoSetor.ApresentacaoID=tApresentacao.ID AND tCanalEvento.CanalID=" + canalID + " " + filtroSetor + tipoSetoresDesc + hoje +
                    disponivelVenda +
                    disponivelAjuste +
        ...
        public DataSet CarregarEmpresaSetores(bool ativos = false)
        {
            try
            {
                DataSet buffer = new DataSet("Buffer");
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado , tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND tEvento.LocalID=tLocal.ID AND tApresentacaoSetor.SetorID=tSetor.ID AND tApresentacaoSetor.ApresentacaoID=tApresentacao.ID " + filtroSetor + tipoSetoresDesc + hoje +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        public DataSet CarregarEmpresaLocaisSetores(bool ativos = false)
        {
            try
            {
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tEmpresa.RegionalID " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        public DataSet CarregarPrecos(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarPrecos(empresaID, ativos);
                else if (localID != 0)
                    return carregarLocalPrecos(localID, ativos);
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tPreco.ID AS PrecoID, tPreco.Nome AS Preco, tPreco.Valor, tPreco.CorID, tPreco.ApresentacaoSetorID, tEmpresa.RegionalID " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEmpresa.Nome,tLocal.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome,tPreco.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        private DataSet carregarPrecos(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tLocal.ID AS LocalID, tLocal.Nome AS Local, tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tPreco.ID AS PrecoID, tPreco.Nome AS Preco, tPreco.Valor, tPreco.CorID, tPreco.ApresentacaoSetorID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK),tPreco (NOLOCK) " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tLocal.Nome,tEvento.Nome,tApresentacao.Horario,tSetor.Nome,tPreco.Nome", ativos ? "AND tLocal.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        private DataSet carregarLocalPrecos(int localID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tEvento.ID AS EventoID, tEvento.Nome AS Evento, tApresentacao.ID AS ApresentacaoID, tApresentacao.Horario AS Apresentacao, tSetor.ID AS SetorID, tSetor.Nome AS Setor, tSetor.Produto, tSetor.LugarMarcado, tPreco.ID AS PrecoID, tPreco.Nome AS Preco, tPreco.Valor, tPreco.CorID, tPreco.ApresentacaoSetorID " +
                    ...
                    disponivelAjuste +
                    disponivelRelatorio +
                    " {0} ORDER BY tEvento.Nome,tApresentacao.Horario,tSetor.Nome,tPreco.Nome", ativos ? "AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        public DataSet CarregarEmpresaPrecos(bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format(@"SELECT DISTINCT tEmpresa.ID, tEmpresa.Nome, tEmpresa.RegionalID 
                        FROM tEmpresa (NOLOCK), tLocal (NOLOCK), tEvento (NOLOCK), tApresentacao (NOLOCK), tApresentacaoSetor (NOLOCK), tSetor (NOLOCK), tPreco (NOLOCK)
                        ...
                        disponivelRelatorio +
                        clausulaRegional +
                        " {0} ORDER BY tEmpresa.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
                ...
                sql = String.Format(@"SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEvento.ID, tEvento.Nome
                        FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK),tPreco (NOLOCK)
                        WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND 
                        tEvento.LocalID=tLocal.ID AND tApresentacaoSetor.SetorID=tSetor.ID AND 
                        tApresentacaoSetor.ApresentacaoID=tApresentacao.ID AND 
                        ...
                        disponivelRelatorio +
                        clausulaRegional +
                        "{0} ORDER BY tEvento.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");

                comm.CommandText = sql;
                adapter.Fill(eventos);

                // Carregar informações de Apresentações
                sql = String.Format(@"SELECT DISTINCT tEvento.ID AS EventoID, tApresentacao.ID, tApresentacao.Horario AS Nome
                        FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tEvento (NOLOCK),tApresentacao (NOLOCK),tApresentacaoSetor (NOLOCK),tSetor (NOLOCK),tPreco (NOLOCK)
                        WHERE tLocal.EmpresaID=tEmpresa.ID AND tEvento.ID=tApresentacao.EventoID AND 
                        tEvento.LocalID=tLocal.ID AND tApresentacaoSetor.SetorID=tSetor.ID AND 
                        tApresentacaoSetor.ApresentacaoID=tApresentacao.ID AND 
                        ...
                        disponivelRelatorio +
                        clausulaRegional +
                        "ORDER BY tApresentacao.Horario", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
                ...
                sql = String.Format(@"SELECT DISTINCT tApresentacao.ID AS ApresentacaoID, tSetor.ID, tSetor.Nome, 
                        ...
                        disponivelRelatorio +
                        clausulaRegional +
                        "{0} ORDER BY tSetor.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
                ...
                sql = String.Format(@"SELECT DISTINCT tApresentacao.ID AS ApresentacaoID, tSetor.ID AS SetorID,
                        ...
                        disponivelRelatorio +
                        clausulaRegional +
                        "{0} ORDER BY tPreco.Nome", ativos ? "AND tEmpresa.Ativo = 'T' AND tEvento.Ativo = 'T' AND tSetor.Ativo = 'T'" : "");
        ...
        public DataSet CarregarPacotes(bool ativos = false)
        {
            ...
            try
            {
                ...
                if (empresaID != 0)
                    return carregarPacotes(empresaID, ativos);
                else if (localID != 0)
                ...
                string sql = "SELECT DISTINCT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tLocal.ID AS LocalID, tLocal.Nome AS Local, tPacote.ID AS PacoteID, tPacote.Nome AS Pacote, tEmpresa.RegionalID " +
                    ...
                    clausulaRegional +
                    (ativos == true ? "AND tEmpresa.Ativo = 'T' AND tLocal.Ativo = 'T' " : "") +
        ...
        private DataSet carregarPacotes(int empresaID, bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = "SELECT tLocal.ID AS LocalID, tLocal.Nome AS Local, tPacote.ID AS PacoteID, tPacote.Nome AS Pacote " +
                    ...
                    (somenteApresentacoesDisponiveisAjuste == true ? "AND tPacoteItem.PacoteID = tPacote.ID AND tApresentacao.ID = tPacoteItem.ApresentacaoID AND tApresentacao.DisponivelAjuste = 'T' " : "") +
                    (ativos == true ? "AND tLocal.Ativo = 'T' " : "") +
        ...
        public DataSet CarregarEmpresaPacotes(bool ativos = false)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT tEmpresa.ID AS EmpresaID, tEmpresa.Nome AS Empresa, tPacote.ID AS PacoteID, tPacote.Nome AS Pacote, tEmpresa.RegionalID " +
                    "FROM tEmpresa (NOLOCK),tLocal (NOLOCK),tPacote (NOLOCK) " +
                    "WHERE tLocal.EmpresaID=tEmpresa.ID AND tPacote.LocalID=tLocal.ID {0} " +
                    "ORDER BY tEmpresa.Nome,tPacote.Nome", ativos ? "AND tEmpresa.Ativo = 'T'" : "");
        ...
        private DataTable CarregarRegionais(bool ativos = false)
        {
            BD bd = new BD();
            try
            {
                ...
                if (ativos)
                    comm.CommandText = "SELECT DISTINCT R.ID, R.Nome FROM tRegional (NOLOCK) AS R INNER JOIN tEmpresa (NOLOCK) AS E ON R.id = E.RegionalID WHERE E.Ativo = 'T' ORDER BY Nome";
                else
