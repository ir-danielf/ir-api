...
namespace IRLib
{
    partial class VendaBilheteria : VendaBilheteria_B
    {
        ...
        public int[] ItensComBaixa { get; set; }
        ...
        public string StringInserir()
        {
            ...
            try
            {
                ...
                sql.Append("INSERT INTO tVendaBilheteria(CaixaID, ClienteID, DataVenda, Status, " +
                    ...
                    "EntregaAgendaID,PdvID, PagamentoProcessado, NomeCartao, ValorSeguro, TaxaProcessamentoValor, AccertifyForceStatus,VendaBilhereriaIDTroca, VendaBilheteriaIDOrigem, EntregaCancelada, ConvenienciaCancelada, SeguroCancelado, VendaCancelada) ");

                sql.Append("VALUES (@001,@002,'@004','@005',@006,'@007','@008','@009','@010','@011','@012'," +
                    "'@013',@014,'@015','@016','@017','@018','@019','@020','@021', @022, @023, @024, '@025', @026, '@027', '@028', @029,@030,@031,@032,@033, '@034', '@035', @036, @037, '@038',@039, @040, '@041', '@042', '@043', '@044');SELECT SCOPE_IDENTITY()");
                ...
                sql.Replace("@040", this.VendaBilheteriaIDOrigem.ValorBD);

                sql.Replace("@041", this.EntregaCancelada.ValorBD);
                sql.Replace("@042", this.ConvenienciaCancelada.ValorBD);
                sql.Replace("@043", this.SeguroCancelado.ValorBD);
                sql.Replace("@044", this.VendaCancelada.ValorBD);
                return sql.ToString();
        ...
        /// <param name="filtrarCancelados"></param>
        /// <returns></returns>
        public DataTable Senhas(int clienteid, bool filtrarCancelados = true)
        {
            ...
            try
            {
                ...
                string sql = "SELECT ID, Senha FROM tVendaBilheteria (NOLOCK) " +
                             "WHERE Status IN('" + PAGO + "','" + EMANALISE + "', '" + AGUARDANDO_APROVACAO + "') ";
                if (filtrarCancelados)
                    sql += "AND VendaCancelada = 'F' ";
        ...
        public DataTable FormasPagamentoCancelamento(int vendaid)
        {
            try
            {
                DataTable tabela = new DataTable("Pagamento");
                string consulta = @"SELECT tFormaPagamento.ID , tFormaPagamento.Nome AS FormaPagamento, 
                    tVendaBilheteriaFormaPagamento.Valor, tVendaBilheteriaFormaPagamento.Dias, tVendaBilheteriaFormaPagamento.TaxaAdm,
                    tVendaBilheteriaFormaPagamento.IR
                    FROM tVendaBilheteria (NOLOCK)
                    INNER JOIN tVendaBilheteriaFormaPagamento (NOLOCK) ON tVendaBilheteria.ID = tVendaBilheteriaFormaPagamento.VendaBilheteriaID 
                    INNER JOIN tFormaPagamento (NOLOCK) ON tVendaBilheteriaFormaPagamento.FormaPagamentoID = tFormaPagamento.ID 
                    WHERE tVendaBilheteriaFormaPagamento.VendaBilheteriaID={0}";
                consulta = string.Format(consulta, vendaid);
        ...
        public IRLib.ClientObjects.EstruturaPagamento.enumTipoPagamento TipoPagamento(int vendaBilheteriaID)
        {
            try
            {
                string sql = @"SELECT IsNull(vbfp.PayerIDPayPal, '') AS PayPal
                                  FROM tvendabilheteriaformapagamento vbfp (nolock) 
                                 WHERE vbfp.VendaBilheteriaID = " + vendaBilheteriaID;

                if (!string.IsNullOrEmpty(bd.ConsultaValor(sql).ToString()))
        ...
        {
            AtualizarStatusAntiFraude(id, status, 0, IRLib.HammerHead.Enumeradores.RetornoAccertify.Indefinido);
        }
        ...
        public void AtualizarStatusAntiFraude(int id, StatusAntiFraude status, int score, HammerHead.Enumeradores.RetornoAccertify retornoAccertify)
        {
            try
            {
                LogUtil.Debug(string.Format("##VendaBilheteria.AtualizandoStatusAntiFraude## VENDA: {0}, STATUS: {1}, SCORE: {2}, RETORNO_ACCERTIFY: {3}", id, status, score, retornoAccertify));

                var oCartao = new Cartao();

                var statusCliente = ' ';
                this.Ler(id);
                this.Status.Valor = Convert.ToString((char)status);

                var cliente = new Cliente(this.Control.UsuarioID);
                ...
                switch (status)
                {
                    case StatusAntiFraude.Fraude:
                        ...
                        this.MotivoID.ValorBD = DBNull.Value.ToString(CultureInfo.InvariantCulture);
            ...
            {
                LogUtil.Error(string.Format("##VendaBilheteria.AtualizandoStatusAntiFraude.EXCEPTION## VENDA: {0}, STATUS: {1}, SCORE: {2}, RETORNO_ACCERTIFY: {3}, MSG: {4}", id, status, score, retornoAccertify, ex.Message), ex);
                throw;
            }
            finally
            {
        ...
        public void AtualizarStatusAntiFraude(int id, StatusAntiFraude status, int MotivoID, string Obs)
        {
            ...
            try
            {
                IRLib.Cartao oCartao = new Cartao();
        ...
        public EstruturaTransacoesDetalhes getTransacoesDetalhes()
        {
            try
            {
                ...
                int FormaPagamentoIDValeIngresso = new IRLib.ConfigGerenciador().getFormaPagamentoIDValeIngresso();
        ...
        public bool AtivaAntiFraude(int CanalID, IRLib.Loja.TEFTipos tipoLoja)
        {
            IRLib.ConfigGerenciador config = new ConfigGerenciador();
        ...
        public void AtualizaVendaCancelada(string senha)
        {
            ...
            var vb = new VendaBilheteria { Senha = { Valor = senha } };
            vb.AtribuirIDPelaSenha();
            vb.Ler(vb.Control.ID);
            if (vb.VendaCancelada.Valor) return;
        ...
        {
            return CarregarVendasParaPagamento(HammerHead.Enumeradores.Site.IngressoRapido);
        }

        public List<HammerHead.EstruturaVenda> CarregarVendasParaPagamento(HammerHead.Enumeradores.Site site, string modo = "todos")
        {
            try
            {
                LogUtil.Info(string.Format("##VendaBilheteria.CarregandoVendasParaPagamento## SITE: {0}", site));

                var vendas = new List<HammerHead.EstruturaVenda>();

                var sql = string.Format("EXEC CarregarPagamentosRiscoFraude {0}", (int)site);
                var mod = (modo == "pares") ? 0 : (modo == "impares") ? 1 : -1;

                if (mod > -1)
                {
                    sql = string.Format("EXEC CarregarPagamentosRiscoFraude {0}, {1}", (int)site, mod);
                }

                LogUtil.Debug(string.Format("##VendaBilheteria.CarregarVendasParaPagamento## SQL_COMMAND: {0}", sql));
                ...
                while (bd.Consulta().Read())
                {
                    vendas.Add(new HammerHead.EstruturaVenda()
                    {
                        ...
                        //Precisa ignorar o valor total, não pode cobrar o VIR!
                        ...
                        VendaCancelada = bd.LerBoolean("VendaCancelada"),
                        ...
                        FormaPagamento = new HammerHead.EstruturaFormaPagamento()
                        {
                            ...
                            VendaBilheteriaFormaPagamentoIDOrigem = bd.LerInt("VendaBilheteriaFormaPagamentoIDOrigem"),
                        },
        ...
        public HammerHead.EstruturaVenda CarregarVendaParaPagamento(int vendaBilheteriaID)
        {
            ...
            {
                var sql =
@"SELECT TOP 1 vb.ID AS                     VendaBilheteriaID,
             vb.Senha AS                  Senha,
             vb.DataVenda AS              DataVenda,
             vb.ValorTotal AS             ValorTotal,
             c.ID AS                      ClienteID,
             c.Nome AS                    ClienteNome,
             c.Email AS                   ClienteEmail,
             vbfp.FormaPagamentoID AS     FormaPagamentoID,
             fp.BandeiraID AS             BandeiraID,
             fp.Parcelas AS               Parcelas,
             ct.CartaoCr AS               NumeroCartao,
             ct.DataCr AS                 DataVencimento,
             ct.CVVCr AS                  CodigoSeguranca,
             vbfp.ID AS                   VendaBilheteriaFormaPagamentoID,
             vbfp.Cupom AS                Cupom,
             vbfp.CodigoRespostaVenda AS  CodigoRespostaVenda,
             vbfp.Valor AS                Valor,
             vb.IP AS                     IP,
             vb.NomeCartao AS             NomeCartao,
             vb.DDD + vb.NumeroCelular AS Celular,
             b.Nome AS                    Bandeira,
             vb.Score AS                  Score,
             vb.VendaCancelada AS         VendaCancelada
FROM tVendaBilheteria AS vb(NOLOCK)
     INNER JOIN tCliente AS c(NOLOCK) ON c.ID = vb.ClienteID
     INNER JOIN tVendaBilheteriaFormaPagamento AS vbfp(NOLOCK) ON vbfp.VendaBilheteriaID = vb.ID
     INNER JOIN tFormaPagamento AS fp(NOLOCK) ON vbfp.FormaPagamentoID = fp.ID
     INNER JOIN tBandeira AS b(NOLOCK) ON fp.BandeiraID = b.ID
     LEFT JOIN tCartao AS ct(NOLOCK) ON ct.ID = vbfp.CartaoID
WHERE vb.ID = {0};";

                sql = string.Format(sql, vendaBilheteriaID);

                bd.Consulta(sql);

                if (!bd.Consulta().Read())
                    throw new Exception(string.Format("venda {0} não encontrada.", vendaBilheteriaID));
        ...
        public void CancelarVenda(HammerHead.EstruturaVenda venda, bool cancelamentoFraude = false)
        {
            try
            {
                LogUtil.Info(string.Format("##VendaBilheteria.CancelandoVenda## VENDA: {0}", venda.ID));

                var oBilheteria = new Bilheteria();
                var caixaID = oBilheteria.VerificaCaixaInternet();

                var cancelamentoGerenciador = new CancelamentoGerenciador
                {
                    UsuarioID = Usuario.INTERNET_USUARIO_ID,
                    CaixaID = caixaID,
                    CanalID = Canal.CANAL_INTERNET,
                    PerfilID = Perfil.SAC_SUPERVISOR
                };

                var ds = cancelamentoGerenciador.PesquisarVenda(venda.ID, false);

                var itensReservados = ds.Tables[CancelamentoGerenciador.TABELA_GRID];
                var tReserva = ds.Tables[CancelamentoGerenciador.TABELA_RESERVA];
                var tInfoVenda = ds.Tables[CancelamentoGerenciador.INFO_VENDA];

                var entregaControleID = Convert.ToInt32(tInfoVenda.Rows[0][CancelamentoGerenciador.ENTREGA_CONTROLE_ID]);
                var entregaAgendaID = Convert.ToInt32(tInfoVenda.Rows[0][CancelamentoGerenciador.ENTREGA_AGENDA_ID]);
                var valorEntrega = Convert.ToDecimal(tInfoVenda.Rows[0][CancelamentoGerenciador.TAXA_ENTREGA_VALOR]);
                var valorTotalConveniencia = Convert.ToDecimal(tInfoVenda.Rows[0][CancelamentoGerenciador.TAXA_CONV_VALOR_TOTAL]);
                var valorTotal = Convert.ToDecimal(tInfoVenda.Rows[0][CancelamentoGerenciador.VALOR_TOTAL]);
                var motivoIDCartaoInvalido = Convert.ToInt32(ConfigurationManager.AppSettings["MotivoIDCartaoInvalido"]);

                var dtPagamento = cancelamentoGerenciador.FormasPagamento(Canal.CANAL_INTERNET, venda.ID, Empresa.INTERNET_EMPRESA_ID);
                dtPagamento.Columns.Add("Valor", typeof(decimal)).DefaultValue = 0;

                var tPagamento = dtPagamento.Clone();
                foreach (var dtr in dtPagamento.Select("ID = " + venda.FormaPagamento.ID))
                {
                ...
                var apoliceMondial = string.Empty;
                if (valorSeguro > 0)
                {
                    var oApolice = new ApoliceMondial();
                    apoliceMondial = oApolice.BuscaApolice(venda.ID);
                }

                var senhaTmp = oBilheteria.Cancelar(itensReservados, tReserva, tPagamento, caixaID, Loja.INTERNET_LOJA_ID, Canal.CANAL_INTERNET, Empresa.INTERNET_EMPRESA_ID,
                                    venda.Cliente.ID, entregaControleID, valorEntrega, valorTotal, Usuario.INTERNET_USUARIO_ID,
                                    string.Empty, string.Empty, true,
                                    entregaControleID > 0, "",
                                    cancelamentoFraude, venda.ID, motivoIDCartaoInvalido, Usuario.INTERNET_USUARIO_ID, entregaAgendaID,
                                    venda.TaxaProcessamento > 0, venda.TaxaProcessamento, false, apoliceMondial, false, false, string.Empty, true);

                LogUtil.Info(string.Format("##VendaBilheteria.CancelandoVenda.SUCCESS## VENDA: {0}, SENHA_CANCEL: {1}", venda.ID, senhaTmp));

                this.AtualizaVendaCancelada(senhaTmp);
            }
            catch (Exception ex)
            {
                LogUtil.Error(string.Format("##VendaBilheteria.CancelarVenda.EXCEPTION## VENDA: {0}, MSG: {1}", venda.ID, ex.Message), ex);
                throw;
            }
        }
        ...
        {
            List<IRLib.ClientObjects.IngressoImpressaoCancelar> listaIngressoImpressaoCancelar = new List<IRLib.ClientObjects.IngressoImpressaoCancelar>();
            ...
            IRLib.ClientObjects.IngressoImpressaoCancelar ingressoImpressaoCancelar;
            ...
            foreach (DataRow linha in tabelaAux)
            {
                if (ingressoIDAux != Convert.ToInt32(linha["IngressoID"]))
                {
                    ...
                    {                        //preenche o objeto com as informações da ultima impressão.
                        ingressoImpressaoCancelar = new IRLib.ClientObjects.IngressoImpressaoCancelar();
        ...
        public string PegarNotaFiscal(int Id)
        {

            try
            {
                string sql =
                    string.Format(@"SELECT tVendaBilheteria.NotaFiscalCliente 
                        FROM  tVendaBilheteria (NOLOCK)
                        WHERE ID = {0}", Id);
                ...
                if (!bd.Consulta().Read())
                    ...
                    return bd.LerString("NotaFiscalCliente");
            }
            finally
            {
                bd.Fechar();
        ...
        public int VerificaFormaEntregaVendaTrocaIngresso()
        {
            BD bd = new BD();
            try
            {
                string sql = string.Format(@"SELECT tVendaBilheteria.EntregaControleID,tIngresso.Status, ISNULL(tEntregaControle.EntregaID,0) as EntregaID from tVendaBilheteria(nolock) 
                            INNER JOIN tIngresso(nolock) ON tIngresso.VendaBilheteriaID = tVendaBilheteria.ID
                            LEFT JOIN tEntregaControle(nolocK) ON tEntregaControle.ID = tVendaBilheteria.EntregaControleID
                            WHERE tVendaBilheteria.ID = {0}", this.Control.ID);

                DataTable dados = bd.QueryToTable(sql);
                string status = dados.Rows[0]["Status"].ToString();
                int EntregaControleID = Convert.ToInt32(dados.Rows[0]["EntregaControleID"]);
                int entregaID = Convert.ToInt32(dados.Rows[0]["EntregaID"]);

                return (status.ToLower() != "i" || entregaID == 11) ? EntregaControleID : 0;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public decimal ConsultarValorIngressos(int ID)
        {
            try
            {
                BD bd = new BD();
                string sql = string.Format(@"SELECT SUM(Valor) as Valor from tIngresso (NOLOCK)
                                             INNER JOIN tPreco ON tPreco.ID = tIngresso.PrecoID
                                             WHERE vendaBilheteriaID = {0}", ID);
                bd.Consulta(sql);
                if (!bd.Consulta().Read())
                    return 0;
                else
                    return bd.LerDecimal("Valor");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        internal bool AtualizaCaixaDaDevolucao(BD bd, int vendaBilheteriaID, int caixaID)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append(@"UPDATE tVendaBilheteria
                            SET CaixaID = @001,
                                DataVenda = '@002'
                          ");
            sql.Append("WHERE ID = @ID");
            sql.Replace("@ID", vendaBilheteriaID.ToString());
            sql.Replace("@001", caixaID.ToString());
            sql.Replace("@002", DateTime.Now.ToString("yyyyMMddHHmmss"));
        ...
        public bool ValidaReservaPNE(DataTable tReservaToPNE, int userId)
        {
            bool retorno = false;
            string sql = string.Empty;
            BD bd = new BD();
            ...
            try
            {
                sql = "exec CheckAcompanhantePNE_Reserva {0}, '', {1};";

                for (int i = 0; i < tReservaToPNE.Rows.Count; i++)
                {
                    int ingressoID = Convert.ToInt32(tReservaToPNE.Rows[i]["IngressoID"]);

                    object x = bd.ConsultaValor(string.Format(sql, ingressoID, userId));

                    retorno = Convert.ToBoolean(x);
        ...
        public int BuscaCanalWhitelabelSiteID(string _senha)
        {
            try
            {
                var sql =
@"SELECT tCanalWhitelabel.SiteID
FROM tVendaBilheteria AS vb(NOLOCK)
     INNER JOIN tCaixa AS tCaixa(NOLOCK) ON tCaixa.ID = vb.CaixaID
     INNER JOIN tLoja AS tLoja(NOLOCK) ON tLoja.ID = tCaixa.LojaID
     INNER JOIN tCanalWhitelabel AS tCanalWhitelabel(NOLOCK) ON tLoja.CanalID = tCanalWhitelabel.CanalID
WHERE vb.Senha = '{0}'";

                sql = string.Format(sql, _senha);

                this.bd.Consulta(sql, null);
                if (!this.bd.Consulta().Read())
                    throw new Exception(string.Format("Erro: senha {0} não encontrada. SQL: {1}", _senha, sql));
                return this.bd.LerInt("SiteID");
            }
            finally
            {
                this.bd.Fechar();
