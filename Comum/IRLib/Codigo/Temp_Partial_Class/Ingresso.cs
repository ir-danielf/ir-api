    ...
    public partial class Ingresso : Ingresso_B
    {
        ...
        public bool MudarPreco(BD bd, int precoID)
        {
            List<SqlParameter> parametros = new List<SqlParameter>()
            {
                ...
                new SqlParameter("cortesiaID", this.CortesiaID.Valor),
                new SqlParameter("ID", this.Control.ID)
            };

            string sql = "UPDATE tIngresso SET PrecoID = @precoID, CortesiaID = @cortesiaID WHERE ID = @ID";
