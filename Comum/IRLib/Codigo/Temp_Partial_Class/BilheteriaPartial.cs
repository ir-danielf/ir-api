...
namespace IRLib
{
    public partial class Bilheteria
    {
        ...
        public int GetPacotesQPodeReservar(int pacoteID, int quantidade, string sessionID, int clienteID, EstruturaReservaInternet estruturaReservaInternet)
        {
            ...
            try
            {
                var qtdePacoteQPodeReservar = quantidade;

                //1. obter a Quantidade desse pacote
                var pacote = new Pacote();
                var infoPacote = pacote.LerToReserva(pacoteID, estruturaReservaInternet.CanalID, bd);

                if (infoPacote.Pacote.Control.ID == 0)
                    throw new BilheteriaException("Pacote não existe.");

                // Quantidade de pacotes disponíveis, que podem vender independente do canal.
                var qtdePacoteDisponivel = infoPacote.Pacote.Quantidade.Valor;

                // Quantidade de pacotes que podem vender por canal
                var qtdePacotePorCanal = infoPacote.QtdDisponivelCanal;

                //qtdePacoteVendido = infoPacote.QtdVendido;
                // Quantidade de pacotes vendidos.
                var qtdePacoteVendido = pacote.LerFromCliente(pacoteID, estruturaReservaInternet.CanalID, clienteID, sessionID, bd);
                ...
                if (qtdePacoteDisponivel != 0)
                {
                    ...
                    if (qtdePacoteVendido + quantidade <= qtdePacoteDisponivel)
                    {
                        qtdePacoteQPodeReservar = quantidade;
                    }
                    else if (qtdePacoteVendido <= qtdePacoteDisponivel)
                    {
                        qtdePacoteQPodeReservar = qtdePacoteDisponivel - qtdePacoteVendido;
                    }
                    else
                    {
                ...
                // Se a quantidade for zero, está liberado para vender à vontade!
                if (qtdePacotePorCanal == 0)
                    return qtdePacoteQPodeReservar;

                // Se a quantidade vendida for maior ou igual à quantidade disponível no canal, não pode vender.
                if (qtdePacoteQPodeReservar >= qtdePacotePorCanal)
        ...
        public DataSet MontaDadosVendaInternet(Dictionary<int, decimal> ingressosID, int clienteID, string sessionID, int canalID, int usuarioID)
        {
            ...
            sql = @"SELECT DISTINCT
					...
					WHERE ClienteID = " + clienteID + " AND SessionID = '" + sessionID + "'  AND UsuarioID = " + usuarioID + " AND tIngresso.Status = 'R' AND tCanalEvento.CanalID = " + canalID + " ORDER BY IngressoID";
            ...
            if (aux.Rows.Count != ingressosID.Count)
                throw new BilheteriaException("Não foi possível realizar a reserva do ingresso. Por favor, reinicie o fluxo de compra novamente.");
        ...
        private string[] MontaRetornoVendaInternetExterno(string msgCodigo, string senha, long vendaBilheteriaID, int caixaID)
        {
            return new string[] { msgCodigo, senha, vendaBilheteriaID.ToString(), caixaID.ToString() };
        }
        ...
        public string[] VenderVIRInternet(EstruturaPagamento oPagamento, int[] virsID, string sessionID, List<EstruturaVirNomePresenteado> lstPresenteado, EstruturaReservaInternet estruturaReservaInternet, string celular)
        {
            ...
            try
            {
                ...
                switch (oPagamento.TipoPagamento)
                {
                    ...
                    case EstruturaPagamento.enumTipoPagamento.Paypal:
                        DoExpressCheckoutPaymentOperation DoCheckOut = PayPalApiFactory.instance.ExpressCheckout().DoExpressCheckoutPayment(oPagamento.Token, oPagamento.PayerID, (double)oPagamento.ValorTotal, PaymentAction.SALE);
            ...
            catch (BilheteriaException ex)
            {
                ...
                if (oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.TEF && ProcessouSitef)
                {
                    ...
                    this.Refund(oPagamento.TransactionID);
        ...
        public string[] VenderWeb(EstruturaPagamento oPagamento, Dictionary<int, decimal> ingressosID, string sessionID, List<int> valeIngresso, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, EstruturaReservaInternet estruturaReservaInternet, decimal ValorTotalSeguro, decimal ValorTotalTaxaProcessamento, Cliente oCliente)
        {
            LogUtil.Debug(string.Format("##BilheteriaPartial.VenderWeb## SESSION {0}", sessionID));
            ...
            try
            {
                ...
                // Verificação se o cliente tem nome, se não tiver usar o nome do cartão
                if (oCliente.Nome == null || string.IsNullOrEmpty(oCliente.Nome.Valor))
                {
                    oPagamento.ClienteNome = oPagamento.NomeCartao;
                }
                else
                {
            ...
            {
                retorno[0] = ex.CodigoErro;
                retorno[1] = ex.Message;
            }
            catch (Exception ex)
            {
                retorno[0] = ((int)Bilheteria.CodMensagemVenda.ErroIndefinido).ToString();
                retorno[1] = ex.Message;
            }
        ...
        public string[] VenderWebCortesia(EstruturaPagamento oPagamento, Dictionary<int, decimal> ingressosID, string sessionID, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, EstruturaReservaInternet estruturaReservaInternet, decimal ValorTotalTaxaProcessamento, Cliente oCliente, int clienteID)
        {
            string[] retorno = new string[2];
            try
            {
                ...
                retorno = VenderInternetCortesia(oPagamento, ingressosID, sessionID, celular, listaDonoIngresso, estruturaReservaInternet, ValorTotalTaxaProcessamento, oCliente, clienteID);
            }
            catch (BilheteriaException ex)
            {
                retorno[0] = ((int)ex.CodigoErro).ToString();
        ...
        public string[] VenderVIRWeb(EstruturaPagamento oPagamento, int[] virsID, string sessionID, List<EstruturaVirNomePresenteado> lstPresenteado, EstruturaReservaInternet estruturaReservaInternet, string celular, Cliente oCliente)
        {
            ...
            try
            {
                ...
                {
                    retorno[0] = ((int)Bilheteria.CodMensagemVenda.TaxaEntregaInexistente).ToString();
        ...
        public string[] VenderInternet(EstruturaPagamento oPagamento, Dictionary<int, decimal> ingressosID, string sessionID, List<int> valeIngresso, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, EstruturaReservaInternet estruturaReservaInternet, decimal ValorTotalSeguro, decimal valorTotalTaxaProcessamento, Cliente oClienteSelecionado)
        {
            LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet## SESSION {0}", sessionID));
            ...
            try
            {
                ...
                switch (oPagamento.TipoPagamento)
                {
                    case EstruturaPagamento.enumTipoPagamento.TEF:
                        ProcessouSitef = this.EfetuaPagamentoTef(oPagamento, vendaAsync);
                        break;
                    case EstruturaPagamento.enumTipoPagamento.TEFDebito:
                        ProcessouSitef = this.EfetuaPagamentoTefDebito(oPagamento);
                        break;
                    case EstruturaPagamento.enumTipoPagamento.Adyen:
                        ProcessouAdyen = this.EfetuaPagamentoAdyen(oPagamento, vendaAsync);
                        break;
                    case EstruturaPagamento.enumTipoPagamento.Paypal:
                        var DoCheckOut = PayPalApiFactory.instance.ExpressCheckout().DoExpressCheckoutPayment(oPagamento.Token, oPagamento.PayerID, (double)oPagamento.ValorTotal, PaymentAction.SALE);

                        DoCheckOut.LocaleCode = LocaleCode.BRAZILIAN_PORTUGUESE;
                        DoCheckOut.CurrencyCode = CurrencyCode.BRAZILIAN_REAL;

                        LogUtil.Debug(string.Format("##BilheteriaPartial.PagandoComPaypal## SESSION {0}, VLR {1}", sessionID, oPagamento.ValorTotal));

                        DoCheckOut.execute();

                        if (DoCheckOut.ResponseNVP.Ack != Ack.SUCCESS)
                        {
                            LogUtil.Debug(string.Format("##BilheteriaPartial.PagandoComPaypal.ERROR## SESSION {0}, VLR {1}, MSG {2}", sessionID, oPagamento.ValorTotal, DoCheckOut.ResponseNVP.getError(0).LongMessage));

                            var msg = string.Empty;
                            var codigoErro = string.Empty;

                            switch (DoCheckOut.ResponseNVP.getError(0).Code)
                            {
                                case "10411":
                                    msg = "Sessão expirada";
                                    codigoErro = ((int)EstruturaPagamento.enumRetornoPagamento.PAGAMENTO_NAO_REALIZADO).ToString();
                                    break;
                                case "10422":
                                case "10486":
                                case "10417":
                                    msg = "A forma de pagamento escolhida foi recusada";
                                    codigoErro = ((int)EstruturaPagamento.enumRetornoPagamento.PAGAMENTO_RECUSADO).ToString();
                                    break;
                                case "10537":
                                    msg = "Venda recusada para seu país";
                                    codigoErro = ((int)EstruturaPagamento.enumRetornoPagamento.PAGAMENTO_NAO_REALIZADO).ToString();
                                    break;
                                default:
                                    msg = "Não foi possível efetuar o pagamento através do PayPal";
                                    codigoErro = DoCheckOut.ResponseNVP.getError(0).Code;
                                    break;
                            }

                            throw new PayPalException(msg, codigoErro);
                        }
                        
                        LogUtil.Debug(string.Format("##BilheteriaPartial.PagandoComPaypal.SUCCESS## SESSION {0}, VLR {1}, TRANS {2}", sessionID, oPagamento.ValorTotal, DoCheckOut.ResponseNVP.TransactionId));
                ...
                LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.Inserindo_tVendaBilheteria## SESSION {0}", sessionID));

                string sqlVendaBilheteria = vendaBilheteria.StringInserir();
                object vendaID = bd.ConsultaValor(sqlVendaBilheteria);
                vendaBilheteria.Control.ID = (vendaID != null) ? Convert.ToInt32(vendaID) : 0;

                LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.Inserindo_tVendaBilheteria.SUCESS## SESSION {0}, VendaBil_ID {1}", sessionID, vendaBilheteria.Control.ID));
                ...
                LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.Inserindo_VendaBilheteriaItem## SESSION {0}", sessionID));
                ...
                {
                    LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.RotinaValeIngresso## SESSION {0}", sessionID));
                ...
                {
                    LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.SemValeIngresso## SESSION {0}", sessionID));
                    ...
                    LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.InserirVendaBilheteriaFormaPagamento## SESSION {0}", sessionID));
                ...
                if (oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.TEF || oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.Adyen)
                {
                    ...
                    {
                        LogUtil.Info(string.Format("##BilheteriaPartial.VenderInternet.oAdyen.CapturarPagamento## SESSION {0}, MSG: {1}", sessionID, "Captura será realizada no HammerHead"));
                    }
                }
                ...
                {
                    LogUtil.Error(string.Format("##BilheteriaPartial.EnvioSMS.EXCEPTION## SESSION {0}, MSG {1}", sessionID, ex.Message), ex);
                }
            ...
            {
                LogUtil.Error(string.Format("##BilheteriaPartial.VenderInternet.EXCEPTION## SESSION {0}, MSG {1}", sessionID, ex.Message), ex);

                LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.DesfazendoTrasaçãoBD## SESSION {0}", sessionID));
                bd.DesfazerTransacao();
                ...
                LogUtil.Debug(string.Format("##BilheteriaPartial.VenderInternet.CancelandoPagamento## SESSION {0}, PGTO_TIPO {1}", sessionID, oPagamento.TipoPagamento));
        ...
        public string[] VenderExterno(EstruturaPagamentoExterno oPagamento, Dictionary<int, decimal> ingressosId, int clienteId, string sessionId, EstruturaReservaInternet estruturaReservaInternet)
        {
            var bd = new BD();
            ...
            try
            {
                ...
                //busca os dados da reserva para determinado cliente
                var ds = MontaDadosVendaInternet(ingressosId, clienteId, sessionId, estruturaReservaInternet.CanalID, estruturaReservaInternet.UsuarioID);

                var itensReservados = ds.Tables[TABELA_GRID]; // Resumo
                var ingressosVendidos = ds.Tables[TABELA_RESERVA]; // Detalhamento

                //verifica se a quantidade reservada no site é a mesma reservada no sistema
                if (ingressosVendidos.Rows.Count != ingressosId.Count)
                {
                    LogUtil.Error(string.Format("##VenderExterno.ERROR## SESSION {0}, MSG {1}", sessionId, "Quantidade de ingressos diferente da reserva."));
                    throw new BilheteriaException("Quantidade de ingressos diferente da reserva.", CodMensagemVenda.FalhaVenda);
                }
                ...
                if (itensReservados.Rows.Count == 0 && ingressosVendidos.Rows.Count == 0)
                {
                    LogUtil.Error(string.Format("##VenderExterno.ERROR## SESSION {0}, MSG {1}", sessionId, "Não existe reserva para o cliente."));
                    throw new BilheteriaException("Não existe reserva para o cliente.", CodMensagemVenda.ReservaInexistente);
                }
                ...
                vendaBilheteria.CaixaID.Valor = estruturaReservaInternet.CaixaID;
                vendaBilheteria.DataVenda.Valor = DateTime.Now;
                vendaBilheteria.PagamentoProcessado.Valor = true;
                vendaBilheteria.IP.Valor = oPagamento.IP;

                //valor total
                var valorAuxiliar = itensReservados.Compute("SUM(Valor)", "");
                var valorTotalIngresso = valorAuxiliar == DBNull.Value ? 0 : (decimal)valorAuxiliar;

                valorAuxiliar = itensReservados.Compute("SUM(TaxaConvenienciaValor)", "");
                var valorTotalConveniencia = valorAuxiliar == DBNull.Value ? 0 : (decimal)valorAuxiliar;

                var valorAuxiliarComissao = itensReservados.Compute("SUM(ComissaoValor)", "");
                var valorTotalComissao = valorAuxiliarComissao == DBNull.Value ? 0 : (decimal)valorAuxiliarComissao;

                var valorTotal = valorTotalIngresso + valorTotalConveniencia;

                vendaBilheteria.ValorTotal.Valor = valorTotal;
                vendaBilheteria.TaxaConvenienciaValorTotal.Valor = valorTotalConveniencia;
                vendaBilheteria.ComissaoValorTotal.Valor = valorTotalComissao;
                vendaBilheteria.Status.Valor = VendaBilheteria.PAGO;
                vendaBilheteria.NotaFiscalCliente.Valor = String.Format("(Ecopag POS)\n Rede {0}\n Valor: R$ {1}\n Parcelas: {2}\n NSU: {3}\n Autorização:{4}\n", oPagamento.formaPagamento, oPagamento.valor, oPagamento.parcelas, oPagamento.NSU, oPagamento.codigoAutorizacao);
                vendaBilheteria.NotaFiscalEstabelecimento.Valor = vendaBilheteria.NotaFiscalCliente.Valor;
                ...
                if (vendaBilheteria.Control.ID == 0)
                {
                    LogUtil.Error(string.Format("##VenderExterno.ERROR## SESSION {0}, MSG {1}", sessionId, "Venda não foi gerada."));
                    throw new BilheteriaException("Venda não foi gerada.", CodMensagemVenda.FalhaVenda);
                }

                var sqlSenha = "SELECT Senha FROM tVendaBilheteria (NOLOCK) WHERE ID=" + vendaBilheteria.Control.ID;
                var ret = bd.ConsultaValor(sqlSenha);
                var senha = (ret != null) ? Convert.ToString(ret) : null;

                var itensNormais = itensReservados.Select(TIPO + "='" + TIPO_INGRESSO + "' AND " + TIPO_LUGAR + "<>'" + Setor.MesaFechada + "'");
                var itensPacote = itensReservados.Select(TIPO + "='" + TIPO_PACOTE + "'");

                #region Ingressos Normais
                foreach (var item in itensNormais)
                {
                    ...
                    if (vendaBilheteriaItem.Control.ID == 0)
                    {
                        LogUtil.Error(string.Format("##VenderExterno.ERROR## SESSION {0}, MSG {1}", sessionId, "Item de venda não foi gerado."));
                        throw new BilheteriaException("Item de venda não foi gerado.", CodMensagemVenda.FalhaVenda);
                    }
                    ...
                    ingressoLog.TimeStamp.Valor = DateTime.Now;
                    ingressoLog.IngressoID.Valor = (int)item[INGRESSOID];
                    ingressoLog.UsuarioID.Valor = estruturaReservaInternet.UsuarioID;
                    ingressoLog.BloqueioID.Valor = 0;
                    ingressoLog.CortesiaID.Valor = Convert.ToInt32(ingressos[0][CORTESIAID]);
                    ingressoLog.PrecoID.Valor = (int)item[PRECOID];
                    ingressoLog.GerenciamentoIngressosID.Valor = (int)item[GERENCIAMENTO_INGRESSOS_ID];
                    ingressoLog.VendaBilheteriaItemID.Valor = vendaBilheteriaItem.Control.ID;
                    ingressoLog.VendaBilheteriaID.Valor = vendaBilheteria.Control.ID;
                    ingressoLog.CaixaID.Valor = estruturaReservaInternet.CaixaID;
                    ingressoLog.LojaID.Valor = estruturaReservaInternet.LojaID;
                    ingressoLog.CanalID.Valor = estruturaReservaInternet.CanalID;
                    ingressoLog.ClienteID.Valor = clienteId;
                    ingressoLog.Acao.Valor = IngressoLog.VENDER;
                    var sqlIngressoLogV = ingressoLog.StringInserir();

                    x = bd.Executar(sqlIngressoLogV);

                    var okV = (x == 1);
                    if (!okV) throw new BilheteriaException("Log de venda do ingresso não foi inserido.", CodMensagemVenda.FalhaVenda);
                }
                ...
                foreach (DataRow item in itensPacote)
                {
                    ...
                    foreach (DataRow i in ingressosPacote)
                    {
                        ...
                        ingressoLog.VendaBilheteriaItemID.Valor = vendaBilheteriaItem.Control.ID;
                        ingressoLog.VendaBilheteriaID.Valor = vendaBilheteria.Control.ID;
                        ingressoLog.CaixaID.Valor = estruturaReservaInternet.CaixaID; //caixa do usuario
                        ingressoLog.LojaID.Valor = estruturaReservaInternet.LojaID; //loja 
                        ingressoLog.CanalID.Valor = estruturaReservaInternet.CanalID; //canal 
                        ingressoLog.EmpresaID.Valor = estruturaReservaInternet.EmpresaID; //empresa IR
                        ingressoLog.ClienteID.Valor = estruturaReservaInternet.ClienteID;
                        ingressoLog.Acao.Valor = IngressoLog.VENDER;
                        string sqlIngressoLogV = ingressoLog.StringInserir();
                        x = bd.Executar(sqlIngressoLogV);
                        bool okV = (x == 1);
                ...
                if (oPagamento.formaPagamentoID > 0 && valorTotal > 0)
                {
                    var vendaBilheteriaFormaPagamento = new VendaBilheteriaFormaPagamento();
                    ...
                    vendaBilheteriaFormaPagamento.Porcentagem.Valor = porcentagemTotal;
                    vendaBilheteriaFormaPagamento.VendaBilheteriaID.Valor = vendaBilheteria.Control.ID;
                    vendaBilheteriaFormaPagamento.VendaBilheteriaFormaPagamentoTEFID.Valor = 0;
                    vendaBilheteriaFormaPagamento.CodigoRespostaVenda.Valor = "00";
                    vendaBilheteriaFormaPagamento.MensagemRetorno.Valor = "";
                    vendaBilheteriaFormaPagamento.HoraTransacao.Valor = DateTime.Now.ToString("hh:mm");
                    vendaBilheteriaFormaPagamento.DataTransacao.Valor = DateTime.Now.ToString("dd/MM/yyyy");
                    vendaBilheteriaFormaPagamento.CodigoIR.Valor = "";
                    vendaBilheteriaFormaPagamento.NumeroAutorizacao.Valor = oPagamento.codigoAutorizacao;
                    vendaBilheteriaFormaPagamento.NSUHost.Valor = oPagamento.NSU;
                    vendaBilheteriaFormaPagamento.NSUSitef.Valor = oPagamento.NSU;
                    vendaBilheteriaFormaPagamento.Cupom.Valor = String.Format("(Ecopag POS)\n Rede {0}\n Valor: R$ {1}\n Parcelas: {2}\n NSU: {3}\n Autorização:{4}\n", oPagamento.formaPagamento, oPagamento.valor, oPagamento.parcelas, oPagamento.NSU, oPagamento.codigoAutorizacao); ;
                    vendaBilheteriaFormaPagamento.DadosConfirmacaoVenda.Valor = "";
                    vendaBilheteriaFormaPagamento.Rede.Valor = "Ecopag";
                    vendaBilheteriaFormaPagamento.CodigoRespostaTransacao.Valor = "";

                    var sqlVendaBilheteriaFormaPagamento = vendaBilheteriaFormaPagamento.StringInserir();

                    if (bd.Executar(sqlVendaBilheteriaFormaPagamento) == 0)
                    {
                        LogUtil.Error(string.Format("##VenderExterno.ERROR## SESSION {0}, MSG {1}", sessionId, "Forma de pagamento não foi cadastrada."));
                        throw new BilheteriaException("Forma de pagamento não foi cadastrada.", CodMensagemVenda.FormaPagamentoNaoCadastrada);
                    }
                }
                ...
                return MontaRetornoVendaInternetExterno(((int)CodMensagemVenda.Sucesso).ToString(), senha, vendaBilheteria.Control.ID, estruturaReservaInternet.CaixaID);
            }
            catch (Exception ex)
            {
                bd.DesfazerTransacao();
        ...
        public string[] VenderInternetCortesia(EstruturaPagamento oPagamento, Dictionary<int, decimal> ingressosID, string sessionID, string celular, List<EstruturaDonoIngresso> listaDonoIngresso, EstruturaReservaInternet estruturaReservaInternet, decimal valorTotalTaxaProcessamento, Cliente oClienteSelecionado, int clienteID)
        {
            BD bd = new BD();
            decimal valorTotalIngresso = 0;
            decimal valorTotalConveniencia = 0;
            ...
            try
            {
                ...
                //TODO: Vilmar - Verificar a necessidade
                entregaSelecionada = oEA.CarregarEntrega(entregaControleID, oPagamento.DataSelecionada);
        ...
        public string[] VenderInternetCortesiaExterno(EstruturaPagamentoExterno oPagamento, Dictionary<int, decimal> ingressosID, string sessionID, EstruturaReservaInternet estruturaReservaInternet, Cliente oClienteSelecionado, int clienteID)
        {
            var bd = new BD();
            ...
            try
            {
                var stbSQL = new StringBuilder();
                //caixa para registrar a venda
                var caixaID = estruturaReservaInternet.CaixaID == 0 ? VerificaCaixaInternet() : estruturaReservaInternet.CaixaID;

                if (clienteID == 0)
                    clienteID = oClienteSelecionado.Control.ID;
                ...
                // VendaBilheteria
                var empresaID = new Canal().BuscaEmpresaIDporLojaID(estruturaReservaInternet.LojaID);
          ...
          string celular, List<EstruturaDonoIngresso> listaDonoIngresso, EstruturaReservaInternet estruturaReservaInternet, decimal ValorTotalSeguro, decimal valorTotalTaxaProcessamento, Cliente oClienteSelecionado)
        {
            ...
            try
            {
                ...
                if (oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.TEF || oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.TEFDebito || oPagamento.TipoPagamento == EstruturaPagamento.enumTipoPagamento.Adyen)
                {
                    if (oPagamento.IniciouTef)
                    {
                        if (oPagamento.oSitef.FinalizaTransacao(Sitef.enumTipoConfirmacao.Confirmar) != Sitef.enumRetornoSitef.Ok)
        ...
        {
            LogUtil.Debug(string.Format("##BilheteriaPartial.EfetuandoPagamentoTef## SESSION {0}, VLR {1}", oPagamento.SessionID, oPagamento.ValorTotal));
        ...
        private bool EfetuaPagamentoTefDebito(EstruturaPagamento oPagamento)
        {
            try
            {
                if (oPagamento.oSitef.IniciaSitefDebito() != Sitef.enumRetornoSitef.Ok)
                    throw new Exception("Não foi possível realizar o pagamento. Por favor tente novamente!");
        ...
        {
            LogUtil.Debug(string.Format("##BilheteriaPartial.EfetuandoPagamentoAdyen## SESSION {0}, VLR {1}", oPagamento.SessionID, oPagamento.ValorTotal));
            ...
            {
                LogUtil.Error(string.Format("##BilheteriaPartial.EfetuandoPagamentoAdyen.EXCEPTION.ReprocessarSITEF## SESSION {0}, MSG {1}", oPagamento.SessionID, ex.Message), ex);
                ...
                try
                {
                    ...
                    LogUtil.Info(string.Format("##BilheteriaPartial.EfetuandoPagamentoAdyen.ReprocessarSITEF.SUCESS## SESSION {0}", oPagamento.SessionID));

                    return true;
                }
                catch (Exception ex1)
                {
                    LogUtil.Error(string.Format("##BilheteriaPartial.EfetuandoPagamentoAdyen.ReprocessarSITEF.EXCEPTION## SESSION {0}, MSG {1}", oPagamento.SessionID, ex1.Message), ex1);
        ...
        {
            LogUtil.Debug(string.Format("##BilheteriaPartial.PayPalRefund## TRANS {0}", TransactionID));
            ...
            {
                LogUtil.Debug(string.Format("##BilheteriaPartial.PayPalRefund.ERROR## TRANS {0}, RESPONSE {1}", TransactionID, Refound.ResponseNVP.Ack));

                throw new Exception("Não foi possivel realizar o estorno do pagamento junto ao PayPal, tente novamente!");
            }

            LogUtil.Debug(string.Format("##BilheteriaPartial.PayPalRefund.SUCCESS## TRANS {0}, RESPONSE {1}", TransactionID, Refound.ResponseNVP.Ack));
        }

        private void RefundPartial(string TransactionID, decimal valor)
        {
            RefundTransaction Refound = PayPalApiFactory.instance.ExpressCheckout().RefundTransaction(TransactionID);
        ...
        private void EfetuarVendaCinema(DataRow[] cinema, string guid, string senha, int vendaBilheteriaID)
        {
            ...
            List<IRLib.Cinema.Programacao> programacoes = IRLib.Cinema.Service.getProgramacoes(codigos);

            var itens = new List<IRLib.Cinema.VendaItem>();
            decimal total = 0;
            ...
            IRLib.Cinema.Service.setRealizaVenda(IRLib.Cinema.SincronizarCinemas.MontarAuth(), new Cinema.Filtros.SetVenda()
            {
                Itens = itens,
                Infos = "Senha=" + senha,
        ...
        private void CancelarVendaCinema(DataRow[] cinema, string guid, string senha, int vendaBilheteriaID)
        {
            ...
            IRLib.Cinema.Service.setCancelaVenda(IRLib.Cinema.SincronizarCinemas.MontarAuth(), new Cinema.Filtros.SetCancela()
            {
                NT_ID = vendaBilheteriaID.ToString(),
                NT_CodBar = senha,
        ...
        public List<IRLib.Assinaturas.Models.Lugar> BuscarLugaresAssinatura(int assinaturaID, int ano, int setorID, int clienteID, int? lugarID)
        {
            BD bd = new BD();
            try
            {
                ...
                List<IRLib.Assinaturas.Models.Lugar> listaLugares = new List<Assinaturas.Models.Lugar>();
                IRLib.Assinaturas.Models.Lugar lugarAux = new Assinaturas.Models.Lugar();
                ...
                do
                {
                    ...
                    {
                        lugarAux = new IRLib.Assinaturas.Models.Lugar();
                        listaLugares.Add(lugarAux);
                        statusAnterior = IRLib.Ingresso.DISPONIVEL;
        ...
        public List<IRLib.Assinaturas.Models.Lugar> BuscarLugaresBancoIngresso(int apresentacaoID, int setorID, int clienteID)
        {
            BD bd = new BD();
            try
            {
        ...
        public List<IRLib.Assinaturas.Models.Lugar> BuscarLugaresBancoIngresso(int apresentacaoID, int setorID, int clienteID, int assinaturaTipoID)
        {
            BD bd = new BD();
            try
            {
                string sql =
                   string.Format(@"
						...
						WHERE i.ApresentacaoID = {0} AND i.SetorID = {1}  --AND asn.AssinaturaTipoID = {2}
						ORDER BY l.ID
						
						", apresentacaoID, setorID, assinaturaTipoID);
                ...
                var v = listaLugares.Where(x => x.S != "I").ToList();
