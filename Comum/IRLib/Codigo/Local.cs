...
namespace IRLib
{
    ...
    public class Local : Local_B
    {
        ...
        public override bool Inserir()
        {
            ...
            try
            {
                ...
                sql.Append("SELECT MAX(ID) AS ID FROM cLocal (NOLOCK)");
                ...
                RetiradaBilheteria, PaisID, ImagemInternet, CodigoPraca, Alvara, AVCB, FonteImposto, PorcentagemImposto, DataEmissaoAlvara, DataValidadeAlvara, DataEmissaoAvcb, DataValidadeAvcb, Lotacao, Ativo) ");
                ...
                sql.Replace("@035", this.Ativo.ValorBD);
        ...
        public override bool Atualizar()
        {
            try
            {
                ...
                Alvara = '@025', AVCB = '@026', FonteImposto = '@027', PorcentagemImposto = @028, DataEmissaoAlvara = '@029', DataValidadeAlvara = '@030', DataEmissaoAvcb = '@031', DataValidadeAvcb = '@032', Lotacao = @033, Ativo = '@034'");
                ...
                sql.Replace("@034", this.Ativo.ValorBD);
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable CarregarLocalEmpresaRegional(string filtro, int paginaAtual, int totItemPorPagina, int regionalID, out int totalRegistros, bool Ativos = false)
        {
            string filtroIDs = "";

            if (regionalID != 0)
                filtroIDs = "(tRegional.ID = @regionalID) AND (tEmpresa.Nome like '%' + @filtro + '%' OR tLocal.Nome  like '%' + @filtro + '%')";
            else
                filtroIDs = "(tRegional.Nome like '%' + @filtro + '%' OR tEmpresa.Nome like '%' + @filtro + '%' OR tLocal.Nome  like '%' + @filtro + '%')";

            if (Ativos)
                if (filtroIDs == string.Empty)
                    filtroIDs += " tLocal.Ativo = 'T' AND tEmpresa.Ativo = 'T' ";
                else
                    filtroIDs += " AND tLocal.Ativo = 'T' AND tEmpresa.Ativo = 'T' ";
            ...
            string query = @"SELECT * FROM (
                                     SELECT ROW_NUMBER() OVER(ORDER BY tRegional.ID) AS RowNumber,
                                        ...
                                        ORDER BY RegionalId;
                                        SELECT COUNT(tLocal.ID) as totalRegistros FROM tLocal (NOLOCK)
                                                        inner join tEmpresa (NOLOCK) on tLocal.EmpresaID = tEmpresa.ID
                                                        inner join tRegional (NOLOCK) on tEmpresa.RegionalID = tRegional.ID 
                                                        where" + filtroIDs + @";";
        ...
        public override void Ler(int id)
        {
            try
            {
                string sql = "SELECT * FROM tLocal (NOLOCK) WHERE ID = " + id;
                bd.Consulta(sql);
        ...
        public void LerEmpresaNome(int id)
        {
            ...
            try
            {
                ...
                string sql = "SELECT ID,EmpresaID,Nome FROM tLocal (NOLOCK) WHERE ID = " + id;
                bd.Consulta(sql);
        ...
        public DataTable AtivosComEmpresaRegional()
        {
            try
            {
        ...
        /// <summary>		
        /// Obter todos os locais ativos por regionalId
        /// </summary>
        /// <returns></returns>
        public DataTable AtivosPorRegionalID(int regionalID)
        {
            ...
            try
            {
                ...
                string sql = "SELECT tLocal.ID, tLocal.Nome , tEmpresa.ID AS EmpresaID " +
                            "FROM tLocal (NOLOCK) " +
                            "INNER JOIN tEmpresa (NOLOCK) ON tEmpresa.ID = tLocal.EmpresaID " +
                            "WHERE tEmpresa.RegionalID = " + regionalID + " AND tLocal.Ativo = 'T' " +
                            "ORDER BY tLocal.Nome";
        ...
        public void CarregaEventosEmTemp(int localID, string sessionID)
        {
            try
            {
                ...
                sql =
                    ...
                    @"' FROM tEvento (NOLOCK) AS e, tApresentacao AS a 
        ...
        public override DataTable Cortesias()
        {
            ...
            try
            {
                ...
                string sql = "SELECT tCortesia.ID,tCortesia.Nome,tCor.RGB " +
                    ...
                    "LEFT JOIN tCor(NOLOCK) on tCor.ID = tCortesia.CorID " +
        ...
        /// <param name="registroZero">Contehudo do registro Zero</param>
        /// <returns></returns>
        public DataTable SetoresAtivosLugarMarcado(string registroZero)
        {
            ...
            try
            {
                ...
                string sql = "SELECT ID, Nome, LugarMarcado, AprovadoPublicacao, IsNull(VersaoBackground, 0) AS VersaoBackground " +
                    "FROM tSetor (NOLOCK) WHERE Ativo='T' AND LocalID=" + this.Control.ID + " AND " +
                    "LugarMarcado <> '" + Setor.Pista + "' ORDER BY Nome";
        ...
        public DataTable SetoresAtivos(string registroZero)
        {
            ...
            try
            {
                ...
                string sql = "SELECT ID,Nome,Produto,LugarMarcado " +
                    "FROM tSetor " +
                    "WHERE LocalID=" + this.Control.ID + " AND Ativo = 'T' ORDER BY Nome";
        ...
        public DataTable EventosAtivos(string registroZero, Apresentacao.Disponibilidade disponibilidade, int localID)
        {
            ...
            try
            {
                ...
                string sql = "SELECT DISTINCT e.ID, e.Nome, e.TipoCodigoBarra " +
                    "FROM tEvento AS e (NOLOCK) INNER JOIN tApresentacao AS a (NOLOCK) ON e.ID = a.EventoID " +
                    "WHERE e.Ativo = 'T' AND e.LocalID = " + localID +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " ORDER BY e.Nome";
        ...
        public object AtivosComEmpresa(int EmpresaID, string RegistroZero)
        {
            try
            {
                DataTable tabela = new DataTable("Local");
        ...
        public DataTable Apresentacoes(int paginaAtual, int totItemPorPagina, out int totalRegistros)
        {
            try
            {
                string diaSemanaImp;
                ...
                tabela.Columns.Add("DiaSemana", typeof(string));
                tabela.Columns.Add("Horario", typeof(string));

                string sql = @"SELECT * FROM (
                                     SELECT ROW_NUMBER() OVER(ORDER BY tApresentacao.ID) AS RowNumber,
                                                                tApresentacao.Horario,
                                                                tApresentacao.ID 
														FROM tApresentacao (NOLOCK)
                                                        INNER JOIN tEvento (NOLOCK) ON tApresentacao.EventoID = tEvento.ID
                                                        WHERE tEvento.LocalID = @LocalID
                                       ) AS TBL
                                        WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage)
                                        ORDER BY ID;
                                        SELECT COUNT(tApresentacao.ID) as totalRegistros FROM tApresentacao (NOLOCK)
                                                        INNER JOIN tEvento (NOLOCK) ON tApresentacao.EventoID = tEvento.ID
                                                        WHERE tEvento.LocalID = @localID";
                ...
                DataSet dsResult = bd.QueryToDataSet(sql, parametros);
                ...
                foreach (DataRow row in dtResult.Rows)
                {
                    ...
                    DateTime dataEvento = this.LerDateTime(row["Horario"].ToString());
                    ...
                    linha["ID"] = row["ID"];
                    linha["Data"] = String.Format("{0:dd/MM/yyyy}", dataEvento);
                    linha["DiaSemana"] = diaSemanaImp;
                    linha["Horario"] = String.Format("{0:HH:mm}", dataEvento);
                    tabela.Rows.Add(linha);
                }
                bd.Fechar();
        ...
        public DataTable ApresentacoesCotas()
        {
            try
            {
                string diaSemanaImp;
                ...
                string sql = String.Format(@"
                    ...
                    WHERE tEvento.LocalID = {0}
                    ORDER BY tApresentacao.Horario", this.Control.ID);
        ...
        public void AtualizarCoordenadas()
        {
            try
            {
                ...
                {
                    GeographicPosition coordenadas = IRLib.CEP.BuscarCoordenadas(item.CEP.ToCEP(), item.Endereco, item.Cidade, item.Estado);
        ...
        public static int getEmpresaID(int localID, BD bd = null)
        {
            var fecharBD = false;
            try
            {
                ...
                return Convert.ToInt32(bd.ConsultaValor("SELECT EmpresaID FROM tLocal(NOLOCK) WHERE ID = @LocalID", new SqlParameter[] { new SqlParameter("@LocalID", localID) }));
            }
            finally
            {
                if (fecharBD)
        ...
        public DateTime LerDateTime(string datahora)
        {
            DateTime retorno;
        ...
        public bool TemEventosComplementar(int localID)
        {
            return (new Evento()).BuscarEventosComplementar(localID).Rows.Count > 0;
        }

        public bool PodeGerarEvento(int localID)
        {
            bool retorno = false;

            try
            {
                string strSql = string.Format("exec proc_LocalPermiteCadastrarEvento {0}", localID);

                bd.Consulta(strSql);

                while (bd.Consulta().Read())
                {
                    retorno = bd.LerInt("PodeCriarEvento") > 0;
                };
            }
            catch (Exception ex)
            {
    ...
    public class LocalLista : LocalLista_B
    {
        ...
        public DataTable LocaisAtivosDisponivelAjuste()
        {
            //Retorna um DataTable com os Locais que tenham pelo menos
            //uma apresentacao com DisponivelAjuste = T
            try
            {
                ...
                string sql = "SELECT " +
                            ...
                            "INNER JOIN tEvento te ON te.LocalID=tl.ID " +
                            "INNER JOIN tApresentacao ta ON ta.EventoID=te.ID " +
                            "WHERE ta.DisponivelAjuste='T' AND tl.Ativo = 'T' ORDER BY tl.Nome ";
        ...
        public DataTable LocaisAtivosDisponivelAjuste(int regionalID)
        {
            //Retorna um DataTable com os Locais que tenham pelo menos
            //uma apresentacao com DisponivelAjuste = T da regionalID definida
            try
            {
                ...
                string sql = "SELECT " +
                            ...
                            "INNER JOIN tApresentacao ta ON ta.EventoID=te.ID " +
                            "INNER JOIN tEmpresa(NOLOCK) ON tEmpresa.ID = tl.EmpresaID " +
                            "WHERE ta.DisponivelAjuste='T' AND tl.Ativo='T' AND tEmpresa.RegionalID = " + regionalID + " ORDER BY tl.Nome ";
