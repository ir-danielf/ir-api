...
namespace IRLib
{
    ...
    public class Evento : Evento_B
    {
        ...
        public override bool Atualizar()
        {
            ...
            try
            {
                ...
                sql.Append("   EXEC dbo.salvar_tEvento_Producao @UsuarioID = " + this.Control.UsuarioID.ToString() + ", @EventoID = @ID, @TimeStamp ='" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + "', " +
                               ...
                               "@DataEmissaoAlvara = @057, @DataValidadeAlvara = @058, @DataEmissaoAvcb = @059, @DataValidadeAvcb = @060, @Lotacao = @061, @Ativo = @062");
                ...
                sql.Replace("@062", "'" + this.Ativo.ValorBD + "'");
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable CarregarEventoEmpresaRegional(string filtro, int paginaAtual, int totItemPorPagina, int regionalID, int localId, out int totalRegistros, bool Ativos = false)
        {

            string filtroIDs = "";
            if (localId != 0)
                filtroIDs = "(tLocal.ID = @localID) AND (tEvento.Nome like '%' + @filtro + '%' OR tLocal.Nome LIKE '%' + @filtro + '%')";
            else if (regionalID != 0)
                filtroIDs = "(tRegional.ID = @regionalID) AND (tEmpresa.Nome like '%' + @filtro + '%' OR tLocal.Nome  like '%' + @filtro + '%' OR tEvento.Nome like '%' + @filtro + '%')";
            else
                filtroIDs = "(tRegional.Nome like '%' + @filtro + '%' OR tEmpresa.Nome like '%' + @filtro + '%' OR tLocal.Nome  like '%' + @filtro + '%' OR tEvento.Nome like '%' + @filtro + '%')";

			if (Ativos)
                filtroIDs += " AND tEvento.Ativo = 'T' AND tLocal.Ativo = 'T' AND tEmpresa.Ativo = 'T' ";


            string query = @"SELECT * FROM (
                                     SELECT ROW_NUMBER() OVER(ORDER BY tRegional.ID) AS RowNumber,
                                                                tRegional.ID AS RegionalId,
																tRegional.Nome AS [Nome Regional],
																tEmpresa.Nome AS [Nome Empresa],
																tLocal.Nome AS [Nome Local],
																tEvento.Nome AS [Nome Evento],
																tApresentacao.CalcHorario AS [Calculo Horario],
																tApresentacao.Horario AS Horario,
                                                                tLocal.EmpresaID,
                                                                tApresentacao.EventoID,
                                                                tEvento.LocalID
														FROM tEvento (NOLOCK) INNER JOIN
														tLocal (NOLOCK) ON tEvento.LocalID = tLocal.ID INNER JOIN
														tEmpresa (NOLOCK) ON tLocal.EmpresaID = tEmpresa.ID INNER JOIN
														tRegional (NOLOCK) ON tEmpresa.RegionalID = tRegional.ID INNER JOIN
														tApresentacao (NOLOCK) ON tEvento.ID = tApresentacao.EventoID
                                                        where" + filtroIDs + @" AND tApresentacao.ID = (Select Top 1 ID from tApresentacao where EventoID = tEvento.ID 
														order by CalcHorario)
                                       ) AS TBL
                                        WHERE RowNumber BETWEEN ((@PageNumber - 1) * @RowspPage + 1) AND (@PageNumber * @RowspPage)
                                        ORDER BY RegionalId;
                                        SELECT COUNT(tEvento.ID) as totalRegistros FROM tEvento (NOLOCK) INNER JOIN
														tLocal (NOLOCK) ON tEvento.LocalID = tLocal.ID INNER JOIN
														tEmpresa (NOLOCK) ON tLocal.EmpresaID = tEmpresa.ID INNER JOIN
														tRegional (NOLOCK) ON tEmpresa.RegionalID = tRegional.ID INNER JOIN
														tApresentacao (NOLOCK) ON tEvento.ID = tApresentacao.EventoID
                                                        where" + filtroIDs + "AND tApresentacao.ID = (Select Top 1 ID from tApresentacao where EventoID = tEvento.ID order by CalcHorario)";
        ...
        public override bool Inserir(CTLib.BD database)
        {
            ...
            sql.Append(@"INSERT INTO tEvento(LocalID, Nome, VendaDistribuida, VersaoImagemIngresso,
                                ...
                                EscolherLugarMarcado, MapaEsquematicoID,PalavraChave, TipoImpressao, TipoCodigoBarra, FilmeID, LimiteMaximoIngressosEvento, CodigoPos, HabilitarRetiradaTodosPDV, VendaSemAlvara, Ativo ) ");
            ...
            sql.Replace("@047", this.Ativo.ValorBD);
        ...
        public EstruturaEvento NovaEstrutura(DataSet info, int empresaID)
        {
            ...
            try
            {
                ...
                this.MenorPeriodoEntrega(bdNovaEsttrutura);
        ...
        public int EmpresaID()
        {
            try
            {
                ...
                string sql =
                    "SELECT     tEmpresa.ID, tEmpresa.Nome " +
                    "FROM       tEmpresa(NOLOCK) INNER JOIN " +
                    "tLocal (NOLOCK) ON tEmpresa.ID = tLocal.EmpresaID INNER JOIN " +
                    "tEvento (NOLOCK) ON tLocal.ID = tEvento.LocalID " +
                    "WHERE      (tEvento.ID = " + this.Control.ID + ") ";
                bd.Consulta(sql);
                if (bd.Consulta().Read())
                {
        ...
        public DataTable Setores(string registroZero)
        {
            try
            {
                ...
                string apres = apresentacoes.Tamanho == 0 ? "" : "WHERE tas.ApresentacaoID in (" + apresentacoes + ")";

                string sql = string.Format("SELECT DISTINCT s.ID, s.Nome " +
                    "FROM tApresentacaoSetor as tas " +
                    "INNER JOIN tSetor as s ON tas.SetorID=s.ID {0} " +
                    "ORDER BY s.Nome", apres);
        ...
        /// <summary>		
        /// Obter todos os setores ativos desse evento
        /// </summary>
        /// <returns></returns>
        public DataTable SetoresAtivos()
        {
            try
            {
                DataTable tabela = new DataTable("Setor");
                ...
                string sql = "SELECT DISTINCT tSetor.ID, tSetor.Nome " +
                    ...
                    "INNER JOIN tSetor (NOLOCK) ON tApresentacaoSetor.SetorID=tSetor.ID " +
                    "INNER JOIN tApresentacao (NOLOCK) ON tApresentacao.ID=tApresentacaoSetor.ApresentacaoID " +
                    "WHERE tSetor.Ativo = 'T' AND tApresentacao.EventoID=" + this.Control.ID + " " +
                    "ORDER BY tSetor.Nome";
        ...
        public DataTable SetoresAtivos(string registroZero)
        {
            try
            {
                ...
                string apres = apresentacoes.Tamanho == 0 ? "" : "AND tas.ApresentacaoID in (" + apresentacoes + ")";

                string sql = string.Format("SELECT DISTINCT s.ID, s.Nome " +
                    "FROM tApresentacaoSetor as tas " +
                    "INNER JOIN tSetor as s ON tas.SetorID=s.ID " +
                    "WHERE s.Ativo = 'T' {0} " +
                    "ORDER BY s.Nome", apres);
        ...
        /// <returns></returns>
        public DataTable Apresentacoes(string registroZero, bool filtraDisponivelAjuste = true)
        {
            ...
            try
            {
                ...
                tabela.Columns.Add("ID", typeof(int));
                tabela.Columns.Add("Horario", typeof(string));
                tabela.Columns.Add("HorarioComSemana", typeof(string));

                if (registroZero != null)
                    tabela.Rows.Add(new Object[] { 0, registroZero, registroZero });



                StringBuilder strBuilder = new StringBuilder();
                strBuilder.Append("SELECT ID, ");
                strBuilder.Append("Horario ");
				strBuilder.Append("FROM tApresentacao (NOLOCK) ");
                strBuilder.Append("WHERE EventoID = @EventoID ");
                if (filtraDisponivelAjuste)
                {
                    strBuilder.Append("AND DisponivelAjuste = 'T'");
                }

				strBuilder.Append(" ORDER BY Horario");

                List<SqlParameter> parametros = new List<SqlParameter>();
                parametros.Add(new SqlParameter(){ DbType = DbType.Int32, ParameterName="@EventoID", Value=this.Control.ID});
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha["ID"] = bd.LerInt("ID");
                    linha["Horario"] = bd.LerStringFormatoDataHora("Horario");
                    linha["HorarioComSemana"] = bd.LerStringFormatoSemanaDataHora("Horario");
                    tabela.Rows.Add(linha);
                }
                bd.Fechar();
        ...
        /// <summary>
        /// Obter canais ativos desse evento de uma empresa
        /// </summary>
        /// <returns></returns>
        public DataTable CanaisAtivos(int empresaid)
        {
            ...
            try
            {
                ...
                string sql = "SELECT ca.ID,ca.Nome FROM tEvento AS e " +
                    "INNER JOIN tCanalEvento AS ce ON ce.EventoID=e.ID " +
                    "INNER JOIN tCanal AS ca ON ca.ID=ce.CanalID  " +
                    "WHERE ca.Ativo = 'T' AND ca.EmpresaID=" + empresaid + " AND " +
                    "ce.EventoID=" + this.Control.ID + " ORDER BY ca.Nome";
        ...
        public List<IRLib.ClientObjects.EstruturaIDNome> EventoPorLocal(int localID, bool registroZero)
        {
            try
            {
        ...
        public void MenorPeriodoEntrega(BD bd, int eventoID = 0)
        {
            if (eventoID == 0)
            ...
            int dias = Convert.ToInt32(bd.ConsultaValor(@"SELECT
	                                ...
	                                WHERE e.ID = @EventoID", new SqlParameter("@EventoID", eventoID)));

            bd.ExecutarComParametros("UPDATE tEvento SET MenorPeriodoEntrega = @Dias WHERE ID = @EventoID", new SqlParameter("@EventoID", eventoID), new SqlParameter("@Dias", dias));
        ...
        public List<IRLib.ClientObjects.EstruturaIDNome> ListaEventoPorCategoria(int EventoTipoID)
        {
            try
            {
        ...
        public List<EstruturaEventoApresentacao> GetEventoListaPos(int CanalID)
        {
            try
            {
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    {
                        EventoID = bd.LerInt("ID"),
                        EventoNome = bd.LerString("Nome").RemoveAcentos(),
                        Cidade = bd.LerString("Cidade").RemoveAcentos(),
                        Estado = bd.LerString("Estado"),
                        Local = bd.LerString("Local").RemoveAcentos(),
                        Endereco = bd.LerString("Endereco").RemoveAcentos() + " " + bd.LerString("Numero"),
                        Bairro = bd.LerString("Bairro").RemoveAcentos(),
        ...
        public EstruturaEventoApresentacao GetEventoPrecosListaPos(int EventoID)
        {
            try
            {
                ...
                retorno.EventoNome = bd.LerString("Nome").RemoveAcentos();
                retorno.Horario = bd.LerString("Apresentacao");
                retorno.Cidade = bd.LerString("Cidade").RemoveAcentos();
                retorno.Estado = bd.LerString("Estado");
                retorno.Local = bd.LerString("Local").RemoveAcentos();
                retorno.Endereco = bd.LerString("Endereco").RemoveAcentos() + " " + bd.LerString("Numero");
                retorno.Bairro = bd.LerString("Bairro").RemoveAcentos();
                ...
                do
                {
                    retorno.EstruturaPreco.Add(new EstruturaEventoPrecos()
                    {
                        ...
                        Nome = bd.LerString("Setor").RemoveAcentos() + " - " + bd.LerString("NomePreco").RemoveAcentos(),
            ...
            try
            {
                ...
                while (bd.Consulta().Read())
                {
                    if (retorno.Where(c => c.EventoID == bd.LerInt("ID")).Count() == 0)
                    {
                        ...
                        estruturaevento.EventoNome = bd.LerString("Nome").RemoveAcentos();
                        estruturaevento.Cidade = bd.LerString("Cidade").RemoveAcentos();
                        estruturaevento.Estado = bd.LerString("Estado");
                        estruturaevento.Local = bd.LerString("Local").RemoveAcentos();
                        estruturaevento.Endereco = bd.LerString("Endereco").RemoveAcentos() + " " + bd.LerString("Numero");
                        estruturaevento.Bairro = bd.LerString("Bairro").RemoveAcentos();
                    ...
                    estruturaPreco.Nome = (DateTime.ParseExact(bd.LerString("Apresentacao"), "yyyyMMddHHmmss", System.Globalization.CultureInfo.InvariantCulture)).ToString("dd \\de MMMM \\de yyyy") + " - " + bd.LerString("Nome").RemoveAcentos();
        ...
        public DataTable TodosPorLocalComApresentacaoDisponivelAjuste(int LocalID, string registroZero)
        {
            try
            {
                ...
                string sql = String.Format(@"SELECT DISTINCT tEvento.ID,tEvento.Nome FROM tEvento (NOLOCK)
                                             ...
                                             WHERE LocalID = {0} ORDER BY Nome", LocalID);
        ...
        public DataTable AtivosPorLocal(int LocalID, string registroZero)
        {
            try
            {
                ...
                string sql = String.Format(@"SELECT ID,Nome FROM tEvento (NOLOCK) WHERE LocalID = {0} AND Ativo = 'T' ORDER BY Nome", LocalID);
        ...
        public DataTable AtivosPorLocalComApresentacaoDisponivelAjuste(int LocalID,string registroZero)
        {
            try
            {
                ...
                string sql = String.Format(@"SELECT DISTINCT tEvento.ID,tEvento.Nome 
                                             FROM tEvento (NOLOCK)
                                             INNER JOIN tApresentacao(NOLOCK) ON tApresentacao.EventoID = tEvento.ID AND tApresentacao.DisponivelAjuste = 'T' 
                                             WHERE LocalID = {0} AND Ativo = 'T' ORDER BY Nome", LocalID);
        ...
        public void DistribuirEventoIR(BD bd, int eventoID = 0)
        {
            if (eventoID == 0)
                eventoID = this.Control.ID;
            var sql = @"INSERT INTO tCanalEvento(CanalID,EventoID,TaxaConveniencia,TaxaMinima,TaxaMaxima,TaxaComissao,ComissaoMinima,ComissaoMaxima)
                        SELECT tc.ID,@EventoID,TaxaConveniencia,TaxaMinima,TaxaMaxima,TaxaComissao,ComissaoMinima,ComissaoMaxima FROM tCanal tc
                        INNER JOIN tEmpresa te ON te.ID = tc.EmpresaID
                        WHERE
                        te.EmpresaVende = 'T' AND te.EmpresaPromove = 'F'";
            bd.ExecutarComParametros(sql, new SqlParameter("@EventoID", eventoID));
        }

        public void DistribuirParaCanais(BD bd, int eventoID, List<int> canais)
        {
            var sql = @"INSERT INTO tCanalEvento(CanalID,EventoID,TaxaConveniencia,TaxaMinima,TaxaMaxima,TaxaComissao,ComissaoMinima,ComissaoMaxima)
                        SELECT tc.ID,@EventoID,TaxaConveniencia,TaxaMinima,TaxaMaxima,TaxaComissao,ComissaoMinima,ComissaoMaxima FROM tCanal tc
                        WHERE
                        tc.ID IN (" + Utilitario.ArrayToString(canais.ToArray()) + ")";
        ...
        public DataTable BuscarEventosComplementar(int localID)
        {

            DataTable tabela = new DataTable("EventoComplementar");

            try
            {
                tabela.Columns.Add("ID", typeof(int));
                tabela.Columns.Add("Nome", typeof(string));
                tabela.Columns.Add("CompletarCadastro", typeof(bool));

                string sql = String.Format(@"SELECT ID, Nome, CompletarCadastro
                                               FROM tEvento (NOLOCK)
                                              WHERE LocalID = {0} 
                                                AND CompletarCadastro = 1 
                                           ORDER BY Nome", localID);
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha["ID"] = bd.LerInt("ID");
                    linha["Nome"] = bd.LerString("Nome");
                    linha["CompletarCadastro"] = bd.LerInt("CompletarCadastro") > 0;
                    tabela.Rows.Add(linha);
                }
    ...
    public class EventoLista : EventoLista_B
    {
        ...
        public void CarregarApenasSelecionadoDisponivelAjuste(int eventoID)
        {

            try
            {

                string sql = @"SELECT DISTINCT tEvento.ID, tEvento.Nome, tApresentacao.DisponivelAjuste
                                FROM tEvento(NOLOCK)
                                Inner Join tApresentacao(NOLOCK) ON tApresentacao.EventoID = tEvento.ID  WHERE EventoID = " + eventoID + " AND DisponivelAjuste = 'T' ORDER BY tEvento.Nome ";
        ...
        public DataTable TabelaAtivosDisponivelAjuste()
        {
            ...
            try
            {
                ...
                sql = "SELECT DISTINCT tEvento.ID, tEvento.Nome " +
                      "FROM tEvento(NOLOCK) INNER JOIN tApresentacao(NOLOCK) ON EventoID = tEvento.ID " +
                      "WHERE tEvento.Ativo = 'T' AND DisponivelAjuste = 'T' ORDER BY Nome ";
        ...
        public DataTable TabelaAtivosDisponivelAjuste(int regionalID)
        {
