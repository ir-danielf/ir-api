...
namespace IRLib
{
    ...
    public abstract class IngressoCliente_B : BaseBD
    {
        ...
        /// <summary>
        /// Preenche todos os atributos de IngressoCliente a partir do ID do Ingresso, para Troca de Preços
        /// </summary>
        /// <param name="IngressoID">Informe o ID</param>
        /// <returns></returns>
        public void ObterInfosParaTroca(int IngressoID)
        {

            try
            {

                string sql = "SELECT * FROM tIngressoCliente WHERE IngressoID = " +
                    IngressoID + " AND ID = (SELECT MAX(ID) FROM tIngressoCliente WHERE IngressoID = " + IngressoID + ")";
                bd.Consulta(sql);
