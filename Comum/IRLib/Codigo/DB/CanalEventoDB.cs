...
namespace IRLib
{
    ...
    public abstract class CanalEvento_B : BaseBD
    {
        ...
        public bool AtualizarCanaisEvento(string canaisSelecionados, int eventoID, int usuarioID, int empresaID)
        {
            StringBuilder sql = new StringBuilder();

            //OBS: formato da string canaisSelecionados: "ID, ID, ID, ID..."
            bd.IniciarTransacao();
            DataTable dt = new DataTable();

            DataColumn ID = new DataColumn();
            ID.DataType = System.Type.GetType("System.Int32");
            ID.ColumnName = "ProductID";
            ID.AutoIncrement = true;

            dt.Columns.Add(ID);
            dt.Columns.Add("CanalID", typeof(int));
            dt.Columns.Add("EventoID", typeof(int));
            dt.Columns.Add("TaxaConveniencia", typeof(decimal));
            dt.Columns.Add("TaxaMinima", typeof(decimal));
            dt.Columns.Add("TaxaMaxima", typeof(decimal));
            dt.Columns.Add("TaxaComissao", typeof(decimal));
            dt.Columns.Add("ComissaoMinima", typeof(decimal));
            dt.Columns.Add("ComissaoMaxima", typeof(decimal));

            sql.Append("SELECT ID AS CanalID, " + eventoID + " AS EventoID, TaxaConveniencia, TaxaMinima, TaxaMaxima, TaxaComissao, ComissaoMinima, ComissaoMaxima FROM tCanal WHERE ID IN (");
            sql.Append(canaisSelecionados);
            sql.Append(")");
            ...
            //Antes de inserir exclui todos os canais próprios desse evento para poder fazer uma nova inclusão
            sql.Clear();
            sql.Append("ExcluirCanalEvento ");
            sql.Append("@EventoID = " + eventoID);
            sql.Append(", ");
            ...
            if (!result)
            {
                // Faz a inserção em massa dos dados
                using (var bulkCopy = new SqlBulkCopy(System.Configuration.ConfigurationManager.AppSettings["conexao"].ToString(), SqlBulkCopyOptions.KeepIdentity))
                {
                    bulkCopy.DestinationTableName = "tCanalEvento";

                    //bulkCopy.ColumnMappings.Add("ID", "ID");
                    bulkCopy.ColumnMappings.Add("CanalID", "CanalID");
                    bulkCopy.ColumnMappings.Add("EventoID", "EventoID");
                    bulkCopy.ColumnMappings.Add("TaxaConveniencia", "TaxaConveniencia");
                    bulkCopy.ColumnMappings.Add("TaxaMinima", "TaxaMinima");
                    bulkCopy.ColumnMappings.Add("TaxaMaxima", "TaxaMaxima");
                    bulkCopy.ColumnMappings.Add("TaxaComissao", "TaxaComissao");
                    bulkCopy.ColumnMappings.Add("ComissaoMinima", "ComissaoMinima");
                    bulkCopy.ColumnMappings.Add("ComissaoMaxima", "ComissaoMaxima");
        ...
        public bool AtualizarCanaisEventoIR(List<EstruturaCanaisEvento> canaisEvento, int eventoID, int usuarioID, int empresaID, string operacao, string canalTipo)
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                sql.Append("@UsuarioID = " + usuarioID);
                sql.Append(", ");
                sql.Append("@CanalTipo = '" + canalTipo + "'");
                int x = bd.ExecutarScalar(sql.ToString());
                bool result = (x >= 0);

                if (result && operacao == "I")
                {
                    DataColumn ID = new DataColumn();
                    ID.DataType = System.Type.GetType("System.Int32");
                    ID.ColumnName = "ProductID";
                    ...
                    foreach (EstruturaCanaisEvento item in canaisEvento)
                    {
                        dt.Rows.Add(0, item.CanalID, eventoID, item.TaxaConveniencia, item.TaxaMinima, item.TaxaMaxima, item.TaxaComissao, item.ComissaoMinima, item.ComissaoMaxima);
                    }
