...
namespace IRLib
{
    ...
    public abstract class Cliente_B : BaseBD
    {
        ...
        public override void Ler(int id)
        {
            ...
            try
            {
                ...
                if (bd.Consulta().Read())
                {
					...
					this.logCliente(this.Control.ID, this.Control.UsuarioID, 'V');
                }
                else
                {
                    this.Limpar();
		...
		/// <summary>
		/// Esse método grava nas tabelas cCliente e xCliente um hitórico de qualquer intereação com o resgitro
		/// da tabela tCliente, Updates e Selects.
		/// Insertes já é contemplado pelo Método Salvar
		/// </summary>
		/// <param name="ID"> ID do Cliente</param>
		/// <param name="usuarioID"> ID do Usuário logado no sistema</param>
		/// <param name="Acao"> Ação que está sendo feita no Registro
		/// I - Insert 
		/// U - Update 
		/// V - Visualização
		/// D - Delete </param>
		/// <returns></returns>
		protected void logCliente(int ID, int usuarioID, char Acao)
		{
			try
			{
				string sql = @"EXEC dbo.salvaLog_cCliente_xCliente    
                             @DATAHORA = '" + DateTime.Now.ToString("yyyyMMddHHmmssffff") + "', " +
							"@ACAO = '" + Acao.ToString().ToUpper() + "', " +
			...
			{
				throw new Exception("Erro desconhecido ao inserir no banco");
			}
		}
