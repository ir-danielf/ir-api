...
namespace IRLib
{
    ...
    public abstract class CotaItem_B : BaseBD
    {
        ...
        public cotatipo Tipo = new cotatipo();
        ...
        public override void Ler(int id)
        {
            ...
            try
            {
                ...
                if (bd.Consulta().Read())
                {
                    ...
                    this.Tipo.Valor = bd.LerInt("Tipo");
                }
                else
                {
                    this.Limpar();
        ...
        protected void InserirLog()
        {
            ...
            try
            {
                ...
                string strSql = @"INSERT INTO xCotaItem
                                 (ID, Versao, CotaID, PrecoIniciaCom, Quantidade, QuantidadePorCliente, ParceiroID, ValidaBin, TextoValidacao, ObrigatoriedadeID,
                                  CPFResponsavel, Termo, TermoSite, Nominal, QuantidadePorCodigo)
                                  SELECT ID, @V, CotaID, PrecoIniciaCom, Quantidade, QuantidadePorCliente, ParceiroID, ValidaBin, TextoValidacao, ObrigatoriedadeID,
                                  CPFResponsavel, Termo, TermoSite, Nominal, QuantidadePorCodigo
                                  FROM tCotaItem WHERE ID = @I";

                var parametros = new List<SqlParameter>();
                parametros.Add(new SqlParameter() { ParameterName = "@I", Value = this.Control.ID.ToString(), DbType = DbType.String });
                parametros.Add(new SqlParameter() { ParameterName = "@V", Value = this.Control.Versao.ToString(), DbType = DbType.String });

                bd.Executar(strSql,parametros);
        ...
        public override bool Atualizar(BD bd)
        {
            ...
            try
            {
                ...
                StringBuilder sql = new StringBuilder();
                sql.Append("UPDATE tCotaItem SET CotaID = @001, PrecoIniciaCom = '@002', Quantidade = @003, QuantidadePorCliente = @004, ParceiroID = @005, ");
                sql.Append("ValidaBin = '@006', TextoValidacao = '@007', ObrigatoriedadeID = @008, CPFResponsavel = '@009', Termo = '@010', ");
                sql.Append("TermoSite = '@011', Nominal = '@012', QuantidadePorCodigo = @013, Tipo = @014 ");
                sql.Append("WHERE ID = @ID");
                sql.Replace("@ID", this.Control.ID.ToString());
                sql.Replace("@001", this.CotaID.ValorBD);
                sql.Replace("@002", this.PrecoIniciaCom.ValorBD);
        ...
        public class cotatipo : IntegerProperty
        {
