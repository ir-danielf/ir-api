...
namespace IRLib
{
    ...
    public class Cartao : Cartao_B
    {
        ...
        {
            LogUtil.Debug(string.Format("##Cartão.InserindoCartao## CLIENTE {0}", clienteID));
            ...
            try
            {
                ...
                LogUtil.Debug(string.Format("##Cartão.InserindoCartao.SUCCESS## CLIENTE {0}, NOME_CARTAO {1}, CARTAO_ID {2}", clienteID, NomeCartao, this.Control.ID));

                return newCartaoID = this.Control.ID;
            }
            catch (Exception ex)
            {
                LogUtil.Error(string.Format("##Cartão.InserindoCartao.EXCEPTION## CLIENTE {0}, MSG {1}", clienteID, ex.Message), ex);
        ...
        {
            LogUtil.Debug(string.Format("##Cartão.AtualizandoCriptografiaCartao## NOME_CARTÃO {0}", NomeCartao));
        ...
        public EstruturaEstornoCartao BuscaInfosCartaoVenda(int cartaoID)
        {
            try
            {
                EstruturaEstornoCartao EstruturaEstornoCartao = new IRLib.EstruturaEstornoCartao();

                string sql = string.Format("SELECT NroCartao, ISNULL(NomeCartao, '') AS NomeCartao, tb.Nome AS Bandeira FROM tCartao AS tc (NOLOCK) INNER JOIN tBandeira AS tb (NOLOCK) ON tb.ID = tc.BandeiraID WHERE tc.ID = {0}", cartaoID);

                bd.Consulta(sql);

                while (bd.Consulta().Read())
                {
                    EstruturaEstornoCartao.Bandeira = bd.LerString("Bandeira");
                    EstruturaEstornoCartao.NumeroCartao = bd.LerString("NroCartao");
                    EstruturaEstornoCartao.TitularCartao = bd.LerString("NomeCartao");
                }

                return EstruturaEstornoCartao;
            }
            finally
            {
                bd.Fechar();
        ...
        public DataTable BuscaInfosCartaoVendaToTable(int cartaoID)
        {
            DataTable tabela = new DataTable("DadosCartao");
            try
            {
                tabela.Columns.Add("Bandeira", typeof(string));
                tabela.Columns.Add("NumeroCartao", typeof(string));
                tabela.Columns.Add("TitularCartao", typeof(string));

                string sql = string.Format("    SELECT NroCartao, " +
                                           "           ISNULL(NomeCartao, '') AS NomeCartao, " +
                                           "           tb.Nome AS Bandeira " +
                                           "      FROM tCartao AS tc (NOLOCK) " +
                                           "INNER JOIN tBandeira AS tb (NOLOCK) ON tb.ID = tc.BandeiraID " +
                                           "     WHERE tc.ID = {0}", cartaoID);
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha["Bandeira"] = bd.LerString("Bandeira");
                    linha["NumeroCartao"] = bd.LerString("NroCartao");
                    linha["TitularCartao"] = bd.LerString("NomeCartao");
