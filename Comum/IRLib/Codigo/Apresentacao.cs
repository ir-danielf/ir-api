...
namespace IRLib
{
    public class Apresentacao : Apresentacao_B
    {
        ...
        public void DesassociarCotaItemIngresso(int ApresentacaoID, int UsuarioID)
        {
            try
            {
                StringBuilder sql = new StringBuilder();
                sql.Append("EXEC DesassociarCotaItemIngresso @001,@002");
        ...
        public int getApreSentacaoID(int ApresentacaoSetor)
        {
            try
            {
                string sql = string.Format("EXEC sp_GetApresentacaoIDAps {0}", ApresentacaoSetor);
        ...
        internal bool Atualizar(BD bd)
        {
            ...
            try
            {
                ...
                sql.Replace("@018", this.Cancelada.ValorBD);
        ...
        public List<EstruturaManutencaoApresentacaoPeriodo> CarregaListaApresentacaoPeriodo(int regionalID, int empresaID, int localID, int eventoID)
        {
            try
            {
                ...
                strBuilder.Append("SELECT a.ID, ");
                strBuilder.Append("a.Horario ");
                strBuilder.Append("FROM tApresentacao (NOLOCK) AS a ");
                strBuilder.Append("INNER JOIN tEvento (NOLOCK) AS e on e.ID = a.EventoID ");
                strBuilder.Append("INNER JOIN tLocal (NOLOCK) AS l on l.ID = e.LocalID ");
                strBuilder.Append("INNER JOIN tEmpresa (NOLOCK) AS em on em.ID = l.EmpresaID ");
                strBuilder.Append("INNER JOIN tRegional (NOLOCK) AS r on r.ID = em.RegionalID ");

                if (regionalID > 0 || empresaID > 0 || localID > 0 || eventoID > 0)
                {
                    StringBuilder strExtra = new StringBuilder();

                    if (regionalID > 0)
                    {
                        strExtra.Append("WHERE r.ID = @IDregional ");
                        parametros.Add(new SqlParameter() { ParameterName = "@IDregional", Value = regionalID, DbType = DbType.Int32 });
                    }

                    if (empresaID > 0)
                    {
                        if (strExtra.Length.Equals(0))
                            strExtra.Append("WHERE em.ID = @IDEmpresa ");
                        else
                            strExtra.Append("AND em.ID = @IDEmpresa ");

                        parametros.Add(new SqlParameter() { ParameterName = "@IDEmpresa", Value = empresaID, DbType = DbType.Int32 });
                    }

                    if (localID > 0)
                    {
                        if (strExtra.Length.Equals(0))
                            strExtra.Append("WHERE l.ID = @IDLocal ");
                        else
                            strExtra.Append("AND l.ID = @IDLocal ");

                        parametros.Add(new SqlParameter() { ParameterName = "@IDLocal", Value = localID, DbType = DbType.Int32 });
                    }

                    if (eventoID > 0)
                    {
                        if (strExtra.Length.Equals(0))
                            strExtra.Append("WHERE e.ID = @IDEvento ");
                        else
                            strExtra.Append("AND e.ID = @IDEvento ");

                        parametros.Add(new SqlParameter() { ParameterName = "@IDEvento", Value = eventoID, DbType = DbType.Int32 });
                    }
                    strBuilder.Append(strExtra);
                }
        ...
        public List<EstruturaManutencaoApresentacaoPeriodo> CarregaListaApresentacaoPeriodo(int regionalID, int empresaID, int localID, int eventoID, string disponivelRelatorio, string disponivelVenda, string disponivelAjuste, string periodoInicial, string periodoFinal)
        {
            try
            {
                ...
                StringBuilder strBuilder = new StringBuilder();
                strBuilder.Append("SELECT a.ID, l.Nome AS Local, e.Nome AS Evento, a.Horario, a.DisponivelAjuste, ");
                strBuilder.Append("a.DisponivelRelatorio, a.DisponivelVenda, a.Cancelada FROM tApresentacao (NOLOCK) AS a ");
                strBuilder.Append("INNER JOIN tEvento (NOLOCK) AS e on e.ID = a.EventoID ");
                strBuilder.Append("INNER JOIN tLocal (NOLOCK) AS l on l.ID = e.LocalID ");
                strBuilder.Append("INNER JOIN tEmpresa (NOLOCK) AS em on em.ID = l.EmpresaID ");
                strBuilder.Append("INNER JOIN tRegional (NOLOCK) AS r on r.ID = em.RegionalID ");
                ...
                if (regionalID > 0)
                    strBuilder.Append(String.Format("r.ID = {0} ", regionalID));

                if (empresaID > 0)
                {
                    if (regionalID > 0)
                        strBuilder.Append(String.Format("AND em.ID = {0} ", empresaID));
                    else
                        strBuilder.Append(String.Format("em.ID = {0} ", empresaID));
                }

                if (localID > 0)
                {
                    if (empresaID > 0 || regionalID > 0)
                        strBuilder.Append(String.Format("AND l.ID = {0} ", localID));
                    else
                        strBuilder.Append(String.Format("l.ID = {0} ", localID));
                }

                if (eventoID > 0)
                {
                    if (localID > 0 || empresaID > 0 || regionalID > 0)
                        strBuilder.Append(String.Format("AND e.ID = {0} ", eventoID));
                    else
                        strBuilder.Append(String.Format("e.ID = {0} ", eventoID));
                }

                if (!String.IsNullOrEmpty(periodoInicial) || !String.IsNullOrEmpty(periodoFinal) || eventoID > 0 || localID > 0 || empresaID > 0 || regionalID > 0)
                    strBuilder.Append(String.Format("AND a.Horario > '{0}' AND a.Horario < '{1}' ", periodoInicial, periodoFinal));
                else if (!String.IsNullOrEmpty(periodoInicial) || !String.IsNullOrEmpty(periodoFinal))
                    strBuilder.Append(String.Format("a.Horario > '{0}' AND a.Horario < '{1}' ", periodoInicial, periodoFinal));
                else if (String.IsNullOrEmpty(periodoFinal))
                    strBuilder.Append(String.Format("a.Horario > '{0} ", periodoInicial));
                else if (String.IsNullOrEmpty(periodoInicial))
                    strBuilder.Append(String.Format("a.Horario < '{1}' ", periodoFinal));
                ...
                while (bd.Consulta().Read())
                {
                    retorno.Add(new EstruturaManutencaoApresentacaoPeriodo()
                    {
                        ...
                        Cancelada = bd.LerBoolean("Cancelada")
                    });
                }
        ...
        public bool AlterarCotaApresentacao(int cotaID)
        {
            try
            {
                bd.IniciarTransacao();
                ...
                StringBuilder sql = new StringBuilder();
                sql.Append("UPDATE tApresentacao SET CotaID = @001");
                sql.Append(" WHERE ID = @ID ");

                if (cotaID == 0)
                    sql.Replace("@001", "NULL");
                else
                    sql.Replace("@001", cotaID.ToString());
                ...
                oControle.ApresentacaoID.Valor = this.Control.ID;
        ...
        public int DesabilitaAutomatico(TipoDesabilitaAutomatico tipo)
        {
            ...
            List<ClientObjects.EstruturaApresentacao> lstApresentacoes = new List<IRLib.ClientObjects.EstruturaApresentacao>();
            ...
            try
            {
                ...
                using (IDataReader oDataReader = bdAutomatico.Consulta(strSQL))
                {
                    ...
                    {
                        apresentacao = new IRLib.ClientObjects.EstruturaApresentacao();
        ...
        public bool SalvarListagem(List<IRLib.ClientObjects.ApresentacaoManutencaoListagem> listagem)
        {
            bd.IniciarTransacao();
            StringBuilder sql = new StringBuilder();
            ...
            try
            {
                foreach (ApresentacaoManutencaoListagem item in listagem)
                {
                    sql.Append(String.Format("UPDATE tApresentacao SET DisponivelAjuste = '{0}'", item.DisponivelAjuste ? "T" : "F"));
                    sql.Append(String.Format(", DisponivelRelatorio = '{0}'", item.DisponivelRelatorio ? "T" : "F"));
                    sql.Append(String.Format(", DisponivelVenda = '{0}'", item.DisponivelVenda ? "T" : "F"));

                    if (item.Horario > DateTime.MinValue)
                    {
                        StringBuilder strBuilder = new StringBuilder();
                        strBuilder.Append(item.Horario.Year.ToString());
                        strBuilder.Append(item.Horario.Month.ToString().Length.Equals(1) ? "0" + item.Horario.Month.ToString() : item.Horario.Month.ToString());
                        strBuilder.Append(item.Horario.Day.ToString().Length.Equals(1) ? "0" + item.Horario.Day.ToString() : item.Horario.Day.ToString());
                        strBuilder.Append(item.Horario.Hour.ToString().Length.Equals(1) ? "0" + item.Horario.Hour.ToString() : item.Horario.Hour.ToString());
                        strBuilder.Append(item.Horario.Minute.ToString().Length.Equals(1) ? "0" + item.Horario.Minute.ToString() : item.Horario.Minute.ToString());
                        strBuilder.Append(item.Horario.Second.ToString().Length.Equals(1) ? "0" + item.Horario.Second.ToString() : item.Horario.Second.ToString());

                        sql.Append(String.Format(", Horario = '{0}'", strBuilder.ToString()));
                    }

                    if (item.MapaEsquematico > 0)
                        sql.Append(String.Format(", MapaEsquematicoID = '{0}'", item.MapaEsquematico));

                    if (!String.IsNullOrEmpty(item.Impressao))
                        sql.Append(String.Format(", Impressao = '{0}'", item.Impressao));

                    sql.Append(String.Format(" WHERE ID = '{0}' ", item.ID));
                }

                if (listagem.Count > 0)
                {
                    blnResult = bd.Executar(sql.ToString()) > 0 ? true : false;
                    bd.FinalizarTransacao();
                }
            }
            catch
        ...
        public bool Venda()
        {
            ...
            try
            {
                ...
                string sql = "SELECT case when DisponivelVenda = 'T' and Cancelada = 'F' then 'T' else 'F' end DisponivelVenda  FROM tApresentacao (NOLOCK) " +
        ...
        public AcrescentarPrecoModel ApresentacaoSetores(List<int> apresentacaoIDs)
        {
            try
            {
                AcrescentarPrecoModel model = new AcrescentarPrecoModel();
                model.apresentacaoSetores = new List<ApresentacaoSetorPrecoModel>();
                model.setores = new List<SetorPrecoModel>();

                string sql = @"SELECT tas.ID,
                                      tas.ApresentacaoID,
                                      a.Horario as ApresentacaoHorario,
                                      tas.SetorID, 
                                      s.Nome as SetorNome
                                FROM tApresentacaoSetor (nolock) tas
                          INNER JOIN tSetor (nolock) s ON tas.SetorID = s.ID
                          INNER JOIN tApresentacao (nolock) a ON a.ID = tas.ApresentacaoID
                        WHERE tas.ApresentacaoID in (" + string.Join(",", apresentacaoIDs.ToArray()) + @") 
                    ORDER BY s.Nome";
                ...
                while (bd.Consulta().Read())
                {
                    var item = new ApresentacaoSetorPrecoModel()
                    {
                        ApresentacaoSetorID = bd.LerInt("ID"),
                        ApresentacaoID = bd.LerInt("ApresentacaoID"),
                        ApresentacaNome = bd.LerStringFormatoSemanaDataHora("ApresentacaoHorario"),
                        SetorID = bd.LerInt("SetorID"),
                        SetorNome = bd.LerString("SetorNome")
                    };
                    model.apresentacaoSetores.Add(item);
                    if (model.setores.Find(t => t.SetorID == item.SetorID) == null)
                    {
                        model.setores.Add(new SetorPrecoModel()
                        {
        ...
        public DataTable SetoresAtivos(string registroZero)
        {
            try
            {
                DataTable tabela = new DataTable("Setor");
                ...
                string sql = "SELECT s.ID, s.Nome FROM tApresentacaoSetor as tas INNER JOIN tSetor as s " +
                    "ON tas.SetorID=s.ID WHERE s.Ativo = 'T' AND tas.ApresentacaoID=" + this.Control.ID + " " +
                    "ORDER BY s.Nome";
        ...
        public DataTable SetoresAtivos(int apresentacaoID, Apresentacao.Disponibilidade disponibilidade)
        {
            ...
            try
            {
                ...
                string sql = "SELECT s.ID, s.Nome FROM tSetor as s INNER JOIN tApresentacaoSetor as tas ON tas.SetorID=s.ID " +
                    "INNER JOIN tApresentacao as a ON a.ID=tas.ApresentacaoID " +
                    "WHERE s.Ativo = 'T' AND tas.ApresentacaoID=" + apresentacaoID +
                    disponivelVenda +
                    disponivelAjuste +
                    disponivelRelatorio +
                    " ORDER BY s.Nome";
        ...
        public DataTable ListagemFiltrada(int localID, int eventoID, Nullable<bool> disponivelAjuste, Nullable<bool> disponivelRelatorio, Nullable<bool> disponivelVenda)
        {
            try
            {
                ...
                tabela.Columns.Add("MapaEsquematicoID", typeof(string));
                tabela.Columns.Add("Cancelada", typeof(bool));
                // Obtendo dados

                StringBuilder sql = new StringBuilder();
                sql.Append("SELECT tEmpresa.Nome AS Empresa, ");
                sql.Append("tLocal.Nome AS Local, ");
                sql.Append("tEvento.Nome AS Evento, ");
                sql.Append("tApresentacao.Horario, ");
                sql.Append("tApresentacao.ID, ");
                sql.Append("tEvento.LocalID, ");
                sql.Append("tApresentacao.DisponivelAjuste, ");
                sql.Append("tApresentacao.DisponivelRelatorio, ");
                sql.Append("tApresentacao.DisponivelVenda, ");
                sql.Append("tApresentacao.Obs, ");
                sql.Append("tApresentacao.Impressao, ");
                sql.Append("tApresentacao.MapaEsquematicoID, ");
                sql.Append("tApresentacao.Cancelada ");
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha["MapaEsquematicoID"] = bd.LerString("MapaEsquematicoID");
                    linha["Cancelada"] = bd.LerBoolean("Cancelada");
                    tabela.Rows.Add(linha);
                }
                bd.Fechar();
                return tabela;
        ...
        public DataTable ApresentacoesPorEvento(int eventoID, string registroZero)
        {
            try
            {
                ...
                dttApresentacao.Columns.Add("DataApresentacao", typeof(string));
        ...
        public DataTable ApresentacoesCotasPorEvento(int eventoID, int cotaID, string registroZero = null)
        {
            try
            {
                string diaSemanaImp;
                ...
                dttApresentacao.Columns.Add("DataApresentacao", typeof(string));
                dttApresentacao.Columns.Add("Evento", typeof(string));
                dttApresentacao.Columns.Add("Cota", typeof(int));

                if (registroZero != null)
                    dttApresentacao.Rows.Add(new Object[] { 0, registroZero, registroZero });

                StringBuilder stbSQL = new StringBuilder();
                stbSQL.Append("SELECT tApresentacao.ID, tApresentacao.Horario, tEvento.Nome AS Evento, tCota.ID AS CotaID ");
                stbSQL.Append("FROM tApresentacao (NOLOCK) ");
                stbSQL.Append("LEFT JOIN tCota (NOLOCK) ON tApresentacao.CotaID = tCota.ID ");
                stbSQL.Append("INNER JOIN tEvento (NOLOCK) ON tApresentacao.EventoID = tEvento.ID ");
                stbSQL.Append("WHERE tApresentacao.EventoID = @eventoID ");
                stbSQL.Append("AND (tCota.ID = @cotaID OR tCota.ID IS NULL) ");
                stbSQL.Append("ORDER BY tApresentacao.Horario ");
                ...
                bd.Consulta(stbSQL.ToString(), parametros);
                while (bd.Consulta().Read())
                {
                    DataRow linha = dttApresentacao.NewRow();
        ...
        public DataTable ApresentacoesPorCota(int cotaID, int eventoID)
        {
            try
            {
                string diaSemanaImp;
                ...
                string sql = String.Format(@"SELECT * FROM tApresentacao (NOLOCK) WHERE CotaID = {0} AND EventoID = {1}", cotaID, eventoID);
        ...
        public int DistribuirCotasPorApresentacao(EstruturaCotasDistribuir oDistribuir, bool fecharBD)
        {
            ...
            try
            {
                ...
                foreach (EstruturaCotaItem item in lstReplicar)
                {
                    if (oControle.getQtdVendidaTotalCota(oDistribuir.ApresentacaoID.ToString(), item.ID) > item.quantidade)
                    {
                        throw new Exception("A quantidade deve ser maior a quantidade já vendida");
                    }
                }
        ...
        public bool Inserir(CTLib.BD database)
        {
            ...
            try
            {
                ...
                StringBuilder sql = new StringBuilder();
                sql.Append("INSERT INTO tApresentacao(EventoID, Horario, DisponivelVenda, DisponivelAjuste, DisponivelRelatorio, VersaoImagemIngresso, VersaoImagemVale, VersaoImagemVale2, VersaoImagemVale3, Impressao, DescricaoPadrao, Descricao, MapaEsquematicoID, CodigoProgramacao, Cancelada) ");
                sql.Append("VALUES (@001,'@002','@003','@004','@005',@006,@007,@008,@009,'@010','@012','@013', @014, '@015', '@016') ");
                ...
                sql.Replace("@016", this.Cancelada.ValorBD);
        ...
        public DataTable SetoresPrecos(List<IRLib.ClientObjects.EstruturaApresentacaoRaioX> lstPesquisa)
        {
            DataRow linha;
            ...
            try
            {
                ...
                foreach (IRLib.ClientObjects.EstruturaApresentacaoRaioX itemPesquisa in lstPesquisa)
                {
        ...
        public AcrescentarApresentacaoModel ListarPrecoSetor(int apresentacaoID)
        {

            AcrescentarApresentacaoModel retorno = new AcrescentarApresentacaoModel();
            retorno.precos = new List<PrecoVendaModel>();
            retorno.setores = new List<GerarEventoEtapa1Fase3Model.SetoresNovo>();
            try
            {
                ...
                string sql = @"
                    SELECT s.ID as SetorID,
		                   s.Nome as SetorNome, 
                           s.LugarMarcado as SetorTipo,
                           (SELECT count(*) from tIngresso (nolock) i WHERE i.ApresentacaoSetorID = aset.ID) as SetorQuantidade,
	                       p.ID as PrecoID,
		                   p.Nome as PrecoNome,
		                   p.Valor as PrecoValor,
		                   (CASE WHEN p.Impressao in ('$', 'A') THEN 'T' ELSE 'F' END) as PrecoImprimeValor,
		                   (CASE WHEN p.Impressao in ('N', 'A') THEN 'T' ELSE 'F' END) as PrecoImprimeNome,
		                   (CASE WHEN p.ID = aset.PrincipalPrecoID THEN 'T' ELSE 'F' END) as PrecoPrincipal
                      FROM tApresentacaoSetor (nolock) aset
                INNER JOIN tSetor (nolock) s ON aset.SetorID = s.ID
                INNER JOIN tPreco (nolock) p ON aset.ID = p.ApresentacaoSetorID
                     WHERE aset.ApresentacaoID = " + apresentacaoID + @"
                  ORDER BY s.ID, PrecoPrincipal desc, p.ID
                ";
                ...
                while (bd.Consulta().Read())
                {
                    string precoNome = bd.LerString("PrecoNome");
                    int setorID = bd.LerInt("SetorID");

                    PrecoVendaModel itemPreco = retorno.precos.Find(t => t.Descricao == precoNome && t.SetorValores.All(x => x.SetorID != setorID));
                    GerarEventoEtapa1Fase3Model.SetoresNovo itemSetor = retorno.setores.Find(t => t.SetorID == setorID);

                    if (itemSetor == null)
                    {
                        itemSetor = new GerarEventoEtapa1Fase3Model.SetoresNovo();
                        itemSetor.SetorID = bd.LerInt("SetorID");
                        itemSetor.NomeSetor = bd.LerString("SetorNome");
                        itemSetor.ApresentacaoID = apresentacaoID;
                        itemSetor.Quantidade = bd.LerInt("SetorQuantidade");
                        itemSetor.Tipo = (Setor.Tipo)bd.LerString("SetorTipo")[0];
                        retorno.setores.Add(itemSetor);
                    }

                    if (itemPreco == null)
                    {
                        itemPreco = new PrecoVendaModel();
                        itemPreco.Descricao = precoNome;
                        itemPreco.ImprimeNome = bd.LerBoolean("PrecoImprimeNome");
                        itemPreco.ImprimeValor = bd.LerBoolean("PrecoImprimeValor");
                        itemPreco.Principal = bd.LerBoolean("PrecoPrincipal");
                        itemPreco.SetorValores = new List<SetorPrecoModel>();

                        retorno.precos.Add(itemPreco);
                    }

                    itemPreco.SetorValores.Add(new SetorPrecoModel()
                    {
                        SetorID = itemSetor.SetorID,
                        SetorNome = itemSetor.NomeSetor,
            ...
            try
            {
                ...
                foreach (int apresentacaoID in apresentacoesID)
                {
                    ...
                    foreach (KeyValuePair<int, int> setorQuantidade in setoresXqtde)
                    {
                        ...
                        //Gerar ingressos
                        oSetor.Ler(oApresentacaoSetor.SetorID.Valor);
                        // Verifica se é lugar marcado.
                        if (oSetor.LugarMarcado.Valor == Setor.Pista)
                        {
                            oIngresso.Acrescentar(oApresentacaoSetor.Control.ID, oApresentacao.EventoID.Valor, oApresentacao.Control.ID, oApresentacaoSetor.SetorID.Valor, empresaID, oEvento.LocalID.Valor, 0, qtdeSetor, 1, bd, false, null);
                        }
                        else
                        {
        ...
        public DataTable ListaBranca(List<IRLib.ClientObjects.EstruturaApresentacaoRaioX> lstPesquisa)
        {
        ...
        public DataTable ListaNegra(IRLib.ClientObjects.EstruturaApresentacaoRaioX estrutura)
        {
        ...
        public DataTable ListaNegra(List<IRLib.ClientObjects.EstruturaApresentacaoRaioX> lstPesquisa)
        {
        ...
        public DateTime ApresentacaoMaisProxima(List<int> listaApresentacao)
        {
            try
            {
                ...
                string sql = @" SELECT MIN(horario) as horario
        ...
        public DataTable CarregarSetores(List<int> apresentacoesIDs)
        {
            CTLib.BD bd = new BD();
            try
            {
                string query = @"SELECT Distinct tSetor.ID,nome FROM tSetor (NOLOCK)
                                 Inner Join tApresentacaoSetor (NOLOCK) ON tApresentacaoSetor.SetorID = tSetor.ID 
                                where ApresentacaoID in (" + string.Join(",", apresentacoesIDs) + ") order by nome";

                bd.Consulta(query);

                DataTable dt = new DataTable("Setor");
                dt.Columns.Add("ID");
                dt.Columns.Add("Setores");
                while (bd.Consulta().Read())
                {
                    dt.Rows.Add(new object[] { bd.LerInt("ID"), bd.LerString("Nome") });
                }
        ...
        public DataTable CarregarSetoresAtivos(List<int> apresentacoesIDs)
        {
            CTLib.BD bd = new BD();
            try
            {
                string query = @"SELECT Distinct tSetor.ID,nome FROM tSetor (NOLOCK)
                                 INNER JOIN tApresentacaoSetor (NOLOCK) ON tApresentacaoSetor.SetorID = tSetor.ID 
                                WHERE tSetor.Ativo = 'T' AND ApresentacaoID in (" + string.Join(",", apresentacoesIDs) + ") order by nome";
        ...
        public bool ValidaExclusaoLugares(int setorID)
        {
            ...
            try
            {
                ...
                strConsulta.Append("SELECT tApresentacao.ID FROM tApresentacao");
                strConsulta.Append(" INNER JOIN tApresentacaoSetor ON");
                strConsulta.Append(" tApresentacaoSetor.ID = tApresentacao.ID");
                strConsulta.Append(" WHERE tApresentacao.Horario < " + dataHora.ToString("yyyyMMddHHmmss"));
                strConsulta.Append(" AND tApresentacao.DisponivelAjuste = 'T'");
                strConsulta.Append(" AND tApresentacaoSetor.SetorID = " + setorID);

                bd.Consulta(strConsulta.ToString());

                //Se o Setor estiver associado a uma Apresentação que não tenha passado e que esteja disponível para ajuste (Disponivel para Ajuste TRUE), não será permitida exclusão de NENHUM lugar
        ...
        public int CountApresentacaoPorEvento(int EventoID)
        {
            try
            {
                string strSql = string.Format(@"SELECT COUNT(ID) AS Qtde FROM tApresentacao (NOLOCK) WHERE eventoid = {0}", EventoID);

                bd.Consulta(strSql);

                if (bd.Consulta().Read())
                    return bd.LerInt("Qtde");
                else
                    return -1;
            }
            catch (Exception ex)
