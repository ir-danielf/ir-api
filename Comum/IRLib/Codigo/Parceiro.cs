...
namespace IRLib
{
    ...
    public class Parceiro : Parceiro_B
    {
        ...
        public List<EstruturaParceiro> getParceirosPorTipo(bool indiceZero, int tipo)
        {
            try
            {
                ...
                sql.Append("SELECT ID, Parceiro, Tipo FROM tParceiro (NOLOCK) ");
                if (tipo >= 0)
                {
                    sql.Append("WHERE Tipo = " + tipo + " ");
                }
                sql.Append("ORDER BY Parceiro");
                //string sql = "SELECT ID, Parceiro, Tipo FROM tParceiro (NOLOCK) ORDER BY Parceiro";
                EstruturaParceiro parceiro;
