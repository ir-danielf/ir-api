...
namespace IRLib
{
    ...
    public class IngressoHistoricoGerenciador : MarshalByRefObject
    {

        public const string ACAO = "Ação";

        public IngressoHistoricoGerenciador() { }
        ...
        public static DataTable EstruturaHistoricoNovo()
        {
            ...
            tabela.Columns.Add("StatusDet", typeof(string));
            tabela.Columns.Add("TaxaEntregaValor", typeof(string));
            tabela.Columns.Add("TaxaConvenienciaValor", typeof(string));
            tabela.Columns.Add("TaxaConveniencia", typeof(int));
            tabela.Columns.Add("TaxaEntregaTipo", typeof(string));
        ...
        public DataTable PesquisarSenhaValeIngresso(string senha, int intVendaBilheteriaID = 0)
        {
        ...
        public DataTable BuscaIngressos(string Senha)
        {
            ...
            string sqlBilheteriaID = "SELECT ID FROM tVendaBilheteria WHERE Senha = '" + Senha + "'";

            int VendaBilheteriaID = 0;

            using (IDataReader oDataReader = bd.Consulta(sqlBilheteriaID))
            {
                while (oDataReader.Read())
                {
            ...
            if (VendaBilheteriaID.Equals(0))
            {
                sql.Append("SELECT ev.Nome AS Evento, l.Nome AS Local, a.Horario AS Apresentacao, s.Nome AS Setor, i.Codigo, s.LugarMarcado, p.Valor, p.Nome AS Preco, tvbi.TaxaConvenienciaValor, i.LugarID ");
                sql.Append("FROM tIngressoLog il (NOLOCK) ");
                sql.Append("INNER JOIN tVendaBilheteria vb (NOLOCK) ON vb.ID = il.VendaBilheteriaID ");
                sql.Append("INNER JOIN tVendaBilheteriaItem tvbi ON tvbi.VendaBilheteriaID = vb.ID AND il.VendaBilheteriaItemID = tvbi.ID ");
                sql.Append("INNER JOIN tPreco p (NOLOCK) ON p.ID = il.PrecoID ");
                sql.Append("INNER JOIN tIngresso i (NOLOCK) ON i.ID = IngressoID "); 
                sql.Append("INNER JOIN tEvento ev (NOLOCK) ON ev.ID = i.EventoID ");
                sql.Append("INNER JOIN tLocal l (NOLOCK) ON l.ID = i.LocalID ");
                sql.Append("INNER JOIN tSetor s (NOLOCK) ON s.ID = i.SetorID ");
                sql.Append("INNER JOIN tApresentacao a (NOLOCK) ON a.ID = i.ApresentacaoID ");
                sql.Append("WHERE vb.Senha = '" + Senha + "'  ORDER BY  il.IngressoID,  il.ID ");
            }
            else
            {
                sql.Append("SELECT ev.Nome AS Evento, l.Nome AS Local, a.Horario AS Apresentacao, s.Nome AS Setor, i.Codigo, s.LugarMarcado, p.Valor, p.Nome AS Preco, ");
                sql.Append("CASE WHEN (SELECT COUNT(ID) FROM tingresso WHERE VendaBilheteriaID = " + VendaBilheteriaID + ") > 1 ");
                sql.Append("THEN CAST(tvbi.TaxaConvenienciaValor/(SELECT COUNT(ID) FROM tIngresso WHERE VendaBilheteriaID = " + VendaBilheteriaID + ") AS DECIMAL(10,2)) ");
                sql.Append("ELSE ISNULL(tvbi.TaxaConvenienciaValor,0) END AS TaxaConvenienciaValor, i.LugarID ");
                sql.Append("FROM tIngressoLog il (NOLOCK) ");
                sql.Append("INNER JOIN tVendaBilheteria vb (NOLOCK) ON vb.ID = il.VendaBilheteriaID ");
                sql.Append("INNER JOIN tVendaBilheteriaItem tvbi ON tvbi.VendaBilheteriaID = vb.ID AND il.VendaBilheteriaItemID = tvbi.ID ");
                sql.Append("INNER JOIN tPreco p (NOLOCK) ON p.ID = il.PrecoID ");
