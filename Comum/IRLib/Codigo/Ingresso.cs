...
namespace IRLib
{
    ...
    public partial class Ingresso : Ingresso_B
    {
        ...
        private const int MOBILE_USUARIO_ID = 19515;
        private const int MOBILE_APP = 21331;
        private const int ENTRETIX_SITE = 22269;
        ...
        /// </summary>
        /// <returns></returns>
        public static int AumentarTempoReservasInternet(int clienteID, string sessionID, BD bd = null)
        {
            bool abriuConexao = (bd == null);

            if (bd == null)
                bd = new BD();

            int retorno = bd.Executar("UPDATE tIngresso SET TimeStampReserva = '" + DateTime.Now.AddMinutes(10).ToString("yyyyMMddHHmmss") + "' WHERE (Status = '" + ((char)Ingresso.StatusIngresso.RESERVADO).ToString() + "' OR Status = '" + ((char)Ingresso.StatusIngresso.RESERVADO_PRE_RESERVA_SITE).ToString() + "') AND ClienteID = " + clienteID + " AND SessionID = '" + sessionID + "'");
        ...
        public List<EstrutruraPreReservaSite> IngressosPreReservaPorSenha(string senha, int clienteID)
        {
            try
            {
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    //mesa fechada?
                    if (setorTipo == (char)IRLib.Setor.enumLugarMarcado.MesaFechada)
                    {
        ...
        {
            string sql = @"SELECT COUNT(tIngressolog.ID) FROM tIngressolog (NOLOCK) 
        ...
        public int TotalImpresso(int EventoID)
        {
            string sql = @"	SELECT COUNT(tIngressoLog.ID) FROM tIngresso (NOLOCK) 
                            INNER JOIN tIngressoLog (NOLOCK) ON tIngresso.ID = IngressoID
            ...
            int EmpresaID, int LocalID, int BloqueioID, int qtd, int codigo, CTLib.BD database, bool trabalhandoTransaction, List<string> CodigosDeBarra)
        {
            ...
            if (codigo == 0)
                inicio = this.UltimoCodigoPista(ApresentacaoSetorID);
            ...
            if (qtd > 0)
            {
                ...
                for (int i = inicio; i < inicio + qtd; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (this.NovoNaoMarcado(ApresentacaoSetorID, EventoID, ApresentacaoID, SetorID, EmpresaID, LocalID, BloqueioID, i + 1, database, codigoSequencial, string.Empty))
                            qtdAcrescentada++;
                        else
                        {
                            if (trabalhandoTransaction)
                                database.DesfazerTransacao();
                            throw new IngressoException("Falha ao adicionar os ingressos: " + ApresentacaoSetorID);
                        }
                    }
                    catch
                    {
                ...
                if ((qtdAcrescentada > 0) && (trabalhandoTransaction))
                    database.FinalizarTransacao();
            }
            ...
            try
            {
                ...
                if (item.ApresentacaoID == 0)
                {
                    ...
                    using (IDataReader oDataReader = bd.Consulta("" +
                        ...
                        "AND " +
                        "   (tSetor.LugarMarcado <> '" + IRLib.Setor.Pista + "')"))
                    {
            ...
            int apresentacaoID, int empresaID, int localID, int qtd, ref string nomeSetor, List<String> CodigosDeBarra, Enumerators.TipoCodigoBarra tipoCodigoBarra)
        {
            ...
            try
            {
                ...
                if (lugares.Consulta().Read())
                {
                    ...
                    if (lugares.LerString("LugarMarcado") == IRLib.Setor.Pista)
        ...
        public int GerarLugares(int setorID, int apresentacaoSetorID, int eventoID,
            int apresentacaoID, int empresaID, int localID, List<String> CodigosDeBarra, Enumerators.TipoCodigoBarra tipoCodigoBarra, BD bd)
        {

            var setor = new Setor();
            var tipoSetor = setor.BuscaTipo(setorID, bd);
            if (tipoSetor == IRLib.Setor.Tipo.Pista)
                throw new IngressoException("Não é permitido criar lugar marcado para setores de pista: " + setorID);

            var lugares = setor.LugaresDoSetor(setorID, bd);
            int sequencia = this.UltimoCodigoSequencial(apresentacaoID) + 1;
            ...
            try
            {
                ...
                {
                    this.NovoMarcado(apresentacaoSetorID, eventoID, apresentacaoID, setorID, empresaID, localID, lugar.BloqueioID.Valor,
                        lugar.Control.ID, lugar.Quantidade.Valor, lugar.QuantidadeBloqueada.Valor, lugar.Codigo.Valor, lugar.Grupo.Valor,
                        lugar.Classificacao.Valor, Utils.Enums.GetChar(tipoSetor).ToString(), bd, sequencia, null);

                    sequencia += lugar.Quantidade.Valor;
                }
                bd.FinalizarTransacao();
            ...
            int setorid, int empresaid, int localid, int bloqueioid, int codigo, CTLib.BD database, int codigoSequencial, string codigoDeBarra)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(codigoDeBarra))
                    codigoBarra.AtualizaCodigoBarraListaBranca(database, codigoDeBarra, apresentacaosetorid);
            ...
            int classificacao, string tipoSetor, CTLib.BD database, int codigoSequencial, string[] codigosDeBarra)
        {
            try
            {
                ...
                if (tipoSetor != IRLib.Setor.Cadeira)
                {
                    if (qtdlugar > 0)
                    {
                        if (qtdlugar > 702)
                        ...
                        for (int x = 0; x < qtdlugar; x++)
                        {
                            ...
                            if (codigo != null && codigo != "")
                            {
                                try
                                {
                                    cod = ((codigo.Length > 17) ? codigo.Substring(0, 17) : codigo) + "-" + letra[x].ToUpper();
                                }
                                catch
                                {
                                    cod = "*ND*-*";
                            ...
                            if (codigosDeBarra != null && codigosDeBarra.Length > 0)
                                codigoBarra.AtualizaCodigoBarraListaBranca(database, codigosDeBarra[x], apresentacaosetorid);
                        }
                    }
                    return ok;
                }
                else
                {
                    if (qtdlugar == 1)
                    {
                        ...
                        if (codigosDeBarra != null && codigosDeBarra.Length > 0)
                            codigoBarra.AtualizaCodigoBarraListaBranca(database, codigosDeBarra[0], apresentacaoid);
                    }
                }
        ...
        public static void PreencherCodigoSequencial()
        {
            ...
            try
            {
                ...
                string sqlIDs = "SELECT DISTINCT ID FROM tApresentacao (NOLOCK) ";
                bd.Executar(sqlIDs);
                int controle = 1;
                string sqlUpdate = "";
                while (bd.Consulta().Read())
        ...
        public static DataTable EstruturaImpressao()
        {
            ...
            DataTable tabela = new DataTable(Bilheteria.TABELA_ESTRUTURA_IMPRESSAO);
            ...
            //Dados do Agregado
            tabela.Columns.Add("AgregadoNome", typeof(string)).DefaultValue = "";
            tabela.Columns.Add("AgregadoCPF", typeof(string)).DefaultValue = "";
            tabela.Columns.Add("AgregadoEmail", typeof(string)).DefaultValue = "";
            tabela.Columns.Add("AgregadoTelefone", typeof(string)).DefaultValue = "";
            ...
            tabela.Columns.Add("ApresentacaoCancelada", typeof(string)).DefaultValue = string.Empty;
        ...
        public override DataRow InfoVendido()
        {
            ...
            try
            {
                ...
                sb.Append("INNER JOIN tIngressoLog il (NOLOCK) ON i.ID=il.IngressoID ");
                sb.Append("INNER JOIN tVendaBilheteriaItem vbi (NOLOCK) ON il.VendaBilheteriaItemID=vbi.ID ");
                sb.Append("INNER JOIN tVendaBilheteria vb (NOLOCK) ON vbi.VendaBilheteriaID=vb.ID ");
                ...
                sb.Append("INNER JOIN tPreco p (NOLOCK) ON il.PrecoID=p.ID ");
                sb.Append("INNER JOIN tUsuario u (NOLOCK) ON il.UsuarioID=u.ID ");
                sb.Append("LEFT JOIN tPacote pac (NOLOCK) ON vbi.PacoteID=pac.ID ");
        ...
        */
        ...
        public override DataTable InfoReservados(int[] ingressosids, CTLib.BD database, int lojaID)
        {
            ...
            try
            {
                ...
                SELECT i.ID, u.Login AS Usuario,e.ImpressaoCodigoBarra,e.Nome AS Evento,i.EventoID, i.CodigoSequencial, 
                a.Horario,i.ApresentacaoID,s.Nome AS Setor,s.Acesso,p.ImprimirCarimbo,p.CarimboTexto1,p.CarimboTexto2,s.Produto,s.LugarMarcado AS TipoSetor, c.Nome AS Cortesia,p.Nome AS Preco, 
                ...
                l.Nome AS Loja,tLocal.ID AS LocalID, tLocal.Nome AS LocalNome, tLocal.Logradouro, tLocal.Numero, tLocal.Cidade AS LocalCidade,tLocal.Estado AS LocalEstado,tLocal.Cep AS LocalCep,e.ObrigaCadastroCliente, IsNull(e.ObrigatoriedadeID, 0) AS ObrigatoriedadeID, 
                tPais.Nome AS Pais,i.GerenciamentoIngressosID,  
                CASE WHEN el.ID IS NOT NULL THEN el.TipoImpressao ELSE e.TipoImpressao END AS TipoImpressao, e.TipoCodigoBarra, 
                 CASE WHEN LEN(a.Alvara) > 0 
                 THEN a.Alvara 
                 ELSE 
                 CASE WHEN LEN (e.Alvara) > 0 
                 THEN e.Alvara 
                 ELSE tLocal.Alvara 
                 END 
                 END AS Alvara, 
                 CASE WHEN LEN (e.FonteImposto) > 0 
                 THEN e.FonteImposto 
                 ELSE tLocal.FonteImposto 
                 END AS FonteImposto, 
                 CASE WHEN LEN(a.AVCB) > 0 
                 THEN a.AVCB 
                 ELSE 
                 CASE WHEN LEN (e.AVCB) > 0 
                 THEN e.AVCB 
                 ELSE tLocal.AVCB 
                 END 
                 END AS AVCB, 
                 CASE WHEN e.PorcentagemImposto > 0 
                 THEN e.PorcentagemImposto 
                 ELSE tLocal.PorcentagemImposto 
                 END AS PorcentagemImposto, 

                 CASE WHEN LEN(a.Lotacao) > 0 
                 THEN a.Lotacao   
                 ELSE CASE WHEN LEN (e.Lotacao) > 0 
                 THEN e.Lotacao   
                 ELSE tLocal.Lotacao 
                 END  END AS Lotacao, 

                 CASE WHEN LEN(a.DataEmissaoAlvara) > 0 
                 THEN a.DataEmissaoAlvara   
                 ELSE CASE WHEN LEN (e.DataEmissaoAlvara) > 0 
                 THEN e.DataEmissaoAlvara 
                 ELSE tLocal.DataEmissaoAlvara 
                 END  END AS DataEmissaoAlvara, 


                 CASE WHEN LEN(a.DataValidadeAlvara) > 0 
                 THEN a.DataValidadeAlvara   
                 ELSE CASE WHEN LEN (e.DataValidadeAlvara) > 0 
                 THEN e.DataValidadeAlvara 
                 ELSE tLocal.DataValidadeAlvara 
                 END  END AS DataValidadeAlvara, 

                 CASE WHEN LEN(a.DataEmissaoAvcb) > 0 
                 THEN a.DataEmissaoAvcb   
                 ELSE CASE WHEN LEN (e.DataEmissaoAvcb) > 0 
                 THEN e.DataEmissaoAvcb 
                 ELSE tLocal.DataEmissaoAvcb 
                 END  END AS DataEmissaoAvcb, 

                 CASE WHEN LEN(a.DataValidadeAvcb) > 0 
                 THEN a.DataValidadeAvcb   
                 ELSE CASE WHEN LEN (e.DataValidadeAvcb) > 0 
                 THEN e.DataValidadeAvcb 
                 ELSE tLocal.DataValidadeAvcb 
                 END  END AS DataValidadeAvcb 

                FROM tIngresso i (NOLOCK) 
                LEFT OUTER JOIN tPreco p (NOLOCK) ON i.ApresentacaoSetorID=p.ApresentacaoSetorID AND i.PrecoID=p.ID 
                ...
                INNER JOIN tPais (NOLOCK) ON tLocal.PaisID = tPais.ID 
                ...
                WHERE i.ID in ({1}) ORDER BY i.Codigo", lojaID, Utilitario.ArrayToString(ingressosids));
        ...
        public override void CancelarPacote()
        {
            ...
            try
            {
                ...
                string sql = "UPDATE tIngresso SET ClienteID=0, PrecoID=0, CortesiaID=0, BloqueioID=0, LojaID=0, PacoteID=0 , PacoteGrupo = '', " +
                    "Status='" + DISPONIVEL + "', CodigoBarra='', SessionID = '', TimeStampReserva = '' " +
        ...
        public override bool CancelarReserva(int usuarioid)
        {
            ...
            try
            {
                ...
                sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, BloqueioID=0, LojaID=0, " +
                    "Status='" + BLOQUEADO + "', SessionID = '', TimeStampReserva = '' " +
                    "WHERE (AssinaturaClienteID IS NULL OR AssinaturaClienteID = 0) AND BloqueioID<>0 AND UsuarioID=" + usuarioid + " AND (Status='" + RESERVADO + "' OR Status='" + RESERVADO_PRE_RESERVA + "') AND ID=" + this.Control.ID;
        ...
        public override bool CancelarReservas(int usuarioid)
        {
            ...
            try
            {
                ...
                sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, " +
                    "Status='" + DISPONIVEL + "', CodigoBarra='', SessionID = '', TimeStampReserva = '' " +
                    "WHERE (AssinaturaClienteID IS NULL OR AssinaturaClienteID = 0) AND BloqueioID=0 AND Status IN ('" + RESERVADO + "', '" + RESERVADO_PRE_RESERVA + "') AND UsuarioID=" + usuarioid;
                x = bd.Executar(sql);
                result |= Convert.ToBoolean(x);
        ...
        public override bool CancelarReservas(int[] ingressosids)
        {
            try
            {
                ...
                sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, " +
                    "Status='" + DISPONIVEL + "', CodigoBarra='', PacoteID = 0, PacoteGrupo= '', SessionID = '', TimeStampReserva = '' " +
                ...
                sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, " +
                    "Status='" + BLOQUEADO + "', CodigoBarra='', PacoteID = 0, PacoteGrupo= '', SessionID = '', TimeStampReserva = ''  " +
                    "WHERE (AssinaturaClienteID IS NULL OR AssinaturaClienteID = 0) AND Status = '" + Ingresso.RESERVADO + "' AND BloqueioID<>0 AND ID in (" + ids + ") AND UsuarioID=" + this.UsuarioID.Valor;
                x = bd.Executar(sql);
                result |= Convert.ToBoolean(x);
        ...
        public bool CancelarReservasInternet(int ingressoid, int usuarioID)
        {
            try
            {
                ...
                string sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, PacoteID=0, " +
                    "PacoteGrupo = '', Status='" + Ingresso.DISPONIVEL + "', SerieID = 0, SessionID = '', TimeStampReserva = '' " +
                ...
                string sql2 = "UPDATE tIngresso SET  ClienteID=0, PrecoID=0, CortesiaID=0, PacoteID=0, " +
                    "PacoteGrupo = '', Status='" + (char)Ingresso.StatusIngresso.PRE_RESERVA + "', SerieID = 0 , SessionID = '', TimeStampReserva = ''" +
        ...
        public bool CancelarTodasReservasInternet(int clienteID, string sessionID)
        {
            try
            {
                ...
                string sql = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, PacoteID = 0, PacoteGrupo = '', " +
                    "Status='" + ((char)Ingresso.StatusIngresso.DISPONIVEL).ToString() + "', SerieID = 0, SessionID = '', TimeStampReserva = '' " +
                ...
                string sqlPreReserva = "UPDATE tIngresso SET ClienteID=0, UsuarioID=0, PrecoID=0, CortesiaID=0, LojaID=0, PacoteID = 0, PacoteGrupo = '', " +
                    "Status='" + ((char)Ingresso.StatusIngresso.RESERVADO_PRE_RESERVA_SITE).ToString() + "', SerieID = 0, SessionID = '', TimeStampReserva = '' " +
        ...
        public override int VendaBilheteriaItemID()
        {
            try
            {
                ...
                string sql =
                    ...
                    "FROM       tIngresso (NOLOCK) INNER JOIN " +
                    "tIngressoLog (NOLOCK) ON tIngresso.ID = tIngressoLog.IngressoID INNER JOIN " +
                    "tVendaBilheteriaItem (NOLOCK) ON tIngressoLog.VendaBilheteriaItemID = tVendaBilheteriaItem.ID " +
        ...
        public override bool VendidoUmaVez()
        {
            ...
            try
            {
                ...
                string sql =
                    ...
                    "FROM          tIngresso (NOLOCK) INNER JOIN " +
                    "tIngressoLog (NOLOCK) ON tIngresso.ID = tIngressoLog.IngressoID " +
        ...
        public override int TotalPorCanalEvento(int canalID, int eventoID)
        {
            ...
            try
            {
                ...
                string sql =
                    ...
                    "FROM         tCanal (NOLOCK) INNER JOIN " +
                    "tLoja (NOLOCK) ON tCanal.ID = tLoja.CanalID INNER JOIN " +
                    "tCaixa (NOLOCK) ON tLoja.ID = tCaixa.LojaID INNER JOIN " +
                    "tVendaBilheteria (NOLOCK) ON tCaixa.ID = tVendaBilheteria.CaixaID INNER JOIN " +
                    "tVendaBilheteriaItem (NOLOCK) ON tVendaBilheteria.ID = tVendaBilheteriaItem.VendaBilheteriaID INNER JOIN " +
                    "tIngresso (NOLOCK) ON tVendaBilheteriaItem.ID = tIngresso.VendaBilheteriaItemID INNER JOIN " +
                    "tApresentacaoSetor (NOLOCK) ON tIngresso.ApresentacaoSetorID = tApresentacaoSetor.ID INNER JOIN " +
                    "tApresentacao (NOLOCK) ON tApresentacaoSetor.ApresentacaoID = tApresentacao.ID INNER JOIN " +
                    "tEvento (NOLOCK) ON tApresentacao.EventoID = tEvento.ID " +
        ...
        public override string SenhaVenda(string codigo, int apresentacaoSetorID)
        {
            ...
            try
            {
                ...
                string sql =
                    ...
                    "FROM         tIngresso (NOLOCK) INNER JOIN " +
                    "tIngressoLog (NOLOCK) ON tIngressoLog.IngressoID = tIngresso.ID INNER JOIN " +
                    "tVendaBilheteriaItem (NOLOCK) ON tIngressoLog.VendaBilheteriaItemID = tVendaBilheteriaItem.ID INNER JOIN " +
                    "tVendaBilheteria (NOLOCK) ON tVendaBilheteriaItem.VendaBilheteriaID = tVendaBilheteria.ID " +
        ...
        public override DataTable Historico()
        {
            ...
            try
            {
                ...
                DataTable tabela = IngressoHistoricoGerenciador.EstruturaHistorico();
                StringBuilder sb = new StringBuilder();
        ...
        public void IdentificaPelaApresentacaoSetorID(int apresentacaoSetorID)
        {
            try
            {
                ...
                string sql =
                    ...
                    "FROM          tApresentacaoSetor (NOLOCK) INNER JOIN " +
                    "tIngresso (NOLOCK) ON tApresentacaoSetor.ID = tIngresso.ApresentacaoSetorID " +
...
*/
        ...
        public override DataTable EventoApresentacaoSetor()
        {
            ...
            try
            {
                ...
                string sql =
                    ...
                    "FROM            tApresentacaoSetor (NOLOCK) INNER JOIN " +
                    "tSetor (NOLOCK) ON tApresentacaoSetor.SetorID = tSetor.ID INNER JOIN " +
        ...
        public int[] LimpaReservasInternet()
        {
            try
            {
                var delayReserva = int.Parse(ConfigurationManager.AppSettings["delayReserva"]);
                var ids = new ArrayList();
                var data = DateTime.Now.AddMinutes(-delayReserva);

                bd.FecharConsulta();

                var usuariosCanaisInternet = ConfiguracaoAppUtil.GetAsListInt("usuariosCanaisInternet");
                var canalTipoPOS = ConfiguracaoAppUtil.GetAsInt("CanalTipoPOS");

                var sqlConsultaIngresso = @"SELECT
                                              ti.ID,
                                              ti.ApresentacaoID,
                                              ti.Status,
                                              ti.ParceiroMidiaID,
                                              ti.CotaItemID
                                            FROM tIngresso AS ti ( NOLOCK )
                                            INNER JOIN tLoja AS tl ( NOLOCK ) ON tl.ID = ti.LojaID
                                            INNER JOIN tCanal AS tc ( NOLOCK ) ON tc.ID = tl.CanalID
                                            WHERE ti.TimeStampReserva IS NOT NULL AND ti.TimeStampReserva <> ''
                                                  AND @data > ti.TimeStampReserva
                                                  AND (ti.Status = @statusReserva
                                                       OR ti.Status = @statusPreReserva)
                                                  AND ti.SessionID IS NOT NULL AND ti.SessionID <> ''
                                                  AND (ti.AssinaturaClienteID IS NULL OR ti.AssinaturaClienteID = 0)
                                                  AND (ti.UsuarioID IN (@canaisInternet) OR tc.CanalTipoID = @canalTiposPOS)";

                sqlConsultaIngresso = sqlConsultaIngresso.Replace("@canaisInternet", string.Join(",", usuariosCanaisInternet));

                var paramentrosConsultaIngresso = new List<SqlParameter>()
                {
                    new SqlParameter("data", data.ToString("yyyyMMddHHmmss")),
                    new SqlParameter("statusReserva", (char) StatusIngresso.RESERVADO),
                    new SqlParameter("statusPreReserva", (char) StatusIngresso.RESERVADO_PRE_RESERVA_SITE),
                    new SqlParameter("canalTiposPOS", canalTipoPOS)
                };

                bd.Consulta(sqlConsultaIngresso, paramentrosConsultaIngresso);
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    var status = bd.LerString("Status");
                    var parceiroId = bd.LerInt("ParceiroMidiaID");
                    var cotaItemId = bd.LerInt("CotaItemID");
                    
                    var sucesso = false;

                    if (status == ((char) StatusIngresso.RESERVADO_PRE_RESERVA_SITE).ToString())
                    {
                        var sqlUpdateIngresso = @"UPDATE tIngresso
                                                  SET SessionID = '', Status = @statusNovo, PacoteID = 0, PacoteGrupo = '', SerieID = 0, CotaItemID = NULL
                                                  WHERE Status = @statusAntigo AND BloqueioID = 0 AND ID = @ingressoId
                                                 ";

                        var parametrosUpdateIngresso = new List<SqlParameter>()
                        {
                            new SqlParameter("statusNovo", (char) StatusIngresso.PRE_RESERVA),
                            new SqlParameter("statusAntigo", (char) StatusIngresso.RESERVADO_PRE_RESERVA_SITE),
                            new SqlParameter("ingressoId", ingressoId)
                        };

                        sucesso = bdUpdate.Executar(sqlUpdateIngresso, parametrosUpdateIngresso) > 0;
                    }
                    else if (parceiroId > 0)
                    {
                        var sqlUpdateIngresso = @"UPDATE tIngresso
                                                  SET SessionID = '', TimeStampReserva = '', ClienteID = 0, Status = @statusNovo, PacoteID = 0, PacoteGrupo = '', SerieID = 0, CotaItemID = NULL
                                                  WHERE Status = @statusAntigo AND ID = @ingressoId
                                                 ";

                        var parametrosUpdateIngresso = new List<SqlParameter>()
                        {
                            new SqlParameter("statusNovo", (char) StatusIngresso.BLOQUEADO),
                            new SqlParameter("statusAntigo", (char) StatusIngresso.RESERVADO_PRE_RESERVA_SITE),
                            new SqlParameter("ingressoId", ingressoId)
                        };
                    ...
                    else
                    {
                        ...
                        var parametrosUpdateIngresso = new List<SqlParameter>()
                        {
                            new SqlParameter("statusNovo", (char)StatusIngresso.DISPONIVEL),
                            new SqlParameter("statusAntigo", (char)StatusIngresso.RESERVADO),
                            new SqlParameter("ingressoId", ingressoId)
                        };
        ...
        public bool AdicionarCotaItem(int id, int? cotaItemId)
        {
            try
            {
                ...
                if (cotaItemId != null)
                {
                    sql = string.Format(@"UPDATE tIngresso SET CotaItemId = {0} WHERE ID = {1}", cotaItemId, id);
                }
                else 
                {
                    sql = string.Format(@"UPDATE tIngresso SET CotaItemId = NULL WHERE ID = {0}", id);
                }

                var linhas = this.bd.Executar(sql);
        ...
        public bool AdicionarCotaItem2(int id, int? cotaItemId)
        {
            try
            {
        ...
        public bool RetirarCotaItemIngresso(int id)
        {
            try
            {
                string sql = "UPDATE tIngresso SET CotaItemID = NULL WHERE ID = " + id.ToString();

                var linhas = bd.Executar(sql);
    ...
    public class IngressoLista : IngressoLista_B
    {
        ...
        public string ItemIDs()
        {
            ...
            try
            {
                ...
                string sql =
                    ...
                    "FROM            tVendaBilheteriaItem (NOLOCK) INNER JOIN " +
                    "tIngressoLog (NOLOCK) ON tVendaBilheteriaItem.ID = tIngressoLog.VendaBilheteriaItemID LEFT OUTER JOIN " +
                    "tIngresso (NOLOCK) ON tIngressoLog.IngressoID = tIngresso.ID " +
        ...
        public Ingresso[] Ingressos2Reserva()
        {
            ...
            if (tamanhoMax == 0)
                sql = "SELECT ID, Codigo FROM tIngresso(NOLOCK) ";
            else
                sql = "SELECT top " + tamanhoMax + " ID FROM tIngresso(NOLOCK) ";
