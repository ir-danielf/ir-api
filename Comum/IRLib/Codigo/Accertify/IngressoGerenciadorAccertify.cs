...
namespace IRLib
{
    public class IngressoGerenciadorAccertify
    {
        ...
        public List<IngressoImpressao> PesquisaVendaBilheteria(int vendaBilheteriaID)
        {
            try
            {
                ...
                foreach (var ingresso in ingressos)
                {
                    ...
                    {
                        LogUtil.Debug(string.Format("##IngressoGerenciadorAccertify.GerandoCodigoDeBarras## VENDA: {0}, INGRESSO_ID: {1}", vendaBilheteriaID, ingresso.ID));

                        var estruturaimpressao = this.RegistrarImpressaoInternet(ingresso.ID);
        ...
        private List<EstruturaRetornoRegistroImpressao> RegistrarImpressaoInternet(int ingressoID)
        {
            try
            {
                LogUtil.Debug(string.Format("##IngressoGerenciadorAccertify.RegistrarImpressaoInternet## INGRESSO_ID: {0}", ingressoID));
