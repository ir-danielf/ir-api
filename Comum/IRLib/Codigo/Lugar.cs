...
namespace IRLib
{
    ...
    public class Lugar : Lugar_B
    {
        ...
        public IRLib.ClientObjects.Lugar[] SalvarLugaresInserir(IRLib.ClientObjects.Lugar[] listaInserir)
        {
            ...
            for (int i = 0; i < listaInserir.Length; i++)
            {
                ...
                this.PNETipo.Valor = listaInserir[i].PNETipo;
                this.LugarIDCadeirante.Valor = listaInserir[i].LugarIDCadeirante;
            ...
            try
            {
                ...
                switch (setorTipo)
                {
                    ...
                    case Setor.enumLugarMarcado.Cadeira:
                        ...
                        string sql = @"
                                    SELECT i.Grupo, COUNT(i.ID) Quantidade 
                                    FROM tIngresso (NOLOCK) i
                                    LEFT JOIN tLugar (NOLOCK) l on l.ID = i.LugarID
                                    WHERE i.ApresentacaoSetorID = "+apresentacaoSetorID+@" 
                                      AND i.Grupo > 0 
                                      AND i.Classificacao > 0 
                                      AND i.Status = '" + Ingresso.DISPONIVEL + @"' 
                                      AND (l.PNETipo is null or l.PNETipo = '')
                                    GROUP BY i.Grupo 
                                    HAVING COUNT(i.ID) >= "+quantidade+@"
                                    ORDER BY i.Grupo, Quantidade
                                    ";
                        ...
                        for (int i = 0; i < gruposCadeira.Count && ingressos.Count < quantidade; i++)
                        {
                            ...
                            // Busca os lugares do grupo em questão.
                            bd.Consulta(@"SELECT i.ID, i.Codigo, i.Classificacao, i.LugarID 
                                            FROM tIngresso (NOLOCK) i
                                            LEFT JOIN tLugar (NOLOCK) l on l.ID = i.LugarID
                                            WHERE i.ApresentacaoSetorID = " + apresentacaoSetorID + @"
                                            AND i.Grupo = " + grupo + @"
        ...
        /// </summary>
        /// <returns></returns>
        public void SalvarLugaresAtualizar(List<IRLib.ClientObjects.Lugar> listaAtualizar)
        {
            for (int i = 0; i < listaAtualizar.Count; i++)
            {
                ...
                this.PNETipo.Valor = listaAtualizar[i].PNETipo;
                this.LugarIDCadeirante.Valor = listaAtualizar[i].LugarIDCadeirante;
                base.Atualizar();
        ...
        public IRLib.ClientObjects.Lugar[] SalvarMapa(IRLib.ClientObjects.Lugar[] alteracoes, List<string> lstExcluidosID)
        {
            ...
            try
            {
                ...
                IRLib.ClientObjects.Lugar[] lugares = this.SalvarLugares(alteracoes, bdLugares);
        ...
        private IRLib.ClientObjects.Lugar[] SalvarLugares(IRLib.ClientObjects.Lugar[] listaInserir, BD bdLugares)
        {
            StringBuilder sql;
            foreach (var lugar in listaInserir)
            {
                ...
                switch (lugar.Acao)
                {
                    case IRLib.ClientObjects.Lugar.TiposAcao.Inserir:

                        sql = new StringBuilder();
                        sql.Append(" INSERT INTO tLugar(SetorID, Codigo, Quantidade, QuantidadeBloqueada, PosicaoX, PosicaoY, Simbolo, BloqueioID, Classificacao, Grupo, Obs, PerspectivaLugarID, PNETipo, LugarIDCadeirante ) ");
                        sql.Append(" VALUES (@001,'@002',@003,@004,@005,@006,@007,@008,@009,@010,'@011',@012,'@013',@014)");
                        ...
                        sql.Replace("@013", lugar.PNETipo.ToString());
                        sql.Replace("@014", lugar.LugarIDCadeirante.ToString());

                        bdLugares.Executar(sql.ToString());

                        break;
                    case IRLib.ClientObjects.Lugar.TiposAcao.Atualizar:

                        sql = new StringBuilder();
                        sql.Append(" UPDATE tLugar SET SetorID = @001 , Codigo = '@002' , Quantidade = @003 , QuantidadeBloqueada = @004 , PosicaoX = @005 , PosicaoY = @006 , ");
                        sql.Append(" Simbolo = '@007' , BloqueioID = @008 , Classificacao = @009 , Grupo = @010 , Obs = '@011' , PerspectivaLugarID = @012 , PNETipo = '@013' , LugarIDCadeirante = @014");
