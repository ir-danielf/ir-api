...
namespace IRLib
{
    ...
    public class Setor : Setor_B
    {
        ...
        public const string CADEIRANTE_NAO_RESERVADO = "CADEIRANTE_NAO_RESERVADO";
        ...
        public Tipo BuscaTipo(int setorID, BD bd = null)
        {
            var fecharBD = false;
            try
            {
                ...
                var tipo = bd.ConsultaValor("SELECT LugarMarcado FROM tSetor(NOLOCK) WHERE ID = @SetorID", new SqlParameter[]{new SqlParameter("@SetorID", setorID)});

                if (tipo == null || tipo == "")
                    throw new ApplicationException("Setor inválido: " + setorID);

                return Utils.Enums.ParseCharEnum<Tipo>(tipo.ToString());
            }
            finally
            {
                if (fecharBD)
        ...
        public CTLib.BD Lugares(int setorID)
        {
            string sql = @"SELECT " +
                ...
                @"FROM tLugar (NOLOCK) " +
                @"INNER JOIN tSetor (NOLOCK) ON tSetor.ID = tLugar.SetorID " +
        ...
        public List<Lugar> LugaresDoSetor(int setorID, BD bd = null)
        {
            bool fecharBD = false;

            string sql = @"SELECT
                tLugar.ID, tLugar.Codigo, tLugar.Quantidade, tLugar.QuantidadeBloqueada, tLugar.BloqueioID, tLugar.Grupo, tLugar.Classificacao
                FROM tLugar (NOLOCK)
                WHERE SetorID = @SetorID ORDER BY Codigo";
            ...
            try
            {
                ...
                var reader = bd.Consulta(sql, new List<SqlParameter>() { new SqlParameter("@SetorID", setorID) });

                while (reader.Read())
                {

                    var l = new Lugar();
                    l.SetorID.Valor = setorID;
                    l.Control.ID = bd.LerInt("ID");
                    l.Codigo.Valor = bd.LerString("Codigo");
                    l.Quantidade.Valor = bd.LerInt("Quantidade");
                    l.QuantidadeBloqueada.Valor = bd.LerInt("QuantidadeBloqueada");
                    l.BloqueioID.Valor = bd.LerInt("BloqueioID");
                    l.Classificacao.Valor = bd.LerInt("Classificacao");
                    l.Grupo.Valor = bd.LerInt("Grupo");
        ...
        public override DataTable Lugares()
        {
            ...
            try
            {
                ...
                tabela.Columns.Add("PNETipo", typeof(string)).DefaultValue = string.Empty;
                tabela.Columns.Add("LugarIDCadeirante", typeof(int)).DefaultValue = 0;
                ...
                while (bd.Consulta().Read())
                {
                    ...
                    linha["PNETipo"] = bd.LerString("PNETipo");
                    linha["LugarIDCadeirante"] = bd.LerInt("LugarIDCadeirante");
                    tabela.Rows.Add(linha);
                }
        ...
        public override int QuantidadeBloqueado(int apresentacaoid)
        {
            ...
            try
            {
                ...
                string sql = "SELECT Count(i.ID) AS Qtde " +
                    "FROM tIngresso(NOLOCK) as i,tApresentacaoSetor(NOLOCK) as aps " +
        ...
        public List<EstruturaSetores>CarregaListaSetoresPorApresentacao(int idApresentacao)
        {
            try
            {
                List<SqlParameter> parametros = new List<SqlParameter>();

                string sql = @"SELECT tSetor.ID AS IdSetor,
		                        tSetor.Nome AS NomeSetor
                        FROM tSetor (NOLOCK)
                        WHERE tSetor.ID IN(SELECT SetorId FROM tApresentacaoSetor (NOLOCK) WHERE ApresentacaoID = @ApresentacaoID)";

                parametros.Add(new SqlParameter() { ParameterName = "@ApresentacaoID", Value = idApresentacao, DbType = System.Data.DbType.Int32 });

                bd.Consulta(sql, parametros);

                List<EstruturaSetores> listaEstruturaSetores = new List<EstruturaSetores>();
                EstruturaSetores _EstruturaSetores;
                while (bd.Consulta().Read())
                {
                    _EstruturaSetores = new EstruturaSetores();
                    _EstruturaSetores.ID = bd.LerInt("IdSetor");
                    _EstruturaSetores.Nome = bd.LerString("NomeSetor");
                    listaEstruturaSetores.Add(_EstruturaSetores);
                }
                return listaEstruturaSetores;
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        /// <summary>
        /// Método para Buscar o Local e Mapa Esquematico ou somente local.
        /// </summary>
        /// <param name="localID"></param>
        /// <param name="mapaEsquematicoId"></param>
        /// <returns>
        /// A = Mesa Aberta
        /// P = Pista
        /// C = Cadeira
        /// M = Mesa Fechada
        /// </returns>
        public DataTable BuscarDataTablePorLocalIDMapaEsquematico(int localID, int? mapaEsquematicoId = null)
        {
            try
            {
                ...
                if (mapaEsquematicoId != null)
                {
                    sql = @"SELECT DISTINCT tSetor.ID, tSetor.Nome,                                    
				                        case
				                        when  tSetor.LugarMarcado = 'A' then 'Mesa Aberta'
				                        when  tSetor.LugarMarcado = 'P' then 'Pista' --- informa
				                        when  tSetor.LugarMarcado = 'C' then 'Cadeira'
				                        when  tSetor.LugarMarcado = 'M' then 'Mesa Fechada'                                   
				                        END as Tipo,
				                        (SELECT SUM(quantidade) FROM tLugar (Nolock) WHERE tLugar.SetorID = tSetor.ID) as Quantidade
				                        FROM tSetor (Nolock)
				                        INNER JOIN tMapaEsquematicoSetor (Nolock) ON tSetor.ID = tMapaEsquematicoSetor.SetorID
                                    WHERE tSetor.LocalID = @localID
                                    AND MapaId = @mapaEsquematicoId ";
                }
                else
                {
                    sql = @"SELECT DISTINCT tSetor.ID, tSetor.Nome,                                    
				                        ...
				                        (SELECT SUM(quantidade) FROM tLugar (Nolock) WHERE tLugar.SetorID = tSetor.ID) as Quantidade
				                        FROM tSetor (Nolock)
				                        LEFT JOIN tMapaEsquematicoSetor (Nolock) ON tSetor.ID = tMapaEsquematicoSetor.SetorID           
                                    WHERE tSetor.LocalID = @localID ";


                }

                sql += " ORDER BY tSetor.Nome ";
                ...
                if (mapaEsquematicoId != null)
                {
                    ...
                    {
                        DbType = DbType.Int32,
                        ParameterName = "@mapaEsquematicoId",
                        Value = mapaEsquematicoId
                    });
                }

                return bd.QueryToTable(sql, parametros);
            }
            catch (Exception)
            {
                throw;
        ...
        /// M = Mesa Fechada
        /// </returns>
        public DataTable BuscarAtivosDataTablePorLocalIDMapaEsquematico(int localID, int? mapaEsquematicoId = null)
        {
            try
            {
                ...
                else
                {
                    sql = @"SELECT DISTINCT tSetor.ID, tSetor.Nome,                                    
				                        ...
				                        FROM tSetor (Nolock)
				                        LEFT JOIN tMapaEsquematicoSetor (Nolock) ON tSetor.ID = tMapaEsquematicoSetor.SetorID           
                                    WHERE tSetor.LocalID = @localID AND tSetor.Ativo = 'T' ";
