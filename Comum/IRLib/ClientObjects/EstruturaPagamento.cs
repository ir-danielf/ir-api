...
namespace IRLib.ClientObjects
{
    public class EstruturaPagamento
    {
        ...
        public enum enumRetornoPagamento
        {
            FALHA_ATUALIZAR_PEDIDO = 0,
            OK = 1,
            CLIENTE_INEXISTENTE = 2,
            RESERVA_INEXISTENTE = 3,
            TAXA_ENTREGA_INEXISTENTE = 4,
            PAGAMENTO_RECUSADO = 5,
            RESERVA_CANCELADA = 6,
            FALHA_SEGURO_MONDIAL = 7,
            PAGAMENTO_NAO_REALIZADO = 8
        }

        public string RedePreferencial { get; set; }

        //public enumTipoPagamento TipoPagamento { get; set; }
        private enumTipoPagamento _TipoPagamento;
        ...
        private IRLib.Sitef ositef { get; set; }

        public IRLib.Sitef oSitef
        {
            get
            {
                if (ositef != null)
                ...
                ositef = new IRLib.Sitef()
                {
                    terminal = this.ClienteID.ToString("00000000"),
                    Empresa = IRLib.Sitef.enumEmpresa.IngressoRapido,
                    ...
                    Bandeira = (IRLib.Sitef.enumBandeira)Enum.Parse(typeof(IRLib.Sitef.enumBandeira), this.Bandeira),
                    NumeroCartao = this.NumeroCartao,
                    DataVencimento = this.DataVencimento,
                    CodigoSeguranca = this.CodigoSeguranca,
                    TipoFinanciamento = IRLib.Sitef.enumTipoFinanciamento.Estabelecimento,
        ...
        public int FormaPagamentoOrigem { get; set; }

        public static EstruturaPagamento Montar
            (
                enumTipoPagamento tipoPagamento,
                EstruturaCompraTemporaria oCompraTemporaria,
                decimal valorTotalVir,
                string numeroCartao,
                string validadeCartao,
                string codigoSeguranca,
                string nomeCartao,
                int cartaoID,
                bool cartaoOutraPessoa,
                string PayerID,
                string Token,
                string IP,
                decimal Diferenca,
            ...
            )
        {
            return new EstruturaPagamento()
            {
                ...
                NomeCartao = string.IsNullOrEmpty(nomeCartao) ? string.Empty : nomeCartao.ToUpper(),
                CartaoID = cartaoID,
                ...
                RedePreferencial = new IRLib.FormaPagamento().RedePreferencialFormaPagamentoID(oCompraTemporaria.FormaPagamentoID, IRLib.Sitef.Loja)
            };
        }
    }
}