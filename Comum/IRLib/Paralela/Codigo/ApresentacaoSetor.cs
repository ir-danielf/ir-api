    ...
    public class ApresentacaoSetor : ApresentacaoSetor_B
    {
        ...
        public override string Mapa()
        {
            try
            {
                ...
                string SQL = string.Format(@"SELECT a.LugarID, MAX(a.BloqueioID) AS BloqueioID, a.Quantidade, a.PosicaoX, a.PosicaoY, a.Codigo, a.PerspectivaLugarID, a.DescricaoPerspectiva 
                    FROM (SELECT DISTINCT tIngresso.LugarID, tIngresso.BloqueioID, tLugar.Quantidade, tLugar.PosicaoX, tLugar.PosicaoY,
                    CASE tSetor.LugarMarcado WHEN 'C' THEN  tIngresso.Codigo ELSE  substring(tIngresso.Codigo,1,patindex('%-%',tIngresso.Codigo)-1) END As Codigo, IsNull(tLugar.PerspectivaLugarID, 0) AS PerspectivaLugarID, IsNull(tPerspectivaLugar.Descricao, '') AS DescricaoPerspectiva
                    ...
                    GROUP BY a.LugarID, a.Quantidade, a.PosicaoX, a.PosicaoY, a.Codigo, a.PerspectivaLugarID, a.DescricaoPerspectiva
        ...
        public DataTable PrecosQtdes()
        {
            ...
            try
            {
                ...
                string sql = "SELECT tPreco.ID, Count(tIngresso.ID) AS Qtde " +
                    "FROM tPreco (nolock) " +
                    "LEFT JOIN tIngresso (nolock) ON tPreco.ID = tIngresso.PrecoID AND tIngresso.ApresentacaoSetorID=" + this.Control.ID + " " +
                    "WHERE tPreco.ApresentacaoSetorID=" + this.Control.ID + " " +
        ...
        /// </summary>
        /// <returns></returns>
        public override DataTable Precos(int apresentacaoid, int setorid)
        {
        ...
        public void AtualizarPrecoPrincipal(BD bd, int precoID)
        {
            //#MapaLugarMarcado
            try
            {
                if (this.Control.ID == 0)
                    throw new Exception("O Vinculo Apresentação e Setor não foi gerado.");
