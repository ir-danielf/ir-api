...
namespace IRLib.Paralela
{
    ...
    public class Motivo : Motivo_B
    {
        ...
        public List<ClientObjects.EstruturaMotivo> MotivoByTipo(TiposMotivos Tipo)
        {
            ...
            ClientObjects.EstruturaMotivo oMotivo;

            try
            {
                string sql = "SELECT ID, Motivo  FROM tMotivo(nolock) WHERE Tipo = '" + Tipo + "'";

                bd.Consulta(sql);

                while (bd.Consulta().Read())
                {
                    oMotivo = new ClientObjects.EstruturaMotivo();
                    //popula objeto
                    oMotivo.ID = bd.LerInt("ID");
                    oMotivo.motivo = bd.LerString("Motivo");
                    oMotivo.tipo = ((char)Tipo).ToString();
                    lista.Add(oMotivo);
                }
                return lista;
