...
namespace IRLib.Paralela
{
    ...
    public class IngressoCliente : IngressoCliente_B
    {
        ...
        public int[] QuantidadeJaCompradaNovo(int CotaItemID, int CotaItemIDAPS)
        {
            try
            {
                int[] qtds = new int[4] { 0, 0, 0, 0 };
                string strSQL = "EXEC sp_getQuantidadePorClienteCotaNovo " + this.ApresentacaoID.Valor + ", " + this.ApresentacaoSetorID.Valor + ", " + this.DonoID.Valor + ", " + CotaItemID + ", " + CotaItemIDAPS;
                bd.Consulta(strSQL);
                if (bd.Consulta().Read())
                {
                    qtds[0] = bd.LerInt("Apresentacao");
