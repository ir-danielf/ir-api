...
namespace IRLib.Paralela
{
    public class FormaPagamento : FormaPagamento_B
    {
        ...
        public IRLib.Paralela.ClientObjects.EstruturaVendaFormaPagamento getFormaPagamentoVendaAssinatura(string bandeira, int parcelas, int empresaID, int clienteID, int usuarioID)
        {
            try
            {
        ...
        public IRLib.Paralela.ClientObjects.EstruturaVendaFormaPagamento getFormaPagamentoVenda(int id, int empresaID)
        {
            try
            {
                IRLib.Paralela.ClientObjects.EstruturaVendaFormaPagamento Retorno = new IRLib.Paralela.ClientObjects.EstruturaVendaFormaPagamento();
                StringBuilder stbSQL = new StringBuilder();
        ...
        public DataSet CarregarCreditoBoleto()
        {
            try
            {
                ...
                string sql = string.Format(@"SELECT 
							   ...
							   INNER JOIN tBandeira b (NOLOCK) ON b.ID = fp.BandeiraID
							   INNER JOIN tFormaPagamentoTipo t (NOLOCK) ON t.ID = fp.FormaPagamentoTipoID
							   WHERE fp.ID <> {0} and Tipo in (0,3,4) 
							   ORDER BY fp.Nome ", FormaPagamento.VALE_INGRESSO);
