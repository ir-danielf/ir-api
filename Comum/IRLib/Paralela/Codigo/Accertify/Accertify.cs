using CTLib;
using IRLib.Paralela.HammerHead;
using System;
...
namespace IRLib.Paralela
{
    public class Accertify
    {
        ...
        public static IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify ParseRetorno(string recommendation)
        {
            try
            {
                var retorno = (IRLib.Paralela.RetornoAccertify.Recommendation)Enum.Parse(typeof(IRLib.Paralela.RetornoAccertify.Recommendation), recommendation);

                switch (retorno)
                {
                    case IRLib.Paralela.RetornoAccertify.Recommendation.ACCEPT:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.Aceitar;
                    case IRLib.Paralela.RetornoAccertify.Recommendation.REJECT:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.CancelarAltoRisco;
                    case IRLib.Paralela.RetornoAccertify.Recommendation.REVIEW:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.AguardarReview;
                    case IRLib.Paralela.RetornoAccertify.Recommendation.FOLLOWUP:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.AcompanhamentoComCliente;
                    case IRLib.Paralela.RetornoAccertify.Recommendation.CANCEL:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.CancelarSemFraude;
                    default:
                        return IRLib.Paralela.HammerHead.Enumeradores.RetornoAccertify.AguardarReview;
                }
            }
            ...
            try
            {
                ...
                do
                {
                    ...
                    if (!string.IsNullOrEmpty(bd.LerString("Email")))
                        stbEmail.Append(bd.LerString("Email").ToCleanString().ToUpper().Trim() + Environment.NewLine);

                    if (!string.IsNullOrEmpty(bd.LerString("Telefone")))
                        stbTelefone.Append(bd.LerString("Telefone").ToCleanString().Trim() + Environment.NewLine);
