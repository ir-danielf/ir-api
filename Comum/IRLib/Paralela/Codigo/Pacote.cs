    ...
    public class Pacote : Pacote_B
    {
        ...
        public ClientObjects.EstruturaPacote GravarPacote(ClientObjects.EstruturaPacote estruturaPacote)
        {
            ...
            try
            {
                ...
                ClientObjects.EstruturaPacotePreco oPacotePreco = new IRLib.Paralela.ClientObjects.EstruturaPacotePreco();
                ...
                List<ClientObjects.EstruturaPacoteCanal> lstPacoteCanalSelecionado = new List<IRLib.Paralela.ClientObjects.EstruturaPacoteCanal>();
        ...
        */
        ...
        public string ImportarAssinatura(XmlNode clienteInfos, int canalID, int lojaID, int empresaID, int usuarioID, int caixaID)
        {
            ...
            try
            {
                ...
                DataSet estruturaVenda = IRLib.Paralela.BilheteriaParalela.EstruturaReservas();
                //esse DataSet serve apenas como auxiliar para o preenchimento da estrutruraVenda
                DataSet estruturaVendaAux = estruturaVenda.Clone();

                //tabelas para a venda das reservas
                DataTable tGridLeve;
                DataTable tReserva = estruturaVenda.Tables[IRLib.Paralela.BilheteriaParalela.TABELA_RESERVA];
                DataTable tImpressao = estruturaVenda.Tables[IRLib.Paralela.BilheteriaParalela.TABELA_ESTRUTURA_IMPRESSAO];
                DataTable tEventoTaxaEntrega = estruturaVenda.Tables[IRLib.Paralela.BilheteriaParalela.TABELA_EVENTO_TAXA_ENTREGA];

                //Informações necessarias para a busca de ingressos da assinatura desejada.
                IRLib.Paralela.ClientObjects.EstruturaAssinaturaInfo assinaturasInfoItem;
                ...
                {
                    List<ClientObjects.EstruturaAssinaturaDetalhe> assinaturasCliente = new List<IRLib.Paralela.ClientObjects.EstruturaAssinaturaDetalhe>();
                    //busca os Nomes,Preços e Setores das assinaturas do cliente atual.
                    foreach (XmlNode node in clienteInfos["Assinaturas"].GetElementsByTagName("Assinatura"))
                    {
                        ClientObjects.EstruturaAssinaturaDetalhe assinatura = new IRLib.Paralela.ClientObjects.EstruturaAssinaturaDetalhe();
                    ...
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.EVENTO_PACOTE);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.HORARIO);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.SETOR_PRODUTO);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.CODIGO);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.PRECO);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.CORTESIA);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.VALOR);
                    tGridLeve.Columns.Remove(IRLib.Paralela.BilheteriaParalela.LUGAR_MARCADO);
                    Dictionary<int, int> dicionario = new Dictionary<int, int>();//precisa desse dicionario para a venda
        ...
        /// <returns></returns>
        private IRLib.Paralela.ClientObjects.EstruturaAssinaturaInfo PesquisarAssinaturaSSP(string assinaturaNome, string setorNome, string precoNome)
        {
            try
            {
                IRLib.Paralela.ClientObjects.EstruturaAssinaturaInfo retorno = new IRLib.Paralela.ClientObjects.EstruturaAssinaturaInfo();
                ...
                foreach (DataRow item in itens.Rows)
                {
                    ...
                    if (listaCotaItem.Count != 0 && mudou)
                    {
                        ...
                        if (lstCotasItemIDs.Count > 0 && (lstCotasItemIDs[0].Equals(-1) || lstCotasItemIDs[1].Equals(-1)))
                            throw new BilheteriaException("Um dos preços do pacote selecionado excedeu o limite de Reserva/Venda.", BilheteriaParalela.CodMensagemReserva.PrecoIndisponivel);
                    }
                ...
                foreach (DataRow item in itens.Rows)
                {
                    ...
                    if (listaCotaItem.Count != 0 && mudou)
                    {
                        ...
                        if (lstCotaItemIDs.Count > 0 && (lstCotaItemIDs[0].Equals(-1) || lstCotaItemIDs[1].Equals(-1)))
                            throw new BilheteriaException("A quantidade de venda do preço \"" + precoNome + "\" foi excedida.", BilheteriaParalela.CodMensagemReserva.PrecoIndisponivel);
                    }
            ...
            try
            {
                ...
                for (int i = 0; i < quantidade; i++)
                {
                    ...
                    foreach (DataRow item in itens.Rows)
                    {
                        ...
                        cotasInfo.QuantidadeApresentacaoSetor = Convert.ToInt32(item["QuantidadeApresentacaoSetor"]);
                        cotasInfo.QuantidadePorClienteApresentacaoSetor = Convert.ToInt32(item["QuantidadePorClienteApresentacaoSetor"]);
                        decimal valorTaxaProcessamento = pctTaxa == 0 ? 0 : Convert.ToDecimal(item["ValorTaxaProcessamento"]); //Não tem taxa de conveniencia? Ignora a taxa e processamento
                        ...
                        ePreco.ID = precoID;
                        ePreco.Quantidade = qtde;
                        ePreco.QuantidadeMapa = 1 * quantidade;
                        ...
                        // Busca a qtd de ingressos do item.
                        string sql = "SELECT Top " + qtde + " tIngresso.ID, Codigo, EventoID, ApresentacaoID, SetorID " +
                            "FROM tIngresso (NOLOCK) " +
                            "WHERE tIngresso.ApresentacaoSetorID=" + apresentacaoSetorID + " AND tIngresso.Status='" + Ingresso.DISPONIVEL + "' " +
                            "ORDER BY Codigo";
                        ...
                        else
                        {
                            foreach (Ingresso id in ids)
                            {
                                ...
                                ingresso.PacoteID.Valor = this.Control.ID;
                                CodigoBarra oCodigoBarra = new CodigoBarra();
                                //ingresso.CodigoBarra.Valor = oCodigoBarra.NovoCodigoBarraIngresso(precoID, eventoID);
                                ingresso.PacoteGrupo.Valor = pacoteGrupo.ToString();
                                ingresso.LojaID.Valor = lojaID;
                                ingresso.SessionID.Valor = sessionID;
                                ingresso.ClienteID.Valor = clienteID;
                                ...
                                string sqlReservar = ingresso.StringReservar();
                                int x = bdReserva.Executar(sqlReservar);
                                bool okR = (x == 1);
                                if (!okR)
                                {
                                    bdReserva.DesfazerTransacao();
        ...
        public DataTable InfoReservado(int[] ingressosIDs, CTLib.BD bd, List<EstruturaIngressoCotaItem> lstIngressoCotaItem, int lojaID)
        {
            ...
            try
            {
                ...
                if (ingressosIDs != null && ingressosIDs.Length > 0)
                {
                    ...
                    tabela.Columns.Add(BilheteriaParalela.COTA_ITEM_ID, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADE_COTA, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADEPORCLIENTE_COTA, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADE_APRESENTACAO, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADEPORCLIENTE_APRESENTACAO, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADE_APRESENTACAO_SETOR, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.QUANTIDADEPORCLIENTE_APRESENTACAO_SETOR, typeof(int)).DefaultValue = 0;
                    tabela.Columns.Add(BilheteriaParalela.VALIDA_BIN, typeof(bool)).DefaultValue = false;
                    tabela.Columns.Add(BilheteriaParalela.NOMINAL, typeof(bool)).DefaultValue = 1;
                    ...
                    foreach (DataRow dtr in tabela.Rows)
                    {
                        for (int i = 0; i < lstIngressoCotaItem.Count; i++)
                        {
                            ...
                            {
                                dtr[BilheteriaParalela.COTA_ITEM_ID] = lstIngressoCotaItem[i].CotaItemID;
                                dtr[BilheteriaParalela.QUANTIDADE_COTA] = lstIngressoCotaItem[i].QuantidadeCotaItem;
                                dtr[BilheteriaParalela.QUANTIDADEPORCLIENTE_COTA] = lstIngressoCotaItem[i].QuantidadePorCliente_CotaItem;
                                dtr[BilheteriaParalela.QUANTIDADE_APRESENTACAO] = lstIngressoCotaItem[i].Quantidade_Apresentacao;
                                dtr[BilheteriaParalela.QUANTIDADE_APRESENTACAO_SETOR] = lstIngressoCotaItem[i].Quantidade_ApresentacaoSetor;
                                dtr[BilheteriaParalela.QUANTIDADEPORCLIENTE_APRESENTACAO] = lstIngressoCotaItem[i].QuantidadePorCliente_Apresentacao;
                                dtr[BilheteriaParalela.QUANTIDADEPORCLIENTE_APRESENTACAO_SETOR] = lstIngressoCotaItem[i].QuantidadePorCliente_ApresentacaoSetor;
                                dtr[BilheteriaParalela.NOMINAL] = lstIngressoCotaItem[i].Nominal;
                                ...
                                if (cotaItemIDaux != lstIngressoCotaItem[i].CotaItemID)
                                {
                                    ...
                                    dtr[BilheteriaParalela.VALIDA_BIN] = validaBinaux;
                                }
                                else
                                    dtr[BilheteriaParalela.VALIDA_BIN] = validaBinaux;
                                break;
