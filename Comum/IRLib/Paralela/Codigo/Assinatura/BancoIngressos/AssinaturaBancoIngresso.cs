    ...
    public class AssinaturaBancoIngresso : AssinaturaBancoIngresso_B
    {
        ...
        public List<CalendarioIngressos> MontarCalendarioIngressos(int ano, int mes, int horaMinima, int clienteID)
        {
            try
            {
                ...
                DateTime dataInicial = (DateTime.Now - dataPesquisa).Milliseconds <= 0 ? dataPesquisa : DateTime.Now;
                dataInicial.AddHours(horaMinima);
                ...
                string sql = string.Format(@"
						SELECT DISTINCT 
								...
								WHERE abi.ClienteID = {2} AND ap.Horario BETWEEN '{0}' AND '{1}' AND abi.Aprovado = 'T' AND bir.ID IS NULL
								ORDER BY ap.Horario
					   ", dataInicial.Date.ToString("yyyyMMddHHmmss"), dataPesquisa.ToString("yyyyMM") + "31999999", clienteID);
        ...
        public List<DataDoacao> CarregarIngressos(int dia, int mes, int ano, int clienteID, int horaMinima)
        {
            try
            {
                ...
                data.AddHours(DateTime.Now.Hour + horaMinima);
                ...
                string sql = string.Format(@"
	                ...
	                WHERE ap.Horario BETWEEN '{0}' AND '{1}' AND bi.ClienteID = {2}
	                ORDER BY ap.Horario, s.Nome", data.ToString("yyyyMMddHHmmss"), data.AddDays(1).Date.ToString("yyyyMMddHHmmss"), clienteID);
        ...
        public List<ApresentacaoProgramacao> ListarProgramacao(int assinaturaTipoID, string anoAtual)
        {
            try
            {
                string sql = @"SELECT DISTINCT 
                                ...
                                Where a.AssinaturaTipoID = " + assinaturaTipoID + @" and an.Ano =  " + anoAtual + @"
        ...
        /// <param name="horaMinima"></param>
        /// <returns></returns>
        public List<IRLib.Paralela.ClientObjects.EstruturaIDNome> CarregarProgramacoes(int dia, int mes, int ano, int horaMinima)
        {
            try
            {
                DateTime data = new DateTime(ano, mes, dia);
