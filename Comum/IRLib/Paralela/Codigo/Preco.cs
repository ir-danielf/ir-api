...
namespace IRLib.Paralela
{
    ...
    public class Preco : Preco_B
    {
        ...
        public void Atualizar(int eventoID, int setorID, int apresentacaoID, int precoID,
            int todosID, int[] canaisSelecionados, string canaisNaoSelecionados, IRLib.Paralela.ApresentacaoSetor.enumAcaoPrecoPrincipal acaoPrecoPrincipal)
        {
            ...
            try
            {
                ...
                foreach (DataRow precoAlteravel in tabelaPrecosAlteraveis.Rows)
                {
                    ...
                    if ((precoID != todosID) && valorOk)
                    {
                        ...
                        if (precoID != todosID)
                        {
                            switch (acaoPrecoPrincipal)
                            {
                                case ApresentacaoSetor.enumAcaoPrecoPrincipal.Remover:
                                    if (recarregarAPS)
                                        apresentacaoSetor.LerNolock((int)precoAlteravel["ApresentacaoSetorID"]);

                                    if (apresentacaoSetor.PrincipalPrecoID.Valor == (int)precoAlteravel["PrecoID"])
                                    {
                                        apresentacaoSetor.PrincipalPrecoID.Valor = 0;
                                        apresentacaoSetor.Atualizar(bd);
                                    }
                                    break;
                                case ApresentacaoSetor.enumAcaoPrecoPrincipal.Adicionar:
                                    if (recarregarAPS)
                                    {
                                        apresentacaoSetor.LerNolock((int)precoAlteravel["ApresentacaoSetorID"]);
                                        apresentacaoSetor.PrincipalPrecoID.Valor = preco.Control.ID;
                                        apresentacaoSetor.Atualizar(bd);
                                    }
                                    break;
                            }
        ...
        /// </summary>
        /// <returns></returns>	
        public int Inserir(int eventoID, int setorID, int[] iHorarios, int todosID,
            bool codigoBarraEstruturado, int[] canaisSelecionados, IRLib.Paralela.ApresentacaoSetor.enumAcaoPrecoPrincipal acaoPrecoPrincipal)
        {
            BD bd = new BD();
            ...
            try
            {
                ...
                while (bdTemp.Consulta().Read())
                {
                    ...
                    { //nao eh null, pode incluir
                        ...
                        {
                            if (acaoPrecoPrincipal == ApresentacaoSetor.enumAcaoPrecoPrincipal.Adicionar)
                            {
                                apresentacaoSetor.LerNolock(this.ApresentacaoSetorID.Valor);
                                apresentacaoSetor.PrincipalPrecoID.Valor = this.Control.ID;
                                apresentacaoSetor.Atualizar(bd);
                            }
                            qtde++;
                        }
        ...
        public bool DistribuirCanaisPrecoIR(List<EstruturaCanaisPreco> canaisPreco)
        {
            try
            {
                CanalPreco canalPreco = new CanalPreco(this.Control.UsuarioID);
            ...
            try
            {
                ...
                foreach (int precoID in listPrecoID)
                {
                    ...
                    foreach (int CanalID in CanaisIDDeletar)
                    {
                        ...
                        oRoboCanalPreco.Operacao.Valor = Convert.ToChar(RoboCanalPreco.operacaobanco.Deleletar).ToString();
